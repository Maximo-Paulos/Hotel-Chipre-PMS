...
export function ResetPasswordPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      // Validamos el código. (El cambio de password real no está implementado en backend en esta etapa.)
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg ring-1 ring-slate-100">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? "Validando..." : "Guardar nueva contraseña"}
          </button>
        </form>
        {saved && <p className="mt-3 rounded-md bg-emerald-50 p-3 text-sm text-emerald-700">Código válido. Redirigiendo al login.</p>}
