...
import { changeSubscriptionPlan, type SubscriptionPlan } from "../../api/subscription";
...
export default function SettingsSubscriptionPage() {
  ...
  const availablePlans = (plansQuery.data?.length ? plansQuery.data : subscription?.available_plans) ?? [];
  ...
  const inactive = subscription && subscription.status !== "active";
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">Administrá el plan, estado y límites del PMS.</p>
        </div>
        {isMock && (
      ...
      {(writeBlocked || inactive) && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <p>
            {writeBlocked
              ? "Suscripción en modo solo lectura (can_write=false). Las acciones de escritura están deshabilitadas."
              : "Suscripción inactiva. Reactivá o cambiá de plan para operar sin bloqueos."}
          </p>
              ...
              <li className="flex items-center gap-2">
                ...
                <span
                  className={`rounded-full px-2 py-1 text-xs font-semibold ${
                    subscription.status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-amber-100 text-amber-800"
                  }`}
                >
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        type="button"
                        disabled={isCurrent || updating || writeBlocked}
                      ...
                      >
                        {writeBlocked ? "Bloqueado" : isCurrent ? "Plan actual" : "Elegir plan"}
                      </button>
                      <button
                        type="button"
