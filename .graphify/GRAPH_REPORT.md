...
## Corpus Check
- Large corpus: 792 files · ~535,813 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 7620 nodes · 26514 edges · 316 communities detected
- Extraction: 61% EXTRACTED · 39% INFERRED · 0% AMBIGUOUS · INFERRED: 10314 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 10314 · contains: 4603 · calls: 3482 · MODIFIES: 2099 · ON_BRANCH: 1726 · rationale_for: 1293 · imports: 761 · imports_from: 601 · method: 593 · inherits: 535 · PARENT_OF: 507


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 792 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 9 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `672b76d`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `Reservation` - 665 edges
2. `ReservationStatusEnum` - 664 edges
3. `HotelConfiguration` - 607 edges
4. `Room` - 515 edges
5. `Guest` - 489 edges
6. `RoomCategory` - 474 edges
7. `Base` - 402 edges
8. `RoomStatusEnum` - 326 edges
9. `TransactionTypeEnum` - 317 edges
...
Cohesion: 0.02
Nodes (247): claude/feature/pms-roles-housekeeping-qa, codex/feature/pms-e2e-scale-hardening, 00bf891 Explain analytics errors and add retry action, 0255d4f Normalize Graphify artifact paths, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 0391f8f Fix flaky detached-node false positive in mobile touch-target check, 03ec9de Refresh Graphify after Fase 9 role QA fixes (+239 more)

### Community 1 - "Community 1"
Cohesion: 0.05
Nodes (205): deactivate_payment_surcharge(), _get_surcharge_or_404(), PaymentSurchargeCreate, PaymentSurchargeRead, PaymentSurchargeUpdate, Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, update_payment_surcharge(), 62f0868 fix(payments): record WHO triggered a payment state change, not just when (+197 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (157): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+149 more)
...
Cohesion: 0.04
Nodes (130): Public booking-engine API authenticated only with hotel API keys., Resolve today's effective rate from the single source of truth and attach it, Update room status for housekeeping. When a room is set to cleaning/maintenance/, Manually trigger the allocation engine to optimally redistribute all reservation, DocumentTypeEnum, Reservation, ReservationSourceEnum, Room (+122 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (149): UpdateRolePayload, BaseModel, 805b47a Unify reservation pricing with signed quote contract, BillingPolicyPayload, BillingPolicyUpdateRequest, EmailTestRequest, MasterAdminLoginRequest, MasterAdminLoginResponse (+141 more)

### Community 6 - "Community 6"
Cohesion: 0.04
Nodes (117): CheckInRequest, FastAPI routes for Check-in / Check-out., add_companions(), FastAPI routes for Guest management., Add new companions to an existing guest., Add new companions to an existing guest., Guest, GuestRatingEnum (+109 more)

### Community 7 - "Community 7"
Cohesion: 0.04
Nodes (115): Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, Base, Base class for all ORM models., DeclarativeBase, GuestCompanion (+107 more)

### Community 8 - "Community 8"
Cohesion: 0.03
Nodes (99): AllocationRunPayload, AllocationRunResponse, listRoomMovementGroups(), revertRoomMovementGroup(), RoomMoveEvent, RoomMovementGroup, triggerAllocationRecalculation(), Category (+91 more)
...
Cohesion: 0.02
Nodes (40): _DecimalAwareEncoder, _is_demo_mode_enabled(), Hotel PMS — FastAPI Main Application. Serves the API + bundled frontend files., Check whether demo-only utilities should be exposed., Check whether demo-only utilities should be exposed., Check whether demo-only utilities should be exposed., Check whether demo-only utilities should be exposed., _require_demo_mode() (+32 more)

### Community 11 - "Community 11"
Cohesion: 0.03
Nodes (80): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, acceptInvitation(), AuthResponse, AuthUser, _build_auth_response(), _generate_code(), getInvitationInfo(), _issue_email_token() (+72 more)

### Community 12 - "Community 12"
Cohesion: 0.03
Nodes (35): _require_demo_mode(), reset_demo(), seed_demo(), Public WhatsApp bot hooks authenticated only by hotel API key., _backfill_not_null_nulls(), _column_fill_value(), get_db(), get_engine() (+27 more)

### Community 13 - "Community 13"
Cohesion: 0.11
Nodes (79): ReservationAllocationLock, Company, Operational reservation models for adjustments, room moves and audit history., Groups multiple RoomMoveEvents triggered by the same cause (v72 §5.4).     Suppo, ReservationAdjustment, ReservationAdjustmentKindEnum, ReservationAdjustmentStatusEnum, ReservationStatusHistory (+71 more)

### Community 14 - "Community 14"
Cohesion: 0.03
Nodes (38): 307e650 Add replayable operational warehouse projections, 7e2edb6 Merge Analytics R1.0, b5bc141 Harden core tenant composite relationships, df173d1 Implement Analytics R1.0 dashboard, chat IA, and support, HotelConfiguration — per-hotel business rules. No global/singleton: one row per, Room and RoomCategory models., AnalyticsComparisonStateRead, AnalyticsComparisonWindowRead (+30 more)
...
Cohesion: 0.05
Nodes (56): add_reservation_guests(), buildQueryString(), cancel_reservation(), cancelReservation(), change_dates(), checkInReservation(), checkOutReservation(), clearReservationManualReview() (+48 more)
...
Cohesion: 0.04
Nodes (21): 1cd0fce Merge claude/feature/pms-roles-housekeeping-qa: integrate parallel QA fixes, 32f7bba Require checkin:perform permission for checkout, not just authentication, 3f48c33 chore: snapshot current implementation state for analysis, c2f87d1 Grant receptionist read access to room categories, f283a66 Fix full-suite cash-register race conditions exposed by the new specs, housekeeping, owner, manager (+13 more)

### Community 24 - "Community 24"
Cohesion: 0.05
Nodes (47): SimpleRateLimiter, Base, MasterAdminAuditEvent, MasterAdminAuthLockout, MasterAdminSession, MasterBillingPolicy, MasterStripeSettings, MasterStripeWebhookEvent (+39 more)
...
Cohesion: 0.07
Nodes (31): ABC, Validate reset code without changing password (paso previo en UI)., Validate reset code without changing password (paso previo en UI)., validate_reset(), EmailProvider, EmailProviderError, get_email_provider(), _mask_email() (+23 more)
...
Cohesion: 0.06
Nodes (22): cancelPaymentLinkTest(), createMercadoPagoPaymentLinkTest(), listPaymentLinkTests(), PaymentLinkTest, PaymentLinkTestCreatePayload, refreshPaymentLinkTest(), 7a43bdd feat: add billing, integrations, and Mercado Pago test flows, IntegrationHelpDrawer() (+14 more)
...
Cohesion: 0.07
Nodes (26): graphify_state(), main(), main(), normalize_instruction_paths(), Replace this repository's absolute root in generated instructions only., main(), run(), validate_json() (+18 more)

### Community 44 - "Community 44"
Cohesion: 0.09
Nodes (33): _derive_payment_status(), public_reservation_status(), public_reservation_status_by_code(), Coarse payment status derived from amounts (no sensitive detail)., Coarse payment status derived from amounts (no sensitive detail)., Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by id (v72 §16).      Scoped to the API key (+25 more)
...
Cohesion: 0.08
Nodes (24): updateHotelConfig(), _attach_current_rate(), createRoom(), createRoomCategory(), get_category(), list_categories(), listRoomCategories(), listRooms() (+16 more)
...
Cohesion: 0.09
Nodes (16): create_movement(), CurrentStock, DecimalValue, _ensure_adjustment_permission(), FastAPI routes for stock and inventory operations., StockItem, StockItemCreate, StockItemRead (+8 more)
...
Cohesion: 0.34
Nodes (27): OTAConnection, OTAProvider, OTARatePlanMapping, OTAReservationLifecycleEnum, OTAReservationLink, OTARoomMapping, OTASyncStatusEnum, OTAWebhookCredential (+19 more)
...
Cohesion: 0.08
Nodes (20): createStockItem(), createStockLocation(), createStockMovement(), getCurrentStock(), listLowStockItems(), listStockItems(), listStockLocations(), listStockMovements() (+12 more)

### Community 76 - "Community 76"
Cohesion: 0.21
Nodes (25): AuditLog, Append-only record of every significant mutation within a hotel tenant.     Do N, PaymentProof, PaymentProofBlob, PaymentProofStatusEnum, Private image evidence awaiting explicit staff confirmation., Private image evidence awaiting explicit staff confirmation., Private binary payload kept separately from queryable proof metadata. (+17 more)
...
Cohesion: 0.11
Nodes (12): eed124a Harden cash reconciliation and transfer proof approvals, Manual bank-transfer evidence and approval lifecycle., approve_transfer_proof(), _decode_image(), get_transfer_proof(), proof_file_path(), _proof_path(), reject_transfer_proof() (+4 more)
...
Cohesion: 0.12
Nodes (21): useSession(), broadcastDomainChange(), DOMAIN_QUERY_PREFIXES, domainForPath(), getSenderId(), invalidateDomainQueries(), normalizeHotelId(), notifyIfRelevant() (+13 more)
...
Cohesion: 0.13
Nodes (20): 4959706 Enforce tenant isolation with PostgreSQL RLS context, f01c735 Refresh Graphify after tenant isolation hardening, Bind a SQLAlchemy transaction to the authenticated tenant.  PostgreSQL RLS polic, Set the authenticated user id used by membership RLS policies., Set or clear the current hotel id for tenant-scoped RLS policies., Set both principals for a request or a single-hotel worker job., _set_context_value(), set_tenant_context() (+12 more)
...
Cohesion: 0.10
Nodes (10): 4236cf3 Harden ledger pricing and payment idempotency, 5de6817 Add field-based bulk rate editing, 6cdbbc5 Polish rate calendar UX and fix Vercel install step, bed8c5c feat: BR audit — pricing source of truth, caja, checkout, occupancy, surcharges, df770c3 Polish rate calendar and reservation pricing, mockCalendar, mockCategories, mockDailyRates (+2 more)

### Community 103 - "Community 103"
Cohesion: 0.13
Nodes (21): AuthContext, Dependency factory that enforces a permission via the configurable hotel     per, Dependency factory that enforces a permission via the configurable hotel     per, Dependency factory that enforces a permission via the configurable hotel     per, Dependency to enforce that the current user has one of the allowed roles in the, Dependency to enforce that the current user has one of the allowed roles in the, Dependency to enforce that the current user has one of the allowed roles in the, Minimal request context used by routers that expect a hotel scope. (+13 more)
...
Cohesion: 0.19
Nodes (12): _auth_headers(), _complete_onboarding(), _configure_resend(), FakeResponse, _register_owner(), test_auth_flows_fail_without_connected_mail_provider(), test_login_prefers_a_completed_hotel_when_multiple_memberships_exist(), test_register_rejects_malformed_email() (+4 more)

### Community 108 - "Community 108"
Cohesion: 0.18
Nodes (17): _enum_value(), _event_to_read(), _group_to_read(), list_movement_groups(), MovementEventRead, MovementGroupRead, _not_found_or_bad_request(), read_movement_group() (+9 more)
...
Cohesion: 0.13
Nodes (5): 1f352e4 feat(reports): SLICE-11 operational reports + alerts, 6f4beb1 feat(v72): Wave 3 — public API/keys + allocation V72 rules, a2c31f7 feat(ops): SLICE-04 room blocks + availability/allocation exclusion, c486aa3 feat(v72): Wave 2 — companies/documents, cash register/arqueo, guest override, Add one-open-cash-session partial unique index.  Revision ID: 20260613_cash_open
...
Cohesion: 0.18
Nodes (9): 156ac6e Repair legacy audit timestamp default, 1a0097b Refresh Graphify after transfer QA fixes, 4529a78 Fix hotel payment method settings save, 9d6d084 Repair legacy payment proof schema, c32574b Report model schema drift at startup, d8494fb Document transfer payment QA recovery, f0b096c Refresh Graphify after audit schema repair, credentials (+1 more)
...
Cohesion: 0.40
Nodes (2): AcceptPayload, LoginRequest
...
Cohesion: 0.67
Nodes (3): list_stock_movements(), Return the newest auditable inventory movements for one hotel., Return the newest auditable inventory movements for one hotel.
...
Cohesion: 1.00
Nodes (1): Tenant-scoped audit log for tracking mutations across core tables. Every write t
...
## Knowledge Gaps
- **492 isolated node(s):** `add hotel scope to core tables  Revision ID: 20260404_add_hotel_scope Revises: c`, `add subscription v2 tables  Revision ID: 20260407_subscription_tables Revises: 2`, `launch security hardening  Revision ID: 20260408_launch_security_hardening Revis`, `add sender metadata to payment link tests  Revision ID: 20260408_payment_link_em`, `reservation financial lifecycle  Revision ID: 20260410_reservation_financial_lif` (+487 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 64`** (2 nodes): `_hotel_default_currency()`, `OTAIntegrationService`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 123`** (1 nodes): `OnboardingState`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 135`** (1 nodes): `DespegarAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 138`** (1 nodes): `FastAPI routes for the commercial configuration domain.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (2 nodes): `FakeRedis`, `test_availability_key_shape_and_serialization()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (2 nodes): `mercadopago_payment_link_webhook()`, `_mercadopago_webhook_impl()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 250`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 266`** (2 nodes): `AcceptPayload`, `LoginRequest`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 267`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 269`** (1 nodes): `HotelIdentityPayload`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 284`** (1 nodes): `FakeRedis`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 289`** (1 nodes): `add hotel scope to core tables  Revision ID: 20260404_add_hotel_scope Revises: c`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 290`** (1 nodes): `v72 gaps phase 1: Numeric precision, room score/accessibility, guest dedup+ratin`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 291`** (1 nodes): `v72 gaps phase 2: guest search indexes, OTA dedup constraint, updated guest_tag_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 292`** (1 nodes): `v72 gaps phase 3: room_movement_groups table, BillingAdjustment/ReservationAdjus`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 293`** (1 nodes): `v72 gaps phase 4: PRE_CHECK_IN state, RoomMoveEvent audit fields, company_docume`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 294`** (1 nodes): `v72 gaps phase 5: optimistic locking on reservations  Revision ID: 20260612_v72_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 295`** (1 nodes): `v72 gaps phase 6: payment tables (payments, payment_links, payment_webhook_event`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 296`** (1 nodes): `laundry and stock foundations  Revision ID: 20260613_laundry_stock Revises: 2026`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 297`** (1 nodes): `Add transaction idempotency key for gateway payments.  Revision ID: 20260613_pay`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 298`** (1 nodes): `permission matrix, role boundaries, and security audit log  Revision ID: 2026061`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 299`** (1 nodes): `Add daily rates and price periods.  Revision ID: 20260624_daily_rates Revises: 2`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 300`** (1 nodes): `v72 fx rate snapshots table.  Revision ID: 20260624_fx_rate_snapshots Revises: 2`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 301`** (1 nodes): `v72 section 12.3 - payment_surcharges table.  Revision ID: 20260624_payment_surc`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 302`** (1 nodes): `drop orphan payment_surcharge_configs table (R5b).  main carries TWO surcharge i`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 303`** (1 nodes): `Add configurable public API rate limit per hotel.  Revision ID: 20260625_public_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 304`** (1 nodes): `v72 §16.2: add 'company' value to reservation_channel_code_enum.  Adds the corpo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 305`** (1 nodes): `add reservations.settlement_due_date for corporate deferred billing (R5b §3.5).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 306`** (1 nodes): `reservation waitlist flags (BRM v72 §9).  Add denormalized waitlist flags to res`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 307`** (1 nodes): `soft delete audited domain records.  Revision ID: 20260625_soft_delete Revises:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 308`** (1 nodes): `Store private transfer-proof bytes separately from searchable metadata.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 309`** (1 nodes): `Add transactions.created_by_user_id for payment audit trail.  Transaction had cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 311`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 312`** (1 nodes): `Payment surcharge model - v72 section 12.3.  Defines per-payment-method surcharg`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 313`** (1 nodes): `Build the canonical quote shared by reservation-facing surfaces.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 316`** (1 nodes): `TestCurrentSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 317`** (1 nodes): `TestExportSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 320`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 321`** (1 nodes): `Tenant-scoped audit log for tracking mutations across core tables. Every write t`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 322`** (2 nodes): `Derive the BRM §16.2 reportable origin for a reservation.      Precedence:`, `reportable_origin()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 323`** (1 nodes): `Schemas for manual OTA reservation entry.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 7` to `Community 248`, `Community 12`, `Community 10`, `Community 228`, `Community 214`, `Community 118`, `Community 310`, `Community 24`, `Community 122`, `Community 60`, `Community 13`, `Community 19`, `Community 33`, `Community 76`, `Community 321`, `Community 17`, `Community 15`, `Community 101`, `Community 9`, `Community 147`, `Community 4`, `Community 6`, `Community 5`, `Community 97`, `Community 1`, `Community 14`, `Community 103`, `Community 66`, `Community 123`, `Community 67`, `Community 32`, `Community 84`, `Community 312`, `Community 3`, `Community 322`, `Community 62`, `Community 2`, `Community 49`, `Community 63`, `Community 252`, `Community 81`, `Community 160`, `Community 107`, `Community 86`, `Community 204`, `Community 205`, `Community 130`?**
  _High betweenness centrality (0.130) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 1` to `Community 8`, `Community 266`, `Community 33`, `Community 11`, `Community 5`, `Community 44`, `Community 14`, `Community 7`, `Community 24`, `Community 49`, `Community 105`, `Community 62`, `Community 19`, `Community 6`, `Community 255`, `Community 103`, `Community 88`, `Community 2`, `Community 67`, `Community 64`, `Community 94`, `Community 9`, `Community 13`, `Community 4`, `Community 92`, `Community 63`, `Community 107`, `Community 95`, `Community 76`, `Community 86`, `Community 66`, `Community 32`, `Community 15`, `Community 17`, `Community 41`, `Community 316`, `Community 317`, `Community 114`?**
  _High betweenness centrality (0.052) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 4` to `Community 2`, `Community 6`, `Community 44`, `Community 49`, `Community 33`, `Community 3`, `Community 7`, `Community 24`, `Community 13`, `Community 60`, `Community 62`, `Community 19`, `Community 15`, `Community 67`, `Community 64`, `Community 1`, `Community 76`, `Community 92`, `Community 108`, `Community 217`, `Community 95`, `Community 17`, `Community 41`, `Community 316`, `Community 317`, `Community 114`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Are the 658 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Reservation` has 658 INFERRED edges - model-reasoned connections that need verification._
- **Are the 662 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`ReservationStatusEnum` has 662 INFERRED edges - model-reasoned connections that need verification._
- **Are the 600 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 600 INFERRED edges - model-reasoned connections that need verification._
- **Are the 510 inferred relationships involving `Room` (e.g. with `RoomBlockCreate` and `RoomBlockRead`) actually correct?**
  _`Room` has 510 INFERRED edges - model-reasoned connections that need verification._