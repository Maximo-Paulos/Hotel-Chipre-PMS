...
## Corpus Check
- Large corpus: 1049 files · ~1,660,735 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 10508 nodes · 53663 edges · 360 communities detected
- Extraction: 68% EXTRACTED · 32% INFERRED · 0% AMBIGUOUS · INFERRED: 16964 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 16964 · ON_BRANCH: 16199 · contains: 6094 · calls: 4537 · MODIFIES: 3688 · rationale_for: 2225 · imports: 1091 · imports_from: 844 · PARENT_OF: 735 · inherits: 666 · method: 617 · re_exports: 3


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 1049 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 12 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `b6cd93c`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `Reservation` - 963 edges
2. `ReservationStatusEnum` - 959 edges
3. `HotelConfiguration` - 940 edges
4. `Room` - 784 edges
5. `Guest` - 745 edges
6. `RoomCategory` - 718 edges
7. `Base` - 550 edges
8. `RoomStatusEnum` - 488 edges
9. `ReservationError` - 438 edges
10. `ReservationSourceEnum` - 382 edges
...
Cohesion: 0.02
Nodes (518): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true). (+510 more)

### Community 1 - "Community 1"
Cohesion: 0.17
Nodes (375): agent-linensplit, agent-login, agent-mobilenav, agent-otafix, agent-settlement, agent-stockedit, agent-stocksplit, agent-ux (+367 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (324): PaymentSurchargeCreate, daily_report(), occupancy_report(), FastAPI routes for Reports & Night Audit. Daily summaries, occupancy reports, re, Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c, Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c, Occupancy report for a date range (default: last 30 days)., Occupancy report for a date range (default: last 30 days). (+316 more)
...
Cohesion: 0.01
Nodes (223): AllocationRunPayload, AllocationRunResponse, listRoomMovementGroups(), revertRoomMovementGroup(), RoomMoveEvent, RoomMovementGroup, triggerAllocationRecalculation(), Category (+215 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (111): cancelPaymentLinkTest(), createMercadoPagoPaymentLinkTest(), listPaymentLinkTests(), PaymentLinkTest, PaymentLinkTestCreatePayload, refreshPaymentLinkTest(), _DecimalAwareEncoder, Hotel PMS — FastAPI Main Application. Serves the API + bundled frontend files. (+103 more)

### Community 7 - "Community 7"
Cohesion: 0.01
Nodes (191): acceptInvitation(), AuthResponse, _burn_auth_timing_work(), _generate_code(), getInvitationInfo(), _issue_email_token(), loginWithGoogle(), _mask_email() (+183 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (254): Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, Subscription status and entitlements endpoints., Base, Base class for all ORM models., lifespan() (+246 more)

### Community 9 - "Community 9"
Cohesion: 0.04
Nodes (247): AuthUser, accept_invitation(), _available_invitation(), get_invitation(), _assert_assignable_role(), _assert_manageable_membership(), invite_user(), inviteUser() (+239 more)
...
Cohesion: 0.02
Nodes (145): FxSnapshotRead, create_laundry_remito(), _housekeeping_remito(), LinenItemCreate, LinenItemRead, LinenLocationCreate, LinenLocationRead, LinenMovementCreate (+137 more)

### Community 12 - "Community 12"
Cohesion: 0.04
Nodes (109): booking_webhook(), despegar_webhook(), expedia_webhook(), _guarded_json_payload(), _handle_ota_webhook(), FastAPI Webhook endpoints for OTA integrations., Receive reservation notifications from Booking.com., Receive reservation notifications from Expedia. (+101 more)
...
Cohesion: 0.02
Nodes (83): _ensure_wide_version_table(), get_url(), Alembic creates alembic_version with version_num VARCHAR(32) by default.     Som, run_migrations_offline(), run_migrations_online(), _booking_to_read(), cancel_booking(), checkin_booking() (+75 more)

### Community 15 - "Community 15"
Cohesion: 0.02
Nodes (112): ad4249c Implement master admin panel, PermissionGate(), StatCard(), StatCardProps, toneClasses, isAppHostname(), Email provider abstraction for platform transactional mail., clearMasterAdminCsrfToken() (+104 more)

### Community 16 - "Community 16"
Cohesion: 0.07
Nodes (119): HotelPermissionOverride, Permission, Configurable permission matrix models., An employee-specific permission decision within one hotel membership., An employee-specific permission decision within one hotel membership., RolePermissionDefault, UserPermissionOverride, SecurityAuditLog (+111 more)

### Community 17 - "Community 17"
Cohesion: 0.06
Nodes (83): _allow_mfa_attempt(), _attach_user_session_cookies(), _audit_security_event(), _build_auth_response(), _build_login_response(), complete_mfa_login(), confirm_mfa_enrollment(), disable_mfa() (+75 more)

### Community 18 - "Community 18"
Cohesion: 0.04
Nodes (59): createPaymentSurcharge(), deactivatePaymentSurcharge(), grossWithSurcharge(), listPaymentSurcharges(), PaymentSurcharge, PaymentSurchargeCreatePayload, PaymentSurchargeType, checkRoomAvailability() (+51 more)

### Community 19 - "Community 19"
Cohesion: 0.05
Nodes (63): ApiKeyPurpose, HotelApiKey, HotelApiKeyIssued, issueApiKey(), IssueHotelApiKeyPayload, listApiKeys(), revokeApiKey(), SessionLike (+55 more)
...
Cohesion: 0.08
Nodes (67): Thin FastAPI transport for the tenant-scoped permission service., Keep permission exceptions aligned with the staff-management boundary., Ask for one exceptional permission without changing the requester's role., Ask for one exceptional permission without changing the requester's role., Tenant-scoped security overview and current-user session revocation., Invalidate every access token previously issued to the caller., Invalidate every access token previously issued to the caller., Invalidate every access token previously issued to the caller. (+59 more)
...
Cohesion: 0.07
Nodes (44): SimpleRateLimiter, MasterAdminAuditEvent, MasterAdminAuthLockout, MasterAdminSession, MasterBillingPolicy, MasterStripeSettings, MasterStripeWebhookEvent, MasterSystemEmailConnection (+36 more)
...
Cohesion: 0.08
Nodes (32): ABC, _authorize_oauth_state_actor(), Revalidate the actor embedded in a short-lived OAuth state token.      Provider, EmailProvider, EmailProviderError, get_email_provider(), _mask_email(), _mask_recipients() (+24 more)
...
Cohesion: 0.08
Nodes (39): approve_temporary_action_grant(), _assert_manageable_membership(), consume_temporary_action_grant(), create_temporary_action_grant(), deny_temporary_action_grant(), EffectivePermissionsResponse, fetchEffectivePermissions(), fetchPermissionMatrix() (+31 more)
...
Cohesion: 0.09
Nodes (35): AcceptPayload, InvitePayload, InviteResponse, PrimaryOwnerTransferPayload, User management per hotel (owners/co-owners)., Transfer billing/property ownership after explicit account reauth., transfer_primary_owner_endpoint(), UpdateRolePayload (+27 more)
...
Cohesion: 0.14
Nodes (15): 2a14eb9 feat(security): add unified tenant-scoped audit timeline read endpoint, AuditTimelineItemRead, AuditTimelineRead, Public, redacted contracts for hotel security settings., RevokeAllSessionsResponse, SecurityCurrentUserRead, SecurityEventRead, SecurityEventsRead (+7 more)
...
Cohesion: 0.33
Nodes (9): _auth(), _client(), test_only_owner_can_use_administration_catalog_and_co_owner_is_denied(), test_owner_can_grant_and_revoke_user_override_then_restore_defaults(), test_owner_can_restore_one_role_override_to_catalog_default_with_audit(), test_owner_can_restore_one_user_override_to_role_default_with_audit(), test_restore_cannot_leave_owner_without_permission_management(), test_restore_override_rejects_stale_expected_version_with_409() (+1 more)
...
- **Thin community `Community 126`** (2 nodes): `ExpediaAdapter`, `OTAProviderAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 128`** (1 nodes): `OnboardingState`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 313`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 318`** (2 nodes): `_override_auth()`, `test_receptionist_can_get_price_quote()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 369`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 370`** (1 nodes): `NEVER_CACHE_PREFIXES`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 8` to `Community 14`, `Community 3`, `Community 6`, `Community 65`, `Community 219`, `Community 10`, `Community 31`, `Community 41`, `Community 35`, `Community 4`, `Community 58`, `Community 97`, `Community 105`, `Community 2`, `Community 30`, `Community 108`, `Community 103`, `Community 0`, `Community 314`, `Community 33`, `Community 32`, `Community 23`, `Community 55`, `Community 199`, `Community 24`, `Community 128`, `Community 7`, `Community 12`, `Community 16`, `Community 78`, `Community 113`, `Community 200`, `Community 81`, `Community 90`, `Community 34`, `Community 17`, `Community 272`, `Community 82`, `Community 22`, `Community 89`, `Community 69`, `Community 73`?**
  _High betweenness centrality (0.130) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 8` to `Community 7`, `Community 55`, `Community 10`, `Community 4`, `Community 101`, `Community 14`, `Community 30`, `Community 97`, `Community 58`, `Community 0`, `Community 3`, `Community 12`, `Community 11`, `Community 61`, `Community 24`, `Community 93`, `Community 98`, `Community 2`, `Community 276`, `Community 81`, `Community 5`, `Community 22`, `Community 89`, `Community 21`, `Community 255`, `Community 31`, `Community 16`, `Community 17`, `Community 56`, `Community 39`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 0` to `Community 4`, `Community 2`, `Community 10`, `Community 14`, `Community 8`, `Community 30`, `Community 35`, `Community 97`, `Community 58`, `Community 108`, `Community 33`, `Community 12`, `Community 72`, `Community 104`, `Community 142`, `Community 39`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Are the 956 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Reservation` has 956 INFERRED edges - model-reasoned connections that need verification._
- **Are the 957 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`ReservationStatusEnum` has 957 INFERRED edges - model-reasoned connections that need verification._
- **Are the 933 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 933 INFERRED edges - model-reasoned connections that need verification._
- **Are the 779 inferred relationships involving `Room` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Room` has 779 INFERRED edges - model-reasoned connections that need verification._