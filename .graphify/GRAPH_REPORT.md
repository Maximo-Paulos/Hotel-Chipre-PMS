...
## Corpus Check
- Large corpus: 800 files · ~539,698 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 7697 nodes · 27236 edges · 313 communities detected
- Extraction: 60% EXTRACTED · 40% INFERRED · 0% AMBIGUOUS · INFERRED: 10877 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 10877 · contains: 4631 · calls: 3491 · MODIFIES: 2128 · ON_BRANCH: 1735 · rationale_for: 1326 · imports: 787 · imports_from: 617 · method: 593 · inherits: 535 · PARENT_OF: 516


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 800 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 9 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `3a2821c`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `Reservation` - 690 edges
2. `ReservationStatusEnum` - 689 edges
3. `HotelConfiguration` - 633 edges
4. `Room` - 540 edges
5. `Guest` - 515 edges
6. `RoomCategory` - 499 edges
7. `Base` - 413 edges
8. `RoomStatusEnum` - 336 edges
...
Cohesion: 0.02
Nodes (274): claude/feature/pms-roles-housekeeping-qa, codex/feature/pms-e2e-scale-hardening, 00bf891 Explain analytics errors and add retry action, 0255d4f Normalize Graphify artifact paths, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 0391f8f Fix flaky detached-node false positive in mobile touch-target check, 03ec9de Refresh Graphify after Fase 9 role QA fixes (+266 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (172): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, acceptInvitation(), AuthResponse, AuthUser, _build_auth_response(), _generate_code(), getInvitationInfo(), _issue_email_token() (+164 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (197): CheckInRequest, FastAPI routes for Check-in / Check-out., Guest, GuestTag, GuestTagTypeEnum, Behavioral / alert tags on a guest (v72 §2.6).     Multiple tags per guest; each, Behavioral / alert tags on a guest (v72 §2.6).     Multiple tags per guest; each, Primary guest record. Contains all fields required for check-in     and legal co (+189 more)
...
Cohesion: 0.02
Nodes (142): AllocationRunPayload, AllocationRunResponse, listRoomMovementGroups(), revertRoomMovementGroup(), RoomMoveEvent, RoomMovementGroup, triggerAllocationRecalculation(), approvePaymentProof() (+134 more)
...
Cohesion: 0.04
Nodes (112): _derive_payment_status(), public_reservation_status(), public_reservation_status_by_code(), Public booking-engine API authenticated only with hotel API keys., Coarse payment status derived from amounts (no sensitive detail)., Coarse payment status derived from amounts (no sensitive detail)., Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by confirmation code (v72 §16).      Scoped (+104 more)

### Community 8 - "Community 8"
Cohesion: 0.04
Nodes (117): delete_room(), housekeeping_summary(), FastAPI routes for Room management + Housekeeping., Resolve today's effective rate from the single source of truth and attach it, Simple availability helper. Returns a placeholder message if required     parame, Simple availability helper. Returns a placeholder message if required     parame, Generic room update (number, floor, notes, status, etc.)., Generic room update (number, floor, notes, status, etc.). (+109 more)

### Community 9 - "Community 9"
Cohesion: 0.12
Nodes (116): cancel_reservation(), create_reservation_charge(), extend_stay(), mark_no_show(), modify_reservation(), FastAPI routes for Reservations. Complete CRUD + cancel, modify, no-show, extend, §5.2 — Run allocation engine in background after reservation changes.     Create, §5.2 — Run allocation engine in background after reservation changes.     Create (+108 more)
...
Cohesion: 0.03
Nodes (102): Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, Base, Base class for all ORM models., DeclarativeBase, AuditLog (+94 more)

### Community 12 - "Community 12"
Cohesion: 0.02
Nodes (37): dc4b68a ojooooo, e34785c prueba, build_default_ota_orchestrator(), get_default_adapter(), Default OTA adapter registry.  This keeps provider construction in one place so, bubbleStyles, modeLabel, roleLabel (+29 more)
...
Cohesion: 0.10
Nodes (79): ReservationAllocationLock, Company, Groups multiple RoomMoveEvents triggered by the same cause (v72 §5.4).     Suppo, RoomMoveEvent, RoomMovementGroup, RoomMoveTypeEnum, ReservationNoShowPolicyAppliedEnum, ReservationOutcomeEnum (+71 more)
...
Cohesion: 0.06
Nodes (52): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+44 more)
...
Cohesion: 0.11
Nodes (56): AnalyticsAIUsageMonthly, AnalyticsAlertSetting, AnalyticsAlertSnooze, AnalyticsCurrencyDisplayEnum, AnalyticsExportFormatEnum, AnalyticsExportJob, AnalyticsExportStatusEnum, FactReservationDaily (+48 more)
...
Cohesion: 0.09
Nodes (41): RoomBlockCreate, RoomBlockRead, add_companions(), FastAPI routes for Guest management., Add new companions to an existing guest., Add new companions to an existing guest., RoomBlockCreate, RoomBlockRead (+33 more)
...
Cohesion: 0.33
Nodes (6): _apply_corporate_pricing(), _calendar_pricing_source(), _compute_deposit_amount(), _daily_rate_pricing_result(), _get_hotel_default_currency(), _pricing_with_total()
...
- **Thin community `Community 223`** (2 nodes): `mercadopago_payment_link_webhook()`, `_mercadopago_webhook_impl()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `SimpleRateLimiter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 243`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 265`** (2 nodes): `housekeeping`, `owner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 266`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 268`** (1 nodes): `HotelIdentityPayload`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 312`** (1 nodes): `Add private bank-transfer proof workflow.  Revision ID: 20260724_payment_proofs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 313`** (1 nodes): `Add (hotel_id, created_at) index on reservations for A2 recent-order paging.  Re`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 314`** (1 nodes): `Add transactions.created_by_user_id for payment audit trail.  Transaction had cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `Base` connect `Community 11` to `Community 241`, `Community 15`, `Community 1`, `Community 224`, `Community 209`, `Community 107`, `Community 242`, `Community 9`, `Community 28`, `Community 111`, `Community 36`, `Community 14`, `Community 20`, `Community 17`, `Community 16`, `Community 100`, `Community 10`, `Community 136`, `Community 7`, `Community 2`, `Community 8`, `Community 32`, `Community 6`, `Community 87`, `Community 4`, `Community 35`, `Community 31`, `Community 112`, `Community 3`, `Community 56`, `Community 33`, `Community 171`, `Community 58`, `Community 18`, `Community 59`, `Community 245`, `Community 63`, `Community 216`, `Community 79`, `Community 198`, `Community 199`, `Community 118`?**
  _High betweenness centrality (0.130) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 2` to `Community 1`, `Community 35`, `Community 9`, `Community 30`, `Community 6`, `Community 7`, `Community 99`, `Community 4`, `Community 11`, `Community 28`, `Community 32`, `Community 322`, `Community 58`, `Community 20`, `Community 18`, `Community 249`, `Community 81`, `Community 56`, `Community 47`, `Community 31`, `Community 3`, `Community 10`, `Community 14`, `Community 59`, `Community 194`, `Community 8`, `Community 85`, `Community 79`, `Community 176`, `Community 33`, `Community 16`, `Community 17`, `Community 43`, `Community 133`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 7` to `Community 18`, `Community 32`, `Community 9`, `Community 8`, `Community 1`, `Community 11`, `Community 28`, `Community 14`, `Community 150`, `Community 36`, `Community 58`, `Community 20`, `Community 2`, `Community 16`, `Community 56`, `Community 47`, `Community 67`, `Community 3`, `Community 97`, `Community 85`, `Community 176`, `Community 17`, `Community 43`, `Community 133`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Are the 683 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Reservation` has 683 INFERRED edges - model-reasoned connections that need verification._
- **Are the 687 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`ReservationStatusEnum` has 687 INFERRED edges - model-reasoned connections that need verification._
- **Are the 626 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 626 INFERRED edges - model-reasoned connections that need verification._
- **Are the 535 inferred relationships involving `Room` (e.g. with `RoomBlockCreate` and `RoomBlockRead`) actually correct?**
  _`Room` has 535 INFERRED edges - model-reasoned connections that need verification._