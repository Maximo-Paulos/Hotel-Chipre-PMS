...
## Corpus Check
- Large corpus: 835 files · ~578,445 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 8368 nodes · 31424 edges · 301 communities detected
- Extraction: 59% EXTRACTED · 41% INFERRED · 0% AMBIGUOUS · INFERRED: 12790 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 12790 · contains: 5000 · calls: 3661 · ON_BRANCH: 2795 · MODIFIES: 2355 · rationale_for: 1541 · imports: 878 · imports_from: 673 · method: 595 · PARENT_OF: 573 · inherits: 560 · re_exports: 3
...
## Graph Freshness
- Built from Git commit: `6b53804`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `ReservationStatusEnum` - 794 edges
2. `Reservation` - 793 edges
3. `HotelConfiguration` - 743 edges
4. `Room` - 634 edges
5. `Guest` - 618 edges
6. `RoomCategory` - 569 edges
7. `Base` - 451 edges
...
Cohesion: 0.02
Nodes (267): Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, add_companions(), FastAPI routes for Guest management., Add new companions to an existing guest., Add new companions to an existing guest. (+259 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (86): _require_demo_mode(), reset_demo(), seed_demo(), Database engine and session management. Supports both PostgreSQL (production) an, Reapply RLS tenant context lost when a commit ends the transaction.      ``app.s, _reapply_tenant_context(), _DecimalAwareEncoder, Hotel PMS — FastAPI Main Application. Serves the API + bundled frontend files. (+78 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (190): _derive_payment_status(), public_reservation_status(), public_reservation_status_by_code(), Public booking-engine API authenticated only with hotel API keys., Coarse payment status derived from amounts (no sensitive detail)., Coarse payment status derived from amounts (no sensitive detail)., Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by confirmation code (v72 §16).      Scoped (+182 more)

### Community 3 - "Community 3"
Cohesion: 0.05
Nodes (199): deactivate_payment_surcharge(), _get_surcharge_or_404(), PaymentSurchargeCreate, PaymentSurchargeRead, PaymentSurchargeUpdate, Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, update_payment_surcharge(), BillingAdjustment (+191 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (178): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+170 more)

### Community 5 - "Community 5"
Cohesion: 0.06
Nodes (187): chore/weekly-orchestrator-2026-07-03, codex/feature/agent-ops-knowledge, codex/feature/trusted-qa-bootstrap, codex/fix-render-postgres-enum-values, main, 01498d0 Harden password reset flow (verified-only, dev code return), 093f7d3 ui(manager): ocultar config y redirigir al dashboard al cambiar vista, 0a4396e Add multi-hotel isolation tests and bcrypt shim (+179 more)

### Community 6 - "Community 6"
Cohesion: 0.06
Nodes (184): worktree-agent-a86cd1e171e139840, worktree-agent-a945788514ece683f, worktree-agent-aa90c760fab5d7651, worktree-agent-aac480096c25247c2, worktree-agent-accdf065d7f106429, 00af83e chore(graphify): regenerate graph after B4 manual OTA + manual tarifa + B4.2, 0255d4f Normalize Graphify artifact paths, 043b24f Record the Fase 10 WebKit Apple matrix findings in the catalog (+176 more)

### Community 7 - "Community 7"
Cohesion: 0.03
Nodes (117): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, acceptInvitation(), AuthResponse, AuthUser, _build_auth_response(), _generate_code(), getInvitationInfo(), _issue_email_token() (+109 more)
...
Cohesion: 0.04
Nodes (139): claude/feature/pms-roles-housekeeping-qa, codex/feature/pms-e2e-scale-hardening, 00bf891 Explain analytics errors and add retry action, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 0391f8f Fix flaky detached-node false positive in mobile touch-target check, 03f4d70 Document cloud date and quote regressions, 0449677 Document cloud role and preview blocker (+131 more)

### Community 10 - "Community 10"
Cohesion: 0.02
Nodes (55): _ensure_wide_version_table(), get_url(), Alembic creates alembic_version with version_num VARCHAR(32) by default.     Som, run_migrations_offline(), run_migrations_online(), 52604e5 feat(db): PostgreSQL real validation green 8/8 — payment models, migration fixes, 7e2edb6 Merge Analytics R1.0, df173d1 Implement Analytics R1.0 dashboard, chat IA, and support (+47 more)
...
Cohesion: 0.03
Nodes (101): StockConsumptionItem, StockConsumptionReportRead, StockItemCreate, StockItemRead, StockItemUpdate, StockLocationCreate, StockLocationRead, StockMovementCreate (+93 more)
...
Cohesion: 0.03
Nodes (73): validateGuestForCheckin(), add_reservation_guests(), addReservationCharge(), addReservationGuests(), buildQueryString(), cancelReservation(), change_dates(), CheckInPayload (+65 more)
...
Cohesion: 0.03
Nodes (22): 099e4c8 fix(e2e): stop the WebKit business Apple matrix from colliding with itself, 0a457cb feat(checkin): B3 partial check-in + mandatory guest profile capture + companions, 10e9d42 fix(security): gate manual reservation total_amount behind reservation:manual_rate, 194af4d fix(reservations): search guests by name/DNI instead of raw ID field, 6b53804 Merge price-quote error fix into pms-e2e-scale-hardening, 8307525 chore(graphify): refresh code graph after guest search panel changes, credentials, navigateFromShell() (+14 more)
...
Cohesion: 0.04
Nodes (57): AllocationRunPayload, AllocationRunResponse, listRoomMovementGroups(), revertRoomMovementGroup(), RoomMoveEvent, RoomMovementGroup, triggerAllocationRecalculation(), Category (+49 more)
...
Cohesion: 0.05
Nodes (32): 03ec9de Refresh Graphify after Fase 9 role QA fixes, 1cd0fce Merge claude/feature/pms-roles-housekeeping-qa: integrate parallel QA fixes, 2efac9f Record Fase 9 role QA findings in the regression catalog, 32f7bba Require checkin:perform permission for checkout, not just authentication, 906d009 Hide room status and block controls from roles that cannot use them, ac2d59b Hide stock adjustment option from manager, bf9b633 Gate the post-close "Aprobar diferencia" button by role too, c2f87d1 Grant receptionist read access to room categories (+24 more)
...
Cohesion: 0.07
Nodes (34): getPaymentSummary(), makePayment(), newPaymentIdempotencyKey(), PaymentMethod, PaymentRequest, PaymentSummary, FastAPI routes for Payments., TransactionType (+26 more)
...
Cohesion: 0.06
Nodes (31): graphify_command_error(), inline_list(), main(), Parse the simple unquoted frontmatter lists used by context packs., Parse the simple unquoted frontmatter lists used by context packs., Parse the simple unquoted frontmatter lists used by context packs., Reject context commands that the installed Graphify CLI cannot route., Reject context commands that the installed Graphify CLI cannot route. (+23 more)

### Community 44 - "Community 44"
Cohesion: 0.07
Nodes (32): FastAPI routes for outsourced laundry vendors, pricing and remitos.  Separate ro, RemitoCreate, RemitoCreateResponse, RemitoLineIn, RemitoLineRead, RemitoRead, VendorBalanceLine, VendorCreate (+24 more)
...
Cohesion: 0.14
Nodes (29): 6e6d179 Prepare release candidate for landing and onboarding, 87b0eb6 Improve marketing SEO content, linking, and metadata, Seo(), SeoProps, StructuredData, resolveAppUrl(), benefitPoints, faqItems (+21 more)

### Community 53 - "Community 53"
Cohesion: 0.13
Nodes (39): AnalyticsCurrencyDisplayEnum, AnalyticsExportFormatEnum, FactReservationRowKindEnum, FactRoomOccupancyStatusAtNightEnum, ReservationCancellationReasonCodeEnum, ReservationGuestSegmentEnum, ReservationGuestSegmentSourceEnum, _allocate_monetary_totals() (+31 more)
...
Cohesion: 0.11
Nodes (25): MasterAdminAuditEvent, MasterAdminAuthLockout, MasterAdminSession, MasterBillingPolicy, MasterStripeSettings, MasterStripeWebhookEvent, MasterSystemEmailConnection, MasterAdminContext (+17 more)
...
Cohesion: 0.10
Nodes (23): 15fdb88 Create missing cash custody handoff schema, b1e1bd0 Repair legacy cash handoff schema at startup, CashHandoffSchemaRepairError, main(), missing_model_tables(), Repair known legacy PostgreSQL schema drift before starting the API.  The manage, Apply the additive cash-handoff repair in one PostgreSQL transaction., Apply the additive cash-handoff repair in one PostgreSQL transaction. (+15 more)
...
Cohesion: 0.32
Nodes (10): _adjacency_bonus_for_room(), AllocationResult, apply_allocation_result(), _check_overlap(), _is_protected_reservation(), _lowest_available_compatible_floor(), _lowest_compatible_floor(), _one_night_gap_penalty_for_room() (+2 more)
...
- **Thin community `Community 67`** (2 nodes): `_hotel_default_currency()`, `OTAIntegrationService`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 123`** (1 nodes): `OnboardingState`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 131`** (1 nodes): `DespegarAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 212`** (2 nodes): `FakeRedis`, `test_availability_key_shape_and_serialization()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 272`** (1 nodes): `Response`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 285`** (2 nodes): `owner`, `receptionist`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 287`** (1 nodes): `FakeRedis`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 293`** (1 nodes): `v72 gaps phase 3: room_movement_groups table, BillingAdjustment/ReservationAdjus`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 296`** (1 nodes): `drop orphan payment_surcharge_configs table (R5b).  main carries TWO surcharge i`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 297`** (1 nodes): `v72 §16.2: add 'company' value to reservation_channel_code_enum.  Adds the corpo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 298`** (1 nodes): `repair: ensure uq_reservation_hotel_id_id exists before payment_links FK  Revisi`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 299`** (1 nodes): `repair: create hotel_memberships table (was never migrated)  Revision ID: 202607`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 300`** (1 nodes): `repair: ensure uq_stock_items_hotel_id_id / uq_stock_locations_hotel_id_id exist`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 301`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 302`** (1 nodes): `owner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 303`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 304`** (1 nodes): `Fast server-side EXPLAIN ANALYZE probe for hot queries on real PostgreSQL.  Rati`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 305`** (2 nodes): `RoomMoveEventRead`, `RoomMovementGroupRead`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 306`** (1 nodes): `Build the canonical quote shared by reservation-facing surfaces.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 312`** (1 nodes): `TestCurrentSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 313`** (1 nodes): `TestExportSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 314`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 0` to `Community 10`, `Community 1`, `Community 201`, `Community 79`, `Community 234`, `Community 8`, `Community 68`, `Community 122`, `Community 72`, `Community 87`, `Community 57`, `Community 53`, `Community 73`, `Community 78`, `Community 22`, `Community 45`, `Community 29`, `Community 20`, `Community 104`, `Community 11`, `Community 149`, `Community 2`, `Community 4`, `Community 35`, `Community 101`, `Community 34`, `Community 69`, `Community 123`, `Community 17`, `Community 3`, `Community 74`, `Community 40`, `Community 28`, `Community 185`, `Community 251`, `Community 71`, `Community 93`, `Community 210`, `Community 211`, `Community 107`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 0` to `Community 13`, `Community 34`, `Community 8`, `Community 7`, `Community 46`, `Community 12`, `Community 2`, `Community 182`, `Community 10`, `Community 45`, `Community 28`, `Community 304`, `Community 53`, `Community 57`, `Community 4`, `Community 256`, `Community 44`, `Community 95`, `Community 74`, `Community 67`, `Community 98`, `Community 3`, `Community 11`, `Community 29`, `Community 68`, `Community 73`, `Community 208`, `Community 99`, `Community 78`, `Community 93`, `Community 69`, `Community 40`, `Community 20`, `Community 181`, `Community 22`, `Community 49`, `Community 312`, `Community 313`, `Community 116`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 2` to `Community 4`, `Community 0`, `Community 28`, `Community 8`, `Community 10`, `Community 45`, `Community 72`, `Community 162`, `Community 87`, `Community 53`, `Community 57`, `Community 20`, `Community 74`, `Community 67`, `Community 58`, `Community 3`, `Community 78`, `Community 29`, `Community 119`, `Community 99`, `Community 22`, `Community 49`, `Community 312`, `Community 313`, `Community 116`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Are the 792 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`ReservationStatusEnum` has 792 INFERRED edges - model-reasoned connections that need verification._
- **Are the 786 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Reservation` has 786 INFERRED edges - model-reasoned connections that need verification._
- **Are the 736 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 736 INFERRED edges - model-reasoned connections that need verification._
- **Are the 629 inferred relationships involving `Room` (e.g. with `RoomBlockCreate` and `RoomBlockRead`) actually correct?**
  _`Room` has 629 INFERRED edges - model-reasoned connections that need verification._