...
## Corpus Check
- Large corpus: 811 files · ~548,789 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 7920 nodes · 28406 edges · 322 communities detected
- Extraction: 59% EXTRACTED · 41% INFERRED · 0% AMBIGUOUS · INFERRED: 11708 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 11708 · contains: 4762 · calls: 3532 · MODIFIES: 2152 · ON_BRANCH: 1746 · rationale_for: 1398 · imports: 814 · imports_from: 635 · method: 593 · inherits: 539 · PARENT_OF: 527


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 811 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 9 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `bd4af0d`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `ReservationStatusEnum` - 743 edges
2. `Reservation` - 742 edges
3. `HotelConfiguration` - 682 edges
4. `Room` - 585 edges
5. `Guest` - 566 edges
6. `RoomCategory` - 535 edges
7. `Base` - 427 edges
8. `RoomStatusEnum` - 372 edges
9. `TransactionTypeEnum` - 327 edges
10. `PaymentMethodEnum` - 318 edges
...
Cohesion: 0.03
Nodes (281): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+273 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (278): claude/feature/pms-roles-housekeeping-qa, codex/feature/pms-e2e-scale-hardening, 00bf891 Explain analytics errors and add retry action, 0255d4f Normalize Graphify artifact paths, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 0391f8f Fix flaky detached-node false positive in mobile touch-target check, 03ec9de Refresh Graphify after Fase 9 role QA fixes (+270 more)
...
Cohesion: 0.05
Nodes (213): PaymentSurchargeCreate, PaymentSurchargeRead, PaymentSurchargeUpdate, Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, 62f0868 fix(payments): record WHO triggered a payment state change, not just when, BillingAdjustment, BillingAdjustmentTypeEnum, Payment (+205 more)

### Community 4 - "Community 4"
Cohesion: 0.06
Nodes (187): chore/weekly-orchestrator-2026-07-03, codex/feature/agent-ops-knowledge, codex/feature/trusted-qa-bootstrap, codex/fix-render-postgres-enum-values, main, 01498d0 Harden password reset flow (verified-only, dev code return), 0737a5a feat(frontend): V72 Wave A/B/C — companies, cash, reports, waitlist, laundry, stock, api-keys, permissions, whatsapp, 093f7d3 ui(manager): ocultar config y redirigir al dashboard al cambiar vista (+179 more)

### Community 5 - "Community 5"
Cohesion: 0.03
Nodes (114): _derive_payment_status(), public_reservation_status(), public_reservation_status_by_code(), Coarse payment status derived from amounts (no sensitive detail)., Coarse payment status derived from amounts (no sensitive detail)., Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by id (v72 §16).      Scoped to the API key (+106 more)
...
Cohesion: 0.02
Nodes (112): BaseModel, BillingPolicyPayload, BillingPolicyUpdateRequest, EmailTestRequest, MasterAdminLoginRequest, MasterAdminLoginResponse, MasterAdminSessionResponse, MasterAdminUserPayload (+104 more)
...
Cohesion: 0.03
Nodes (106): RoomBlockCreate, RoomBlockRead, Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, add_companions(), FastAPI routes for Guest management. (+98 more)

### Community 10 - "Community 10"
Cohesion: 0.03
Nodes (95): AcceptPayload, FastAPI routes for Reports & Night Audit. Daily summaries, occupancy reports, re, Occupancy report for a date range (default: last 30 days)., Revenue report for a date range., Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c, InvitePayload, InviteResponse, User management per hotel (owners/co-owners). (+87 more)

### Community 11 - "Community 11"
Cohesion: 0.02
Nodes (34): dc4b68a ojooooo, e34785c prueba, build_default_ota_orchestrator(), get_default_adapter(), Default OTA adapter registry.  This keeps provider construction in one place so, bubbleStyles, modeLabel, roleLabel (+26 more)

### Community 12 - "Community 12"
Cohesion: 0.07
Nodes (66): FastAPI Webhook endpoints for OTA integrations., Receive reservation notifications from Booking.com., Receive reservation notifications from Expedia., Receive reservation notifications from Despegar., ReservationAdjustment, ReservationAdjustmentKindEnum, ReservationAdjustmentStatusEnum, OTACommissionRule (+58 more)

### Community 13 - "Community 13"
Cohesion: 0.02
Nodes (44): _require_demo_mode(), reset_demo(), seed_demo(), booking_webhook(), despegar_webhook(), expedia_webhook(), _handle_ota_webhook(), deactivate_payment_surcharge() (+36 more)
...
Cohesion: 0.08
Nodes (62): GuestCompanion, Companion/additional occupant traveling with a primary guest.     Hotels often n, Companion/additional occupant traveling with a primary guest.     Hotels often n, Companion/additional occupant traveling with a primary guest.     Hotels often n, RateLimitEvent, db(), db_engine(), hotel_config() (+54 more)

### Community 21 - "Community 21"
Cohesion: 0.05
Nodes (55): validateGuestForCheckin(), add_reservation_guests(), addReservationGuests(), buildQueryString(), cancelReservation(), change_dates(), CheckInPayload, checkInReservation() (+47 more)
...
Cohesion: 0.24
Nodes (56): Company, Operational reservation models for adjustments, room moves and audit history., ReservationStatusHistory, RoomMoveEvent, RoomMoveTypeEnum, ReservationChannelCodeEnum, ReservationNoShowPolicyAppliedEnum, ReservationOutcomeEnum (+48 more)

### Community 30 - "Community 30"
Cohesion: 0.05
Nodes (37): addGuestCompanions(), addGuestTag(), _build_guest_ledger_csv(), checkInGuestReservation(), createGuest(), _enum_value(), export_guest_ledger(), getGuest() (+29 more)

### Community 31 - "Community 31"
Cohesion: 0.05
Nodes (36): create_movement(), createStockItem(), createStockLocation(), createStockMovement(), CurrentStock, DecimalValue, _ensure_adjustment_permission(), getCurrentStock() (+28 more)
...
Cohesion: 0.06
Nodes (37): getPaymentSummary(), makePayment(), newPaymentIdempotencyKey(), PaymentMethod, PaymentRequest, PaymentSummary, FastAPI routes for Payments., TransactionType (+29 more)
...
Cohesion: 0.06
Nodes (30): graphify_state(), main(), main(), normalize_instruction_paths(), Replace this repository's absolute root in generated instructions only., Replace this repository's absolute root in generated instructions only., main(), run() (+22 more)
...
Cohesion: 0.12
Nodes (18): _authorize_override(), checkin(), checkin_partial(), a2c31f7 feat(ops): SLICE-04 room blocks + availability/allocation exclusion, c486aa3 feat(v72): Wave 2 — companies/documents, cash register/arqueo, guest override, c514405 fix(perf): seed permission matrix once per process, not per request, e74081c feat(reservations): add minimal search filter for global reservation lookup, _apply_corporate_pricing() (+10 more)
...
Cohesion: 0.23
Nodes (15): _apply_custom_deposit_amount(), _apply_pricing_result_to_reservation(), calculate_reservation_pricing(), check_room_availability(), create_reservation(), find_available_rooms(), _invalidate_availability_cache(), normalize_pricing_payment_method() (+7 more)
...
- **Thin community `Community 245`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 268`** (2 nodes): `housekeeping`, `owner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 269`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 270`** (2 nodes): `credentials`, `PAGES`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 288`** (1 nodes): `FakeRedis`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 297`** (1 nodes): `guest checkin profile fields  Revision ID: 17f1689785f3 Revises: 20260725_res_ho`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 319`** (1 nodes): `Add private bank-transfer proof workflow.  Revision ID: 20260724_payment_proofs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 320`** (1 nodes): `Add (hotel_id, created_at) index on reservations for A2 recent-order paging.  Re`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 321`** (1 nodes): `Add transactions.created_by_user_id for payment audit trail.  Transaction had cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 322`** (1 nodes): `baseline  Revision ID: cb9001557529 Revises:  Create Date: 2026-03-31 19:04:53.7`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 323`** (2 nodes): `get_mongo_db()`, `mongo_healthcheck()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 324`** (2 nodes): `get_neo4j_driver()`, `neo4j_healthcheck()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 325`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 327`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 328`** (1 nodes): `Fast server-side EXPLAIN ANALYZE probe for hot queries on real PostgreSQL.  Rati`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 336`** (1 nodes): `TestCurrentSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 337`** (1 nodes): `TestExportSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 338`** (1 nodes): `Defensive datastore clients for optional infrastructure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 339`** (1 nodes): `Application decorators.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 340`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 9` to `Community 242`, `Community 13`, `Community 2`, `Community 224`, `Community 208`, `Community 107`, `Community 244`, `Community 6`, `Community 41`, `Community 112`, `Community 60`, `Community 49`, `Community 22`, `Community 10`, `Community 18`, `Community 29`, `Community 14`, `Community 96`, `Community 8`, `Community 136`, `Community 0`, `Community 20`, `Community 28`, `Community 4`, `Community 84`, `Community 113`, `Community 3`, `Community 12`, `Community 15`, `Community 168`, `Community 62`, `Community 5`, `Community 63`, `Community 247`, `Community 66`, `Community 216`, `Community 83`, `Community 194`, `Community 195`, `Community 120`?**
  _High betweenness centrality (0.128) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 10` to `Community 2`, `Community 6`, `Community 5`, `Community 97`, `Community 4`, `Community 9`, `Community 41`, `Community 328`, `Community 62`, `Community 22`, `Community 0`, `Community 251`, `Community 86`, `Community 12`, `Community 91`, `Community 3`, `Community 8`, `Community 29`, `Community 63`, `Community 20`, `Community 92`, `Community 83`, `Community 84`, `Community 174`, `Community 15`, `Community 14`, `Community 18`, `Community 46`, `Community 336`, `Community 337`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 0` to `Community 9`, `Community 5`, `Community 10`, `Community 6`, `Community 2`, `Community 41`, `Community 49`, `Community 60`, `Community 62`, `Community 22`, `Community 14`, `Community 271`, `Community 12`, `Community 70`, `Community 3`, `Community 29`, `Community 20`, `Community 92`, `Community 125`, `Community 174`, `Community 18`, `Community 46`, `Community 336`, `Community 337`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Are the 741 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`ReservationStatusEnum` has 741 INFERRED edges - model-reasoned connections that need verification._
- **Are the 735 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Reservation` has 735 INFERRED edges - model-reasoned connections that need verification._
- **Are the 675 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 675 INFERRED edges - model-reasoned connections that need verification._
- **Are the 580 inferred relationships involving `Room` (e.g. with `RoomBlockCreate` and `RoomBlockRead`) actually correct?**
  _`Room` has 580 INFERRED edges - model-reasoned connections that need verification._