...
## Corpus Check
- Large corpus: 798 files · ~538,671 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 7665 nodes · 26954 edges · 321 communities detected
- Extraction: 61% EXTRACTED · 39% INFERRED · 0% AMBIGUOUS · INFERRED: 10646 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 10646 · contains: 4621 · calls: 3487 · MODIFIES: 2116 · ON_BRANCH: 1730 · rationale_for: 1311 · imports: 787 · imports_from: 617 · method: 593 · inherits: 535 · PARENT_OF: 511


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 798 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 9 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `04cd07c`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `Reservation` - 681 edges
2. `ReservationStatusEnum` - 680 edges
3. `HotelConfiguration` - 624 edges
4. `Room` - 531 edges
5. `Guest` - 506 edges
6. `RoomCategory` - 490 edges
7. `Base` - 410 edges
8. `RoomStatusEnum` - 335 edges
...
Cohesion: 0.02
Nodes (270): claude/feature/pms-roles-housekeeping-qa, codex/feature/pms-e2e-scale-hardening, 00bf891 Explain analytics errors and add retry action, 0255d4f Normalize Graphify artifact paths, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 0391f8f Fix flaky detached-node false positive in mobile touch-target check, 03ec9de Refresh Graphify after Fase 9 role QA fixes (+262 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (212): Application Configuration. Uses pydantic-settings for environment variable manag, Hotel PMS — FastAPI Main Application. Serves the API + bundled frontend files., chore/weekly-orchestrator-2026-07-03, codex/feature/agent-ops-knowledge, codex/feature/trusted-qa-bootstrap, codex/fix-render-postgres-enum-values, main, 01498d0 Harden password reset flow (verified-only, dev code return) (+204 more)

### Community 2 - "Community 2"
Cohesion: 0.05
Nodes (209): deactivate_payment_surcharge(), _get_surcharge_or_404(), PaymentSurchargeCreate, PaymentSurchargeRead, PaymentSurchargeUpdate, Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, update_payment_surcharge(), HotelConfiguration (+201 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (157): _derive_payment_status(), public_reservation_status(), public_reservation_status_by_code(), Public booking-engine API authenticated only with hotel API keys., Coarse payment status derived from amounts (no sensitive detail)., Coarse payment status derived from amounts (no sensitive detail)., Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by confirmation code (v72 §16).      Scoped (+149 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (148): delete_room(), housekeeping_summary(), FastAPI routes for Room management + Housekeeping., Resolve today's effective rate from the single source of truth and attach it, Simple availability helper. Returns a placeholder message if required     parame, Simple availability helper. Returns a placeholder message if required     parame, Generic room update (number, floor, notes, status, etc.)., Generic room update (number, floor, notes, status, etc.). (+140 more)
...
Cohesion: 0.04
Nodes (129): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+121 more)

### Community 7 - "Community 7"
Cohesion: 0.03
Nodes (134): UpdateRolePayload, BaseModel, BillingPolicyPayload, BillingPolicyUpdateRequest, EmailTestRequest, MasterAdminLoginRequest, MasterAdminLoginResponse, MasterAdminSessionResponse (+126 more)
...
Cohesion: 0.11
Nodes (109): cancel_reservation(), create_reservation_charge(), extend_stay(), mark_no_show(), modify_reservation(), FastAPI routes for Reservations. Complete CRUD + cancel, modify, no-show, extend, §5.2 — Run allocation engine in background after reservation changes.     Create, §5.2 — Run allocation engine in background after reservation changes.     Create (+101 more)

### Community 10 - "Community 10"
Cohesion: 0.02
Nodes (36): dc4b68a ojooooo, e34785c prueba, build_default_ota_orchestrator(), get_default_adapter(), Default OTA adapter registry.  This keeps provider construction in one place so, bubbleStyles, modeLabel, roleLabel (+28 more)

### Community 11 - "Community 11"
Cohesion: 0.03
Nodes (97): Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, Base, Base class for all ORM models., DeclarativeBase, AuditLog (+89 more)

### Community 12 - "Community 12"
Cohesion: 0.03
Nodes (40): CategoryPayload, DepositPolicyPayload, finishOnboarding(), getOnboardingStatus(), HotelIdentityPayload, OnboardingProviderSetup, OnboardingStatus, OTAChannelsPayload (+32 more)

### Community 13 - "Community 13"
Cohesion: 0.09
Nodes (88): ReservationAllocationLock, Company, Groups multiple RoomMoveEvents triggered by the same cause (v72 §5.4).     Suppo, RoomMoveEvent, RoomMovementGroup, RoomMoveTypeEnum, ReservationNoShowPolicyAppliedEnum, ReservationOutcomeEnum (+80 more)

### Community 14 - "Community 14"
Cohesion: 0.04
Nodes (69): FastAPI routes for Reports & Night Audit. Daily summaries, occupancy reports, re, Occupancy report for a date range (default: last 30 days)., Revenue report for a date range., Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c, Core reservation entity.     Tracks the full lifecycle from booking to checkout,, Reservation, Date-ranged block on a room. NULL ends_at means indefinite.     resolved_at popu, RoomBlock (+61 more)

### Community 15 - "Community 15"
Cohesion: 0.03
Nodes (34): FastAPI routes for provider connections. Exposes /api/connections/{provider}/con, _require_demo_mode(), reset_demo(), seed_demo(), _backfill_not_null_nulls(), _column_fill_value(), get_db(), get_engine() (+26 more)
...
Cohesion: 0.11
Nodes (52): AnalyticsAIUsageMonthly, AnalyticsAlertSetting, AnalyticsAlertSnooze, AnalyticsCurrencyDisplayEnum, AnalyticsExportFormatEnum, AnalyticsExportJob, AnalyticsExportStatusEnum, FactReservationDaily (+44 more)
...
Cohesion: 0.06
Nodes (46): add_reservation_guests(), addReservationCharge(), buildQueryString(), cancelReservation(), change_dates(), checkInReservation(), checkOutReservation(), clearReservationManualReview() (+38 more)
...
Cohesion: 0.06
Nodes (46): Base, MasterAdminAuditEvent, MasterAdminAuthLockout, MasterAdminSession, MasterBillingPolicy, MasterStripeSettings, MasterStripeWebhookEvent, MasterSystemEmailConnection (+38 more)
...
Cohesion: 0.10
Nodes (10): booking_webhook(), despegar_webhook(), expedia_webhook(), _handle_ota_webhook(), FastAPI Webhook endpoints for OTA integrations., Receive reservation notifications from Booking.com., Receive reservation notifications from Expedia., Receive reservation notifications from Despegar. (+2 more)
...
Cohesion: 0.07
Nodes (28): AllocationRunPayload, AllocationRunResponse, listRoomMovementGroups(), revertRoomMovementGroup(), RoomMoveEvent, RoomMovementGroup, triggerAllocationRecalculation(), moveReservationRoom() (+20 more)
...
Cohesion: 0.14
Nodes (34): FactReservationRowKindEnum, FactRoomOccupancyStatusAtNightEnum, ReservationCancellationReasonCodeEnum, ReservationGuestSegmentSourceEnum, _allocate_monetary_totals(), backfill_channel_code(), build_analytics_window(), build_comparison_state() (+26 more)
...
Cohesion: 0.31
Nodes (29): OTAConnection, OTAProvider, OTARatePlanMapping, OTAReservationLifecycleEnum, OTAReservationLink, OTARoomMapping, OTASyncStatusEnum, OTAWebhookCredential (+21 more)
...
Cohesion: 0.13
Nodes (14): 4959706 Enforce tenant isolation with PostgreSQL RLS context, get_public_api_context(), _public_api_rate_limit_for_hotel(), Public API-key authentication, separate from staff JWT auth., Middleware to block mutating requests when subscription enforcement is enabled., SubscriptionEnforcementMiddleware, Bind a SQLAlchemy transaction to the authenticated tenant.  PostgreSQL RLS polic, Set the authenticated user id used by membership RLS policies. (+6 more)
...
- **Thin community `Community 219`** (2 nodes): `mercadopago_payment_link_webhook()`, `_mercadopago_webhook_impl()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (1 nodes): `SimpleRateLimiter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 261`** (2 nodes): `housekeeping`, `owner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 263`** (1 nodes): `HotelIdentityPayload`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 309`** (1 nodes): `Add private bank-transfer proof workflow.  Revision ID: 20260724_payment_proofs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 310`** (1 nodes): `Add transactions.created_by_user_id for payment audit trail.  Transaction had cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 315`** (2 nodes): `get_neo4j_driver()`, `neo4j_healthcheck()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 316`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 320`** (1 nodes): `Fast server-side EXPLAIN ANALYZE probe for hot queries on real PostgreSQL.  Rati`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 330`** (1 nodes): `TestCurrentSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 331`** (1 nodes): `TestExportSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 335`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 336`** (2 nodes): `Derive the BRM §16.2 reportable origin for a reservation.      Precedence:`, `reportable_origin()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 339`** (2 nodes): `Preview QA already requires a strong MASTER_ADMIN_PASSWORD/EMAIL, but     produc`, `test_validate_runtime_security_rejects_weak_master_admin_password_in_production()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 340`** (2 nodes): `MERCADOPAGO_WEBHOOK_SECRET is required only when MP_ACCESS_TOKEN is set.`, `test_validate_runtime_security_rejects_missing_mp_webhook_when_mp_configured()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 11` to `Community 238`, `Community 15`, `Community 332`, `Community 1`, `Community 220`, `Community 205`, `Community 110`, `Community 239`, `Community 9`, `Community 29`, `Community 113`, `Community 36`, `Community 13`, `Community 21`, `Community 58`, `Community 18`, `Community 17`, `Community 103`, `Community 8`, `Community 138`, `Community 3`, `Community 4`, `Community 6`, `Community 7`, `Community 92`, `Community 2`, `Community 35`, `Community 32`, `Community 114`, `Community 61`, `Community 33`, `Community 14`, `Community 171`, `Community 336`, `Community 49`, `Community 63`, `Community 241`, `Community 67`, `Community 212`, `Community 83`, `Community 195`, `Community 196`, `Community 120`?**
  _High betweenness centrality (0.130) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 2` to `Community 5`, `Community 35`, `Community 9`, `Community 31`, `Community 7`, `Community 3`, `Community 101`, `Community 1`, `Community 11`, `Community 29`, `Community 320`, `Community 58`, `Community 21`, `Community 6`, `Community 14`, `Community 245`, `Community 85`, `Community 4`, `Community 61`, `Community 46`, `Community 32`, `Community 8`, `Community 13`, `Community 63`, `Community 191`, `Community 90`, `Community 83`, `Community 33`, `Community 17`, `Community 18`, `Community 42`, `Community 330`, `Community 331`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 14` to `Community 6`, `Community 3`, `Community 9`, `Community 4`, `Community 1`, `Community 11`, `Community 29`, `Community 13`, `Community 152`, `Community 36`, `Community 58`, `Community 21`, `Community 17`, `Community 61`, `Community 46`, `Community 71`, `Community 2`, `Community 49`, `Community 98`, `Community 90`, `Community 18`, `Community 42`, `Community 330`, `Community 331`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Are the 674 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Reservation` has 674 INFERRED edges - model-reasoned connections that need verification._
- **Are the 678 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`ReservationStatusEnum` has 678 INFERRED edges - model-reasoned connections that need verification._
- **Are the 617 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 617 INFERRED edges - model-reasoned connections that need verification._
- **Are the 526 inferred relationships involving `Room` (e.g. with `RoomBlockCreate` and `RoomBlockRead`) actually correct?**
  _`Room` has 526 INFERRED edges - model-reasoned connections that need verification._