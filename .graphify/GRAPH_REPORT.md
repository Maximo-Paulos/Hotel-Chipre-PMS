...
## Corpus Check
- Large corpus: 814 files · ~552,585 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 7976 nodes · 29025 edges · 347 communities detected
- Extraction: 58% EXTRACTED · 42% INFERRED · 0% AMBIGUOUS · INFERRED: 12211 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 12211 · contains: 4782 · calls: 3538 · MODIFIES: 2180 · ON_BRANCH: 1747 · rationale_for: 1430 · imports: 830 · imports_from: 646 · method: 594 · inherits: 539 · PARENT_OF: 528
...
## Graph Freshness
- Built from Git commit: `0a457cb`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `ReservationStatusEnum` - 774 edges
2. `Reservation` - 773 edges
3. `HotelConfiguration` - 698 edges
4. `Room` - 616 edges
5. `Guest` - 597 edges
6. `RoomCategory` - 551 edges
7. `Base` - 427 edges
8. `RoomStatusEnum` - 380 edges
9. `ReservationError` - 340 edges
...
Cohesion: 0.04
Nodes (273): PaymentSurchargeCreate, PaymentSurchargeRead, PaymentSurchargeUpdate, Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, FastAPI routes for Reports & Night Audit. Daily summaries, occupancy reports, re, Occupancy report for a date range (default: last 30 days)., Revenue report for a date range., Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c (+265 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (282): claude/feature/pms-roles-housekeeping-qa, codex/feature/pms-e2e-scale-hardening, 00bf891 Explain analytics errors and add retry action, 0255d4f Normalize Graphify artifact paths, 02c5612 Add tenant realtime sync and warehouse boundary, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 0391f8f Fix flaky detached-node false positive in mobile touch-target check (+274 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (219): Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, Resolve today's effective rate from the single source of truth and attach it, Resolve today's effective rate from the single source of truth and attach it, Simple availability helper. Returns a placeholder message if required     parame, Generic room update (number, floor, notes, status, etc.). (+211 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (216): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, Application Configuration. Uses pydantic-settings for environment variable manag, Hotel PMS — FastAPI Main Application. Serves the API + bundled frontend files., chore/weekly-orchestrator-2026-07-03, codex/feature/agent-ops-knowledge, codex/feature/trusted-qa-bootstrap, codex/fix-render-postgres-enum-values, main (+208 more)

### Community 4 - "Community 4"
Cohesion: 0.05
Nodes (183): apply_period_to_daily_rates(), ApplyPeriodOut, _bulk_field_value(), bulk_update_daily_rate_field(), bulk_upsert_daily_rates(), BulkFieldRateIn, BulkRateIn, BulkRateOut (+175 more)

### Community 5 - "Community 5"
Cohesion: 0.04
Nodes (150): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+142 more)

### Community 6 - "Community 6"
Cohesion: 0.10
Nodes (153): cancel_reservation(), create_reservation_charge(), extend_stay(), mark_no_show(), modify_reservation(), occupancy_grid(), FastAPI routes for Reservations. Complete CRUD + cancel, modify, no-show, extend, §5.2 — Run allocation engine in background after reservation changes.     Create (+145 more)

### Community 7 - "Community 7"
Cohesion: 0.03
Nodes (117): AcceptPayload, User management per hotel (owners/co-owners)., LoginRequest, AuditLog, Append-only record of every significant mutation within a hotel tenant.     Do N, HotelConfiguration, Serialize policies dict to JSON string., Check if a specific payment method is enabled. (+109 more)

### Community 8 - "Community 8"
Cohesion: 0.03
Nodes (92): apiFetch(), buildAuthHeaders(), buildUrl(), formatErrorDetail(), handleUnauthorized(), hasValidSession(), isTokenExpired(), jwtExpMs() (+84 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (109): CheckInRequest, FastAPI routes for Payments., UpdateRolePayload, BaseModel, BillingPolicyPayload, BillingPolicyUpdateRequest, EmailTestRequest, MasterAdminLoginRequest (+101 more)

### Community 10 - "Community 10"
Cohesion: 0.03
Nodes (69): requestVerification(), verifyEmail(), CategoryPayload, DepositPolicyPayload, finishOnboarding(), getOnboardingStatus(), HotelIdentityPayload, OnboardingProviderSetup (+61 more)

### Community 11 - "Community 11"
Cohesion: 0.02
Nodes (32): dc4b68a ojooooo, e34785c prueba, build_default_ota_orchestrator(), get_default_adapter(), Default OTA adapter registry.  This keeps provider construction in one place so, bubbleStyles, modeLabel, roleLabel (+24 more)

### Community 12 - "Community 12"
Cohesion: 0.02
Nodes (38): _require_demo_mode(), reset_demo(), seed_demo(), deactivate_payment_surcharge(), _get_surcharge_or_404(), update_payment_surcharge(), _backfill_not_null_nulls(), _column_fill_value() (+30 more)
...
Cohesion: 0.05
Nodes (57): validateGuestForCheckin(), add_reservation_guests(), addReservationGuests(), buildQueryString(), cancelReservation(), change_dates(), CheckInPayload, checkInReservation() (+49 more)
...
Cohesion: 0.06
Nodes (44): acceptInvitation(), AuthResponse, AuthUser, _build_auth_response(), _generate_code(), getInvitationInfo(), _issue_email_token(), login() (+36 more)
...
Cohesion: 0.17
Nodes (35): OTAConnection, OTAProvider, OTARatePlanMapping, OTAReservationLifecycleEnum, OTAReservationLink, OTARoomMapping, OTASyncStatusEnum, OTAWebhookCredential (+27 more)
...
Cohesion: 0.06
Nodes (26): PaymentMethod, ReservationStatus, 0a457cb feat(checkin): B3 partial check-in + mandatory guest profile capture + companions, 5577ff8 feat(reservations): add ReservationDetailDrawer component, e74081c feat(reservations): add minimal search filter for global reservation lookup, CheckinCaptureForm, emptyCaptureForm, guestFullName() (+18 more)
...
Cohesion: 0.07
Nodes (33): _attach_current_rate(), checkRoomAvailability(), createRoom(), createRoomCategory(), delete_room(), get_category(), housekeeping_summary(), list_categories() (+25 more)
...
Cohesion: 0.07
Nodes (15): OccupancyGridBlock, OccupancyGridReservation, OccupancyGridResponse, OccupancyGridRoom, 2eccdd8 feat(reservations): add occupancy-grid endpoint for B2 planilla, dad6d72 feat(frontend): add occupancy grid planilla (B2) at /operacion/planilla, CategoryGroup, DATE_LABEL (+7 more)
...
Cohesion: 0.15
Nodes (28): _apply_corporate_pricing(), _apply_custom_deposit_amount(), _apply_manual_total_override(), _apply_pricing_result_to_reservation(), assert_reservation_version(), calculate_reservation_pricing(), _calendar_pricing_source(), check_room_availability() (+20 more)
...
Cohesion: 0.10
Nodes (17): listGuests(), listRoomCategories(), listRooms(), cancelWaitlistEntry(), createWaitlistEntry(), listWaitlistEntries(), promoteWaitlistEntry(), API routes for reservation waitlist operations. (+9 more)
...
Cohesion: 0.11
Nodes (18): Category, listCategories(), Reservation, emptyQuickGuestForm(), GuestQuickCreatePanelProps, hasQuickGuestFormData(), QuickGuestFormValues, CHANNEL_OPTIONS (+10 more)
...
Cohesion: 0.12
Nodes (13): 218ef8b chore(graphify): regenerate graph after A2 reservation pagination/order, 3a2821c feat(reservations): wire skip/limit/order into the frontend reservation list, credentials, useReservationDrawer(), reservationsKey(), useReservations(), DashboardPage(), todayIso() (+5 more)

### Community 99 - "Community 99"
Cohesion: 0.27
Nodes (19): AnalyticsAIUsageMonthly, AnalyticsAlertSetting, AnalyticsAlertSnooze, AnalyticsCurrencyDisplayEnum, AnalyticsExportFormatEnum, FactReservationRowKindEnum, FactRoomOccupancyDaily, FactRoomOccupancyStatusAtNightEnum (+11 more)
...
Cohesion: 0.11
Nodes (16): OTAConnectionStatusEnum, OTASyncJobStatusEnum, PaymentLinkStatusEnum, PaymentProviderEnum, PaymentStatusEnum, Reference values for payments.provider / payment_links.provider (stored as Strin, Reference values for payments.status (stored as String)., Reference values for payment_links.status (stored as String). (+8 more)
...
Cohesion: 0.32
Nodes (5): _authorize_override(), checkin(), checkin_partial(), FastAPI routes for Check-in / Check-out., B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan
...
Cohesion: 0.48
Nodes (6): _active_reservation_conflicts_for_room(), create_grouped_room_move(), get_group(), list_groups(), revert_group(), RoomMovementGroupError
...
- **Thin community `Community 138`** (1 nodes): `FastAPI routes for the commercial configuration domain.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (2 nodes): `FakeRedis`, `test_availability_key_shape_and_serialization()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `Public WhatsApp bot hooks authenticated only by hotel API key.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (2 nodes): `mercadopago_payment_link_webhook()`, `_mercadopago_webhook_impl()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 261`** (1 nodes): `SimpleRateLimiter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 265`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 281`** (2 nodes): `housekeeping`, `owner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 282`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 283`** (2 nodes): `credentials`, `PAGES`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 304`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 305`** (2 nodes): `POST /api/checkin/checkout only enforced authentication (get_auth_context),`, `test_housekeeping_cannot_checkout_reservation()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 315`** (1 nodes): `add hotel scope to core tables  Revision ID: 20260404_add_hotel_scope Revises: c`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 340`** (1 nodes): `Add (hotel_id, created_at) index on reservations for A2 recent-order paging.  Re`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 341`** (1 nodes): `Add transactions.created_by_user_id for payment audit trail.  Transaction had cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 342`** (1 nodes): `ota hardening: hotel-scoped mappings and webhook credentials  Revision ID: a7f3d`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 343`** (1 nodes): `baseline  Revision ID: cb9001557529 Revises:  Create Date: 2026-03-31 19:04:53.7`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 344`** (2 nodes): `get_mongo_db()`, `mongo_healthcheck()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 345`** (2 nodes): `get_neo4j_driver()`, `neo4j_healthcheck()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 350`** (1 nodes): `Fast server-side EXPLAIN ANALYZE probe for hot queries on real PostgreSQL.  Rati`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 361`** (1 nodes): `_DecimalAwareEncoder`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 362`** (1 nodes): `Defensive datastore clients for optional infrastructure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 363`** (1 nodes): `Application decorators.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 364`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 365`** (2 nodes): `Derive the BRM §16.2 reportable origin for a reservation.      Precedence:`, `reportable_origin()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 371`** (2 nodes): `Preview QA already requires a strong MASTER_ADMIN_PASSWORD/EMAIL, but     produc`, `test_validate_runtime_security_rejects_weak_master_admin_password_in_production()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 372`** (2 nodes): `MERCADOPAGO_WEBHOOK_SECRET is required only when MP_ACCESS_TOKEN is set.`, `test_validate_runtime_security_rejects_missing_mp_webhook_when_mp_configured()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 2` to `Community 262`, `Community 12`, `Community 361`, `Community 3`, `Community 240`, `Community 222`, `Community 116`, `Community 264`, `Community 6`, `Community 28`, `Community 120`, `Community 104`, `Community 139`, `Community 4`, `Community 99`, `Community 65`, `Community 71`, `Community 7`, `Community 15`, `Community 13`, `Community 96`, `Community 147`, `Community 5`, `Community 21`, `Community 64`, `Community 60`, `Community 121`, `Community 0`, `Community 37`, `Community 111`, `Community 32`, `Community 105`, `Community 365`, `Community 44`, `Community 49`, `Community 57`, `Community 63`, `Community 132`, `Community 232`, `Community 84`, `Community 309`, `Community 207`, `Community 208`, `Community 131`?**
  _High betweenness centrality (0.128) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 7` to `Community 8`, `Community 6`, `Community 18`, `Community 9`, `Community 4`, `Community 97`, `Community 3`, `Community 2`, `Community 28`, `Community 0`, `Community 350`, `Community 99`, `Community 71`, `Community 5`, `Community 44`, `Community 64`, `Community 86`, `Community 37`, `Community 58`, `Community 89`, `Community 57`, `Community 80`, `Community 65`, `Community 132`, `Community 105`, `Community 84`, `Community 60`, `Community 309`, `Community 32`, `Community 13`, `Community 252`, `Community 41`, `Community 15`, `Community 46`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 0` to `Community 5`, `Community 4`, `Community 6`, `Community 43`, `Community 2`, `Community 3`, `Community 28`, `Community 104`, `Community 99`, `Community 71`, `Community 13`, `Community 225`, `Community 44`, `Community 7`, `Community 37`, `Community 58`, `Community 106`, `Community 49`, `Community 242`, `Community 227`, `Community 105`, `Community 134`, `Community 41`, `Community 15`, `Community 46`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Are the 772 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`ReservationStatusEnum` has 772 INFERRED edges - model-reasoned connections that need verification._
- **Are the 766 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Reservation` has 766 INFERRED edges - model-reasoned connections that need verification._
- **Are the 691 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 691 INFERRED edges - model-reasoned connections that need verification._
- **Are the 611 inferred relationships involving `Room` (e.g. with `RoomBlockCreate` and `RoomBlockRead`) actually correct?**
  _`Room` has 611 INFERRED edges - model-reasoned connections that need verification._