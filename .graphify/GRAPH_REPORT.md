...
## Corpus Check
- Large corpus: 1241 files · ~1,820,356 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 13235 nodes · 75471 edges · 476 communities detected
- Extraction: 66% EXTRACTED · 34% INFERRED · 0% AMBIGUOUS · INFERRED: 25882 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 25882 · ON_BRANCH: 23831 · contains: 7174 · calls: 5242 · MODIFIES: 5001 · rationale_for: 3461 · imports: 1391 · imports_from: 1097 · PARENT_OF: 877 · inherits: 775 · method: 736 · re_exports: 4


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 1241 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 12 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `f072302`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `Reservation` - 1390 edges
2. `ReservationStatusEnum` - 1335 edges
3. `HotelConfiguration` - 1295 edges
4. `Room` - 1201 edges
5. `RoomCategory` - 1058 edges
6. `Guest` - 981 edges
7. `RoomStatusEnum` - 715 edges
8. `Base` - 647 edges
9. `ReservationError` - 608 edges
10. `ReservationSourceEnum` - 533 edges
...
Cohesion: 0.01
Nodes (591): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses the ca, Calculate pricing for a potential booking without persisting it.     Uses the ca (+583 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (538): checkin_partial(), B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan, B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan, B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan, B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan, email_status(), Lightweight status so the frontend can check the active system email provider., Lightweight status so the frontend can check the active system email provider. (+530 more)

### Community 2 - "Community 2"
Cohesion: 0.15
Nodes (492): agent-linensplit, agent-login, agent-mobilenav, agent-otafix, agent-settlement, agent-stockedit, agent-stocksplit, agent-ux (+484 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (256): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, create_fx_snapshot(), FxRateItem, FxRateUsdOficial, FxSnapshotCreateResponse, get_all_rates(), get_single_rate(), get_usd_oficial_rate() (+248 more)

### Community 4 - "Community 4"
Cohesion: 0.04
Nodes (392): apply_period_to_daily_rates(), ApplyPeriodOut, _bulk_field_value(), bulk_update_daily_rate_field(), bulk_upsert_daily_rates(), BulkFieldRateIn, BulkRateIn, BulkRateOut (+384 more)

### Community 5 - "Community 5"
Cohesion: 0.01
Nodes (192): _archive_historical(), _build_cases(), main(), _write_json(), Shared, secret-safe binding for the Render QA bootstrap configuration., acceptInvitation(), AuthProvidersResponse, AuthResponse (+184 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (262): ApiKeyPurpose, HotelApiKey, HotelApiKeyIssued, issueApiKey(), IssueHotelApiKeyPayload, listApiKeys(), revokeApiKey(), Category (+254 more)

### Community 7 - "Community 7"
Cohesion: 0.03
Nodes (263): AuthUser, accept_invitation(), _available_invitation(), get_invitation(), _source_key(), _assert_assignable_role(), _assert_manageable_membership(), invite_user() (+255 more)

### Community 8 - "Community 8"
Cohesion: 0.01
Nodes (214): AllocationRunPayload, AllocationRunResponse, listRoomMovementGroups(), revertRoomMovementGroup(), RoomMoveEvent, RoomMovementGroup, triggerAllocationRecalculation(), getGuestProhibitedDetail() (+206 more)

### Community 9 - "Community 9"
Cohesion: 0.03
Nodes (212): booking_webhook(), despegar_webhook(), expedia_webhook(), _guarded_json_payload(), _handle_ota_webhook(), FastAPI Webhook endpoints for OTA integrations., Receive reservation notifications from Booking.com., Receive reservation notifications from Expedia. (+204 more)

### Community 10 - "Community 10"
Cohesion: 0.01
Nodes (211): FxSnapshotRead, create_laundry_remito(), _housekeeping_remito(), LinenItemCreate, LinenItemRead, LinenLocationCreate, LinenLocationRead, LinenMovementCreate (+203 more)

### Community 11 - "Community 11"
Cohesion: 0.13
Nodes (235): CheckInRequest, FastAPI routes for Check-in / Check-out., cancel_reservation(), change_dates(), create_new_reservation(), create_or_update_manual_ota(), create_reservation_charge(), _ensure_action_permission() (+227 more)

### Community 12 - "Community 12"
Cohesion: 0.01
Nodes (165): AnalyticsAIChatPage(), AnalyticsAIChatResponse, AnalyticsAIStatus, AnalyticsCategoryDetailPage(), AnalyticsChannelsPage(), AnalyticsCompanyDetailPage(), AnalyticsEnvelope, analyticsErrorMessage() (+157 more)

### Community 13 - "Community 13"
Cohesion: 0.01
Nodes (105): _ensure_wide_version_table(), get_url(), Alembic creates alembic_version with version_num VARCHAR(32) by default.     Som, run_migrations_offline(), run_migrations_online(), _booking_to_read(), cancel_booking(), checkin_booking() (+97 more)

### Community 14 - "Community 14"
Cohesion: 0.05
Nodes (208): Authenticated field-level collaboration endpoints.  Drafts are ephemeral and saf, Issue a short-lived, one-use ticket for one tenant resource., Persist an optimistic, field-level merge under the current tenant., Process-local peers plus best-effort Redis pub/sub fan-out., Authenticate with a one-use ticket, then exchange safe draft signals., Keep collaboration permissions aligned with the normal resource API., 955325e feat(security): add critical/step-up/delegable metadata and optimistic versioning to permission catalog, HotelMembership (+200 more)

### Community 15 - "Community 15"
Cohesion: 0.02
Nodes (186): Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, _require_demo_mode(), reset_demo(), seed_demo(), Base, Base class for all ORM models. (+178 more)

### Community 16 - "Community 16"
Cohesion: 0.02
Nodes (147): claude/feature/pms-roles-housekeeping-qa, 00bf891 Explain analytics errors and add retry action, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 03f4d70 Document cloud date and quote regressions, 0449677 Document cloud role and preview blocker, 0510fa0 Add guest companion UI journey, 05d296d Fix cash freshness after reservation payments (+139 more)

### Community 17 - "Community 17"
Cohesion: 0.05
Nodes (146): apple_callback(), _apple_full_name(), apple_login(), _apple_login_config(), apple_start(), _attach_user_session_cookies(), _audit_security_event(), auth_providers() (+138 more)

### Community 18 - "Community 18"
Cohesion: 0.02
Nodes (65): export_cash_ledger_csv(), addCashMovement(), approveCashCloseDifference(), CashCloseReport, CashCustodyHandoff, CashDailyCollector, CashDailyEntry, CashDailyPaymentMethod (+57 more)
...
Cohesion: 0.03
Nodes (57): requestVerification(), verifyEmail(), CategoryPayload, DepositPolicyPayload, finishOnboarding(), getOnboardingStatus(), HotelIdentityPayload, OnboardingProviderSetup (+49 more)
...
Cohesion: 0.03
Nodes (46): applyGemmaDraft(), approveGemmaAction(), archiveGemmaChatSession(), fetchGemmaChatHistory(), fetchGemmaChatSession(), fetchGemmaInsights(), fetchGemmaRuntimeStatus(), GemmaApplyDraftPayload (+38 more)

### Community 24 - "Community 24"
Cohesion: 0.11
Nodes (72): AnalyticsAIUsageMonthly, AnalyticsAlertSetting, AnalyticsAlertSnooze, FactReservationRowKindEnum, FactRoomOccupancyDaily, FactRoomOccupancyStatusAtNightEnum, HotelAuditEvent, RoomStateEvent (+64 more)
...
Cohesion: 0.05
Nodes (63): SimpleRateLimiter, MasterAdminAuditEvent, MasterAdminAuthLockout, MasterAdminSession, MasterStripeWebhookEvent, audit_master_action(), create_master_admin_mfa_challenge(), _json_or_null() (+55 more)
...
Cohesion: 0.06
Nodes (46): audit_timeline(), _build_audit_timeline_csv(), _current_user(), export_audit_timeline(), Tenant-scoped security overview and current-user session revocation., Invalidate every access token previously issued to the caller., Invalidate every access token previously issued to the caller., Invalidate every access token previously issued to the caller. (+38 more)

### Community 37 - "Community 37"
Cohesion: 0.06
Nodes (44): Base, MasterBillingPolicy, MasterStripeSettings, MasterSystemEmailConnection, FxPolicy, ProductRoomCompatibility, RatePlan, RatePlanPrice (+36 more)
...
Cohesion: 0.07
Nodes (27): delete_company_document(), get_company_document(), _get_document_or_404(), 02908b1 feat(storage): add verified tenant object metadata, CompanyDocument, CompanyDocumentStatusEnum, CompanyDocumentTypeEnum, CompanyDocument — attachments/vouchers for company reservations (v72 §3.6). Trac (+19 more)

### Community 41 - "Community 41"
Cohesion: 0.06
Nodes (37): AnalyticsCurrencyDisplayEnum, AnalyticsExportFormatEnum, HotelAPIKey, Hotel API key model — v72 §16.  Each hotel can have multiple named API credentia, Per-hotel API credential. `key_hash` stores a hashed version of the secret;, PaymentLinkStatusEnum, PaymentProviderEnum, PaymentStatusEnum (+29 more)
...
Cohesion: 0.15
Nodes (36): Operational task inbox and shift handoff endpoints., OperationalTask, OperationalTaskEvent, OperationalTaskPriorityEnum, OperationalTaskStatusEnum, OperationalTaskTypeEnum, ShiftHandoff, ShiftHandoffStatusEnum (+28 more)

### Community 44 - "Community 44"
Cohesion: 0.10
Nodes (40): RoomBlockCreate, RoomBlockRead, RoomBlock, RoomBlockReasonEnum, is_review_for_today(), A review is 'for today' when the stay is active on `today`.      §13.3: reviews, §13.3 review routing.      If the reservation under manual review is active *tod, A review is 'for today' when the stay is active on `today`.      §13.3: reviews (+32 more)
...
Cohesion: 0.11
Nodes (30): differentiators, faqItems, founder, heroBullets, integrationPoints, integrations, marketingRoutes, moduleGroups (+22 more)
...
Cohesion: 0.15
Nodes (24): 6e6d179 Prepare release candidate for landing and onboarding, 87b0eb6 Improve marketing SEO content, linking, and metadata, 9da1f66 Rebuild the public landing page as Hotels-PMS, b48fd42 Add public marketing inquiry flow and SEO, Seo(), SeoProps, StructuredData, resolveAppUrl() (+16 more)
...
Cohesion: 0.08
Nodes (24): graphify_command_error(), inline_list(), main(), Parse the simple unquoted frontmatter lists used by context packs., Parse the simple unquoted frontmatter lists used by context packs., Parse the simple unquoted frontmatter lists used by context packs., Reject context commands that the installed Graphify CLI cannot route., Reject context commands that the installed Graphify CLI cannot route. (+16 more)
...
Cohesion: 0.10
Nodes (26): admin_comped_override(), change_plan(), changeSubscriptionPlan(), getSubscriptionStatus(), listSubscriptionPlans(), _remaining_trial_days(), _serialize_status_payload(), start_subscription_trial() (+18 more)
...
Cohesion: 0.13
Nodes (27): 362fdd4 feat(security): add core TOTP/MFA enrollment, verification, and recovery codes, get_encryption_fernet(), Shared Fernet encryption for secrets stored in the database.  The integration en, Return the repository's existing Fernet instance for stored secrets., _account_label_from_payload(), connection_account_label(), decrypt_payload(), derive_expires_at() (+19 more)
...
Cohesion: 0.10
Nodes (29): 12f7011 fix(subscription): route all subscription writes through the v2 sync service, _collect_plan_entitlements(), delete_entitlement_override(), ensure_entitlements_seeded(), ensure_subscription(), entitlements_payload(), get_effective_room_limit(), get_entitlements() (+21 more)

### Community 79 - "Community 79"
Cohesion: 0.16
Nodes (28): da6d6d2 fix(security): shorten master admin session TTL (TECH-0071), allow_master_admin_mfa_attempt(), _as_aware(), authenticate_master_login(), authenticate_master_mfa_login(), _authorize_user_for_master_panel(), _bootstrap_master_credentials_match(), create_master_session() (+20 more)
...
Cohesion: 0.13
Nodes (20): collaboration_websocket(), CollaborationManager, CollaborationPatchResponse, CollaborationTicket, _consume_ticket(), create_collaboration_ticket(), _live_ticket_is_authorized(), _message_allowed() (+12 more)
...
Cohesion: 0.11
Nodes (20): ensure_room_within_limit(), ensure_staff_within_limit(), get_effective_staff_limit(), is_enforcement_enabled(), Raise 402 when enforcement is enabled and subscription is inactive., Raise 402 when enforcement is enabled and subscription is inactive., Raise 402 when enforcement is enabled and subscription is inactive., Enforce room limit entitlement; honors enforcement flag. (+12 more)
...
Cohesion: 0.13
Nodes (13): submitLead(), hero, EarlyAccessForm(), EarlyAccessFormProps, MESSAGES, Status, HeroSection(), DAYS (+5 more)
...
Cohesion: 0.14
Nodes (11): 67e4e68 fix(data-integrity): make audit_logs append-only at the database level, Reproduces the DELETE /api/stock/items/{id} incident: a stray NOT     NULL colum, test_audit_log_hotel_delete_is_restricted(), test_failed_audit_log_insert_does_not_roll_back_callers_change(), downgrade(), _hotel_fk(), Prevent hotel deletion from cascading into audit_logs.  Replaces the audit_logs., _replace_fk() (+3 more)
...
Cohesion: 0.23
Nodes (15): Auditable guest communications initiated from a reservation., One attempted reservation email, scoped to the owning hotel.      ``accepted`` m, ReservationEmailDelivery, ReservationEmailKindEnum, ReservationEmailStatusEnum, HotelOutboundSendResult, build_reservation_email(), _clean_email() (+7 more)

### Community 134 - "Community 134"
Cohesion: 0.18
Nodes (16): apply_resource_changes(), editable_resource_values(), get_resource(), get_resource_spec(), _json_safe(), _model_dump_for_database(), _model_dump_for_wire(), normalize_resource_type() (+8 more)
...
Cohesion: 0.14
Nodes (4): _plan(), The two endpoints the public website calls, and the ways an anonymous caller cou, TestLeadCapture, TestPublicPricing
...
Cohesion: 0.15
Nodes (16): DomainEventOutbox, One durable delivery attempt for one hotel/domain invalidation., queue_domain_change(), QueuedDomainChange, A safe invalidation signal waiting for the surrounding DB commit., A safe invalidation signal waiting for the surrounding DB commit., A safe invalidation signal waiting for the surrounding DB commit., Queue one tenant-scoped change for publication after commit.      Services can c (+8 more)
...
Cohesion: 0.29
Nodes (7): Dependency factory that enforces a permission via the configurable hotel     per, Dependency factory that enforces a permission via the configurable hotel     per, Dependency factory that enforces a permission via the configurable hotel     per, Dependency factory that enforces a permission via the configurable hotel     per, Keep a hard role boundary while also requiring a delegated capability., require_permission(), require_roles_and_permission()
...
Cohesion: 0.50
Nodes (4): Emit bounded HTTP timing fields and make correlation IDs observable., Emit bounded HTTP timing fields and make correlation IDs observable., Emit bounded HTTP timing fields and make correlation IDs observable., request_telemetry()
...
## Knowledge Gaps
- **1369 isolated node(s):** `add user TOTP MFA and recovery codes  Revision ID: 0c66ee6f32fa Revises: 2026082`, `add temporary action grants  Revision ID: 0f85dca5b98b Revises: 20260820_user_se`, `Install the PostgreSQL tenant policy; no-op on other dialects.`, `Remove the PostgreSQL tenant policy before dropping the table.`, `guest checkin profile fields  Revision ID: 17f1689785f3 Revises: 20260725_res_ho` (+1364 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 72`** (1 nodes): `BookingAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (1 nodes): `test_database_foundation_complete()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 148`** (2 nodes): `ExpediaAdapter`, `OTAProviderAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 152`** (1 nodes): `OnboardingState`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 160`** (1 nodes): `DespegarAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 271`** (2 nodes): `FakeRedis`, `test_availability_key_shape_and_serialization()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 299`** (2 nodes): `mercadopago_payment_link_webhook()`, `_mercadopago_webhook_impl()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 339`** (2 nodes): `_context()`, `test_generic_room_status_patch_projects_event_and_reallocates()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 364`** (1 nodes): `Staff management endpoints for hotel public API keys.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 369`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 373`** (2 nodes): `HotelConfigUpdate`, `_normalize_currency()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 374`** (1 nodes): `HotelIdentityPayload`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 384`** (1 nodes): `Response`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 399`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 400`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 401`** (2 nodes): `ownerCredentials`, `receptionistCredentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 406`** (2 nodes): `POST /api/checkin/checkout only enforced authentication (get_auth_context),`, `test_housekeeping_cannot_checkout_reservation()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 407`** (2 nodes): `_override_auth()`, `test_receptionist_can_get_price_quote()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 412`** (1 nodes): `Regression: provider-supplied OAuth error text must not break out of the inline`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 423`** (1 nodes): `add user TOTP MFA and recovery codes  Revision ID: 0c66ee6f32fa Revises: 2026082`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 424`** (1 nodes): `guest checkin profile fields  Revision ID: 17f1689785f3 Revises: 20260725_res_ho`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 425`** (1 nodes): `v72 gaps phase 1: Numeric precision, room score/accessibility, guest dedup+ratin`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 426`** (1 nodes): `v72 gaps phase 4: PRE_CHECK_IN state, RoomMoveEvent audit fields, company_docume`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 427`** (1 nodes): `v72 gaps phase 5: optimistic locking on reservations  Revision ID: 20260612_v72_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 428`** (1 nodes): `v72 gaps phase 6: payment tables (payments, payment_links, payment_webhook_event`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 429`** (1 nodes): `Add one-open-cash-session partial unique index.  Revision ID: 20260613_cash_open`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 430`** (1 nodes): `laundry and stock foundations  Revision ID: 20260613_laundry_stock Revises: 2026`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 431`** (1 nodes): `Add transaction idempotency key for gateway payments.  Revision ID: 20260613_pay`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 432`** (1 nodes): `permission matrix, role boundaries, and security audit log  Revision ID: 2026061`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 433`** (1 nodes): `Add PostgreSQL trigram indexes for guest search hot path.  Revision ID: 20260614`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 434`** (1 nodes): `Add daily rates and price periods.  Revision ID: 20260624_daily_rates Revises: 2`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 435`** (1 nodes): `v72 fx rate snapshots table.  Revision ID: 20260624_fx_rate_snapshots Revises: 2`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 436`** (1 nodes): `drop orphan payment_surcharge_configs table (R5b).  main carries TWO surcharge i`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 437`** (1 nodes): `Add configurable public API rate limit per hotel.  Revision ID: 20260625_public_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 438`** (1 nodes): `v72 §16.2: add 'company' value to reservation_channel_code_enum.  Adds the corpo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 439`** (1 nodes): `add reservations.settlement_due_date for corporate deferred billing (R5b §3.5).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 440`** (1 nodes): `reservation waitlist flags (BRM v72 §9).  Add denormalized waitlist flags to res`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 441`** (1 nodes): `soft delete audited domain records.  Revision ID: 20260625_soft_delete Revises:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 442`** (1 nodes): `Store private transfer-proof bytes separately from searchable metadata.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 443`** (1 nodes): `Add private bank-transfer proof workflow.  Revision ID: 20260724_payment_proofs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 444`** (1 nodes): `Add (hotel_id, created_at) index on reservations for A2 recent-order paging.  Re`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 445`** (1 nodes): `Add (hotel_id, room_id, check_in_date, check_out_date) index on reservations for`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 446`** (1 nodes): `Add transactions.created_by_user_id for payment audit trail.  Transaction had cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 447`** (1 nodes): `repair: create hotel_memberships table (was never migrated)  Revision ID: 202607`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 448`** (1 nodes): `Add quoted_amount_ars/quoted_amount_usd to reservations (dual manual OTA pricing`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 449`** (1 nodes): `stock_items.kind (supply vs linen) + soft-delete-aware name uniqueness  Revision`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 450`** (1 nodes): `repair: ensure (hotel_id, id) unique constraints exist on rooms/room_categories/`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 451`** (1 nodes): `add idempotency_key to stock_movements  Revision ID: 20260818_stock_movement_ide`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 452`** (1 nodes): `Add normal-user server-side sessions for TECH-0021.  Revision ID: 20260820_user_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 453`** (1 nodes): `Add soft-delete metadata to guests and payments for TECH-0110.  The columns are`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 454`** (1 nodes): `Grant receptionist the same-category room move default.  Phase A narrowed reserv`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 455`** (1 nodes): `Fix ota_reservation_lifecycle_enum labels to match the ORM's values_callable.  ``
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 456`** (1 nodes): `add public marketing pricing plans and early-access leads  Revision ID: 20260910`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 457`** (1 nodes): `merge stock kind fix and laundry vendor settlements  Revision ID: 513720cf2551 R`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 458`** (1 nodes): `bind users to Google subject  Revision ID: 5e86f1b9ccbb Revises: 0c66ee6f32fa Cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 459`** (1 nodes): `merge linen split and reservation quoted dual amounts  Revision ID: 6feb3dd16d0f`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 460`** (1 nodes): `laundry vendors and remitos  Revision ID: 795e124adaad Revises: 62fddec52b79 Cre`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 461`** (1 nodes): `repair: ensure uq_stock_items_hotel_id_id / uq_stock_locations_hotel_id_id exist`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 462`** (1 nodes): `laundry vendor settlements (quarterly paid/not-paid mark)  Revision ID: e2c4a9f7`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 463`** (1 nodes): `repair audit_logs stray legacy columns  Revision ID: ebd2db08c7d0 Revises: 6feb3`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 464`** (2 nodes): `main()`, `validate()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 465`** (2 nodes): `BridgeActivity`, `MainActivity`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 466`** (1 nodes): `owner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 467`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 471`** (1 nodes): `One-shot notification cycle: generate due daily reports, then deliver pending ou`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 475`** (1 nodes): `TestCurrentSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 476`** (1 nodes): `TestExportSession`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 477`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 480`** (1 nodes): `Minimal API for recording room preferences on a guest profile.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 483`** (1 nodes): `Pydantic schemas for the small guest-room preference signal.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 487`** (1 nodes): `Tighten housekeeping's default access to occupancy planning.  Revision ID: 20260`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 488`** (1 nodes): `Set one global default without creating duplicate rows on reruns.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 489`** (1 nodes): `add tenant-scoped guest room preferences  Revision ID: 3c2dea57666d Revises: 202`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 490`** (1 nodes): `Install the PostgreSQL tenant policy; no-op on other dialects.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 491`** (1 nodes): `Remove the PostgreSQL tenant policy before dropping its table.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 15` to `Community 13`, `Community 3`, `Community 397`, `Community 27`, `Community 37`, `Community 31`, `Community 38`, `Community 0`, `Community 24`, `Community 41`, `Community 67`, `Community 11`, `Community 1`, `Community 18`, `Community 26`, `Community 4`, `Community 40`, `Community 117`, `Community 151`, `Community 30`, `Community 25`, `Community 14`, `Community 5`, `Community 55`, `Community 34`, `Community 142`, `Community 112`, `Community 28`, `Community 152`, `Community 43`, `Community 9`, `Community 68`, `Community 85`, `Community 92`, `Community 132`, `Community 133`, `Community 44`, `Community 370`, `Community 6`, `Community 80`, `Community 35`, `Community 104`, `Community 69`, `Community 17`, `Community 94`, `Community 19`, `Community 103`, `Community 410`, `Community 140`, `Community 267`, `Community 415`, `Community 416`, `Community 417`, `Community 36`, `Community 158`, `Community 165`, `Community 207`, `Community 102`?**
  _High betweenness centrality (0.135) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 1` to `Community 6`, `Community 34`, `Community 11`, `Community 0`, `Community 9`, `Community 5`, `Community 118`, `Community 13`, `Community 15`, `Community 37`, `Community 24`, `Community 285`, `Community 133`, `Community 10`, `Community 66`, `Community 28`, `Community 95`, `Community 4`, `Community 44`, `Community 80`, `Community 78`, `Community 124`, `Community 3`, `Community 119`, `Community 67`, `Community 230`, `Community 19`, `Community 41`, `Community 107`, `Community 103`, `Community 410`, `Community 140`, `Community 53`, `Community 27`, `Community 68`, `Community 14`, `Community 415`, `Community 416`, `Community 417`, `Community 36`, `Community 158`, `Community 165`, `Community 60`, `Community 26`, `Community 48`, `Community 475`, `Community 476`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 1` to `Community 0`, `Community 9`, `Community 11`, `Community 43`, `Community 13`, `Community 15`, `Community 37`, `Community 38`, `Community 24`, `Community 40`, `Community 30`, `Community 95`, `Community 4`, `Community 44`, `Community 85`, `Community 133`, `Community 286`, `Community 41`, `Community 107`, `Community 145`, `Community 26`, `Community 48`, `Community 475`, `Community 476`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Are the 1383 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses the ca`) actually correct?**
  _`Reservation` has 1383 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1333 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses the ca`) actually correct?**
  _`ReservationStatusEnum` has 1333 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1288 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 1288 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1196 inferred relationships involving `Room` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses the ca`) actually correct?**
  _`Room` has 1196 INFERRED edges - model-reasoned connections that need verification._