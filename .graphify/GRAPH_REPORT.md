...
## Corpus Check
- Large corpus: 786 files · ~527,567 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 7429 nodes · 25374 edges · 312 communities detected
- Extraction: 62% EXTRACTED · 38% INFERRED · 0% AMBIGUOUS · INFERRED: 9519 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 9519 · contains: 4546 · calls: 3442 · MODIFIES: 2039 · ON_BRANCH: 1681 · rationale_for: 1207 · imports: 758 · imports_from: 594 · method: 591 · inherits: 535 · PARENT_OF: 462


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 786 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 8 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `d508aad`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `ReservationStatusEnum` - 620 edges
2. `Reservation` - 620 edges
3. `HotelConfiguration` - 554 edges
4. `Room` - 478 edges
5. `Guest` - 451 edges
6. `RoomCategory` - 435 edges
7. `Base` - 372 edges
8. `RoomStatusEnum` - 311 edges
9. `TransactionTypeEnum` - 287 edges
10. `PaymentMethodEnum` - 277 edges
...
Cohesion: 0.02
Nodes (180): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, acceptInvitation(), AuthResponse, AuthUser, _build_auth_response(), _generate_code(), getInvitationInfo(), _issue_email_token() (+172 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (199): Public booking-engine API authenticated only with hotel API keys., Resolve today's effective rate from the single source of truth and attach it, Resolve today's effective rate from the single source of truth and attach it, Simple availability helper. Returns a placeholder message if required     parame, Generic room update (number, floor, notes, status, etc.)., Update room status for housekeeping. When a room is set to cleaning/maintenance/, Update the category of a specific room., Manually trigger the allocation engine to optimally redistribute all reservation (+191 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (186): claude/feature/pms-roles-housekeeping-qa, 00bf891 Explain analytics errors and add retry action, 0255d4f Normalize Graphify artifact paths, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 0391f8f Fix flaky detached-node false positive in mobile touch-target check, 03ec9de Refresh Graphify after Fase 9 role QA fixes, 03f4d70 Document cloud date and quote regressions (+178 more)
...
Cohesion: 0.06
Nodes (172): Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, FastAPI routes for Reports & Night Audit. Daily summaries, occupancy reports, re, Occupancy report for a date range (default: last 30 days)., Revenue report for a date range., Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c, HotelConfiguration, Configuration table scoped by hotel (id == hotel_id)., BillingAdjustment (+164 more)

### Community 5 - "Community 5"
Cohesion: 0.07
Nodes (124): cancel_reservation(), extend_stay(), mark_no_show(), modify_reservation(), FastAPI routes for Reservations. Complete CRUD + cancel, modify, no-show, extend, §5.2 — Run allocation engine in background after reservation changes.     Create, §5.2 — Run allocation engine in background after reservation changes.     Create, Register the deferred corporate collection (v72 §3.5): settled. (+116 more)

### Community 6 - "Community 6"
Cohesion: 0.03
Nodes (101): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+93 more)

### Community 7 - "Community 7"
Cohesion: 0.03
Nodes (110): BaseModel, BillingPolicyPayload, BillingPolicyUpdateRequest, EmailTestRequest, MasterAdminLoginRequest, MasterAdminLoginResponse, MasterAdminSessionResponse, MasterAdminUserPayload (+102 more)

### Community 8 - "Community 8"
Cohesion: 0.03
Nodes (101): AllocationRunPayload, AllocationRunResponse, listRoomMovementGroups(), revertRoomMovementGroup(), RoomMoveEvent, RoomMovementGroup, triggerAllocationRecalculation(), createPaymentLink() (+93 more)
...
Cohesion: 0.02
Nodes (29): dc4b68a ojooooo, e34785c prueba, build_default_ota_orchestrator(), get_default_adapter(), Default OTA adapter registry.  This keeps provider construction in one place so, DashboardPage(), todayIso(), bubbleStyles (+21 more)

### Community 11 - "Community 11"
Cohesion: 0.04
Nodes (93): Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, Base, Base class for all ORM models., lifespan(), Initialize database on application startup. (+85 more)

### Community 12 - "Community 12"
Cohesion: 0.05
Nodes (77): RoomBlockCreate, RoomBlockRead, add_companions(), FastAPI routes for Guest management., Add new companions to an existing guest., Add new companions to an existing guest., RoomBlockCreate, RoomBlockRead (+69 more)

### Community 13 - "Community 13"
Cohesion: 0.08
Nodes (86): ReservationAllocationLock, Company, Groups multiple RoomMoveEvents triggered by the same cause (v72 §5.4).     Suppo, RoomMoveEvent, RoomMovementGroup, RoomMoveTypeEnum, ReservationNoShowPolicyAppliedEnum, AllocationError (+78 more)

### Community 14 - "Community 14"
Cohesion: 0.03
Nodes (35): FastAPI routes for provider connections. Exposes /api/connections/{provider}/con, _require_demo_mode(), reset_demo(), seed_demo(), _backfill_not_null_nulls(), _column_fill_value(), get_db(), get_engine() (+27 more)

### Community 15 - "Community 15"
Cohesion: 0.04
Nodes (45): email_status(), getHotelConfig(), HotelConfig, HotelConfigUpdate, FastAPI routes for Hotel Configuration (Admin Panel)., Lightweight status so the frontend can check the active system email provider., updateHotelConfig(), createPaymentSurcharge() (+37 more)

### Community 16 - "Community 16"
Cohesion: 0.11
Nodes (64): CashCloseReport, CashCustodyHandoff, CashCustodyStatusEnum, CashMovement, CashMovementTypeEnum, CashSession, CashSessionStatusEnum, Cash register (caja) models — Sprint 1 requirement per v72 §13.  Lifecycle:   Ca (+56 more)

### Community 17 - "Community 17"
Cohesion: 0.04
Nodes (47): getPaymentSummary(), makePayment(), newPaymentIdempotencyKey(), PaymentMethod, PaymentRequest, PaymentSummary, FastAPI routes for Payments., TransactionType (+39 more)
...
Cohesion: 0.09
Nodes (52): CompanyDocument, CompanyDocumentStatusEnum, CompanyDocumentTypeEnum, CompanyDocument — attachments/vouchers for company reservations (v72 §3.6). Trac, Document/voucher associated with a company reservation (v72 §3.6).     If requir, HotelPermissionOverride, Permission, Configurable permission matrix models. (+44 more)
...
Cohesion: 0.07
Nodes (53): Payment, PaymentLink, PaymentLinkStatusEnum, PaymentProviderEnum, PaymentStatusEnum, Payment gateway tracking models: PaymentLink, Payment, PaymentWebhookEvent.  Sou, One gateway payment object (MP preference / PayPal order / Stripe intent)., Reference values for payments.provider / payment_links.provider (stored as Strin (+45 more)

### Community 26 - "Community 26"
Cohesion: 0.06
Nodes (50): _canonical_identity(), _enabled(), _identity_contains(), _load_local_evidence(), _load_provider_evidence(), Fail-closed safety guard for PostgreSQL tests that mutate schema or data.  Remot, Return ``dsn`` only when provider evidence proves a disposable QA target.      E, Return ``dsn`` only when provider evidence proves a disposable QA target.      E (+42 more)
...
Cohesion: 0.14
Nodes (44): AnalyticsAIUsageMonthly, AnalyticsAlertSetting, AnalyticsAlertSnooze, AnalyticsCurrencyDisplayEnum, AnalyticsExportFormatEnum, FactReservationRowKindEnum, FactRoomOccupancyDaily, FactRoomOccupancyStatusAtNightEnum (+36 more)
...
Cohesion: 0.07
Nodes (40): main(), valid_https(), bootstrap_configuration_fingerprint(), Shared, secret-safe binding for the Render QA bootstrap configuration., Hash the exact provider-observed values without exposing them individually., Hash the exact provider-observed values without exposing them individually., Hash the exact provider-observed values without exposing them individually., _canonical_host() (+32 more)

### Community 33 - "Community 33"
Cohesion: 0.06
Nodes (34): ABC, Auth endpoints: register, login, email verification and password reset. Verifica, Validate reset code without changing password (paso previo en UI)., Prefer a hotel whose onboarding is already complete.      When a user belongs to, validate_reset(), User management per hotel (owners/co-owners)., UpdateRolePayload, EmailProvider (+26 more)
...
Cohesion: 0.18
Nodes (37): OTAConnection, OTAProvider, OTARatePlanMapping, OTAReservationLifecycleEnum, OTAReservationLink, OTARoomMapping, OTAReservationMapping, OTASyncStatusEnum (+29 more)
...
Cohesion: 0.11
Nodes (9): _ensure_membership_and_subscription(), ensure_plans_seeded(), get_or_create_hotel_for_owner(), Hotel helper utilities (ownership + bootstrap)., Seed default plans if missing., Find a hotel configuration owned by the given email, or create a new one.     Al, Create owner membership and starter subscription if missing., _hotel_default_currency() (+1 more)
...
Cohesion: 0.08
Nodes (12): 4f50de1 Add successor cash register controls, b4c9bd8 Harden cash rotation and payment integrity, _add_successor_reference(), downgrade(), _drop_successor_reference(), Add zero-balance cash rotation and custody handoffs., upgrade(), downgrade() (+4 more)
...
Cohesion: 0.09
Nodes (15): create_movement(), CurrentStock, DecimalValue, _ensure_adjustment_permission(), FastAPI routes for stock and inventory operations., StockItem, StockItemCreate, StockItemRead (+7 more)
...
Cohesion: 0.10
Nodes (18): _attach_current_rate(), createRoom(), createRoomCategory(), get_category(), list_categories(), listRoomCategories(), listRooms(), FastAPI routes for Room management + Housekeeping. (+10 more)

### Community 68 - "Community 68"
Cohesion: 0.17
Nodes (25): PaymentProof, PaymentProofBlob, PaymentProofStatusEnum, Manual bank-transfer evidence and approval lifecycle., Private image evidence awaiting explicit staff confirmation., Private image evidence awaiting explicit staff confirmation., Private binary payload kept separately from queryable proof metadata., Private binary payload kept separately from queryable proof metadata. (+17 more)
...
Cohesion: 0.12
Nodes (23): delete_room(), housekeeping_summary(), Simple availability helper. Returns a placeholder message if required     parame, Generic room update (number, floor, notes, status, etc.)., Update room status for housekeeping. When a room is set to cleaning/maintenance/, Update the category of a specific room., Manually trigger the allocation engine to optimally redistribute all reservation, Get housekeeping overview: how many rooms in each status. (+15 more)

### Community 90 - "Community 90"
Cohesion: 0.09
Nodes (18): createStockItem(), createStockLocation(), createStockMovement(), getCurrentStock(), listLowStockItems(), listStockItems(), listStockLocations(), listStockMovements() (+10 more)
...
Cohesion: 0.11
Nodes (11): Tests for database models — validates schema creation, constraints, relationship, Verify auto-generated timestamps., Tests for Transaction model., Create a transaction and verify attributes., Verify Transaction → Reservation relationship., Tests for Guest and GuestCompanion models., Verify guest with full data., Verify the has_valid_identity property detects missing documents. (+3 more)
...
Cohesion: 0.20
Nodes (9): 1ab7400 fix(stock): allow "Ajuste" movements to decrease stock, not only increase it, 3082b76 fix(stock): add a direction toggle for "Ajuste" and split history labels, 84296c9 test(stock): add a dedicated end-to-end inventory journey (Fase 7), d508aad docs(qa): record Fase 7 inventory findings in the regression catalog, credentials, _constraint(), downgrade(), Allow downward stock adjustments.  Previously "adjustment" stock movements could (+1 more)
...
Cohesion: 0.33
Nodes (10): _adjacency_bonus_for_room(), AllocationResult, build_slots_from_db(), _check_overlap(), _is_protected_reservation(), _lowest_available_compatible_floor(), _lowest_compatible_floor(), _one_night_gap_penalty_for_room() (+2 more)
...
Cohesion: 0.47
Nodes (9): _image_base64(), _jpeg_with_exif_base64(), _reservation(), test_approval_creates_completed_transfer_transaction_and_updates_balance(), test_proof_bytes_are_tenant_scoped(), test_rejected_proof_requires_reason_and_cannot_be_approved(), test_submit_transfer_proof_allows_amount_covering_billing_adjustments(), test_submit_transfer_proof_reencodes_and_removes_exif() (+1 more)
...
- **Thin community `Community 130`** (1 nodes): `FastAPI routes for the commercial configuration domain.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 199`** (2 nodes): `FakeRedis`, `test_availability_key_shape_and_serialization()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `Public WhatsApp bot hooks authenticated only by hotel API key.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 295`** (1 nodes): `v72 gaps phase 6: payment tables (payments, payment_links, payment_webhook_event`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 296`** (1 nodes): `Add one-open-cash-session partial unique index.  Revision ID: 20260613_cash_open`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 297`** (1 nodes): `laundry and stock foundations  Revision ID: 20260613_laundry_stock Revises: 2026`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 298`** (1 nodes): `Add transaction idempotency key for gateway payments.  Revision ID: 20260613_pay`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 299`** (1 nodes): `permission matrix, role boundaries, and security audit log  Revision ID: 2026061`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 300`** (1 nodes): `Add PostgreSQL trigram indexes for guest search hot path.  Revision ID: 20260614`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 301`** (1 nodes): `Add daily rates and price periods.  Revision ID: 20260624_daily_rates Revises: 2`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 302`** (1 nodes): `v72 fx rate snapshots table.  Revision ID: 20260624_fx_rate_snapshots Revises: 2`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 303`** (1 nodes): `v72 section 12.3 - payment_surcharges table.  Revision ID: 20260624_payment_surc`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 304`** (1 nodes): `drop orphan payment_surcharge_configs table (R5b).  main carries TWO surcharge i`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 305`** (1 nodes): `Add configurable public API rate limit per hotel.  Revision ID: 20260625_public_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 306`** (1 nodes): `v72 §16.2: add 'company' value to reservation_channel_code_enum.  Adds the corpo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 307`** (1 nodes): `add reservations.settlement_due_date for corporate deferred billing (R5b §3.5).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 308`** (1 nodes): `reservation waitlist flags (BRM v72 §9).  Add denormalized waitlist flags to res`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 309`** (1 nodes): `soft delete audited domain records.  Revision ID: 20260625_soft_delete Revises:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 310`** (1 nodes): `Store private transfer-proof bytes separately from searchable metadata.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 311`** (1 nodes): `Add private bank-transfer proof workflow.  Revision ID: 20260724_payment_proofs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 316`** (2 nodes): `get_neo4j_driver()`, `neo4j_healthcheck()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 322`** (1 nodes): `Defensive datastore clients for optional infrastructure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 323`** (1 nodes): `Application decorators.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 324`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 325`** (2 nodes): `Derive the BRM §16.2 reportable origin for a reservation.      Precedence:`, `reportable_origin()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 11` to `Community 242`, `Community 14`, `Community 0`, `Community 243`, `Community 150`, `Community 263`, `Community 5`, `Community 30`, `Community 115`, `Community 37`, `Community 13`, `Community 29`, `Community 63`, `Community 16`, `Community 20`, `Community 102`, `Community 9`, `Community 139`, `Community 1`, `Community 12`, `Community 23`, `Community 4`, `Community 3`, `Community 46`, `Community 31`, `Community 116`, `Community 36`, `Community 34`, `Community 25`, `Community 68`, `Community 325`, `Community 6`, `Community 58`, `Community 245`, `Community 62`, `Community 215`, `Community 84`, `Community 197`, `Community 198`, `Community 123`?**
  _High betweenness centrality (0.128) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 4` to `Community 15`, `Community 46`, `Community 5`, `Community 0`, `Community 33`, `Community 69`, `Community 3`, `Community 11`, `Community 30`, `Community 12`, `Community 98`, `Community 29`, `Community 6`, `Community 1`, `Community 249`, `Community 48`, `Community 86`, `Community 36`, `Community 31`, `Community 9`, `Community 13`, `Community 58`, `Community 79`, `Community 63`, `Community 193`, `Community 92`, `Community 84`, `Community 107`, `Community 120`, `Community 34`, `Community 16`, `Community 43`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 1` to `Community 6`, `Community 12`, `Community 69`, `Community 4`, `Community 5`, `Community 67`, `Community 89`, `Community 3`, `Community 11`, `Community 30`, `Community 13`, `Community 153`, `Community 37`, `Community 29`, `Community 20`, `Community 266`, `Community 36`, `Community 48`, `Community 25`, `Community 68`, `Community 99`, `Community 92`, `Community 107`, `Community 120`, `Community 46`, `Community 16`, `Community 43`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Are the 618 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`ReservationStatusEnum` has 618 INFERRED edges - model-reasoned connections that need verification._
- **Are the 613 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Reservation` has 613 INFERRED edges - model-reasoned connections that need verification._
- **Are the 547 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 547 INFERRED edges - model-reasoned connections that need verification._
- **Are the 473 inferred relationships involving `Room` (e.g. with `RoomBlockCreate` and `RoomBlockRead`) actually correct?**
  _`Room` has 473 INFERRED edges - model-reasoned connections that need verification._