...
## Corpus Check
- Large corpus: 779 files · ~518,765 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 7313 nodes · 24867 edges · 320 communities detected
- Extraction: 63% EXTRACTED · 37% INFERRED · 0% AMBIGUOUS · INFERRED: 9306 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 9306 · contains: 4497 · calls: 3402 · MODIFIES: 1977 · ON_BRANCH: 1613 · rationale_for: 1178 · imports: 750 · imports_from: 590 · method: 590 · inherits: 535 · PARENT_OF: 429
...
Cohesion: 0.03
Nodes (199): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+191 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (142): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, requestVerification(), apiFetch(), buildAuthHeaders(), buildUrl(), formatErrorDetail(), handleUnauthorized(), hasValidSession() (+134 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (188): claude/feature/pms-roles-housekeeping-qa, codex/feature/pms-e2e-scale-hardening, 00bf891 Explain analytics errors and add retry action, 0255d4f Normalize Graphify artifact paths, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 03f4d70 Document cloud date and quote regressions, 0449677 Document cloud role and preview blocker (+180 more)

### Community 3 - "Community 3"
Cohesion: 0.06
Nodes (185): chore/weekly-orchestrator-2026-07-03, codex/feature/agent-ops-knowledge, codex/feature/trusted-qa-bootstrap, codex/fix-render-postgres-enum-values, main, 01498d0 Harden password reset flow (verified-only, dev code return), 0737a5a feat(frontend): V72 Wave A/B/C — companies, cash, reports, waitlist, laundry, stock, api-keys, permissions, whatsapp, 093f7d3 ui(manager): ocultar config y redirigir al dashboard al cambiar vista (+177 more)

### Community 4 - "Community 4"
Cohesion: 0.05
Nodes (161): deactivate_payment_surcharge(), _get_surcharge_or_404(), PaymentSurchargeCreate, PaymentSurchargeRead, PaymentSurchargeUpdate, Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, update_payment_surcharge(), BillingAdjustment (+153 more)
...
Cohesion: 0.04
Nodes (88): Base, Base class for all ORM models., DeclarativeBase, audited_change(), Record an AuditLog row for a successful entity mutation., Record an AuditLog row for a successful entity mutation., AuditLog, Append-only record of every significant mutation within a hotel tenant.     Do N (+80 more)
...
Cohesion: 0.07
Nodes (57): add_companions(), FastAPI routes for Guest management., Add new companions to an existing guest., Add new companions to an existing guest., GuestRatingEnum, GuestTag, GuestTagTypeEnum, Guest and GuestCompanion models. Exhaustive fields for check-in panel: identity (+49 more)
...
Cohesion: 0.24
Nodes (32): Exception, OTAConnection, OTAProvider, OTARatePlanMapping, OTAReservationLifecycleEnum, OTAReservationLink, OTARoomMapping, OTAReservationMapping (+24 more)
...
Cohesion: 0.10
Nodes (16): 19b8b7b Harden isolated preview QA and trusted release evidence, RuntimeError, _credentials(), E2ESafetyError, main(), normalize_sqlite_url(), prepare_e2e_environment(), Prepare the fixed local SQLite database used by Playwright E2E tests.  The safet (+8 more)
...
Cohesion: 0.10
Nodes (16): 181a413 Record Fase5 reception journey findings in the regression catalog, 411ce75 Add a full laundry cycle regression test for the housekeeping role, 54c5e97 Record housekeeping laundry cycle regression in the catalog, 9e4d99e Fix the reservation payment panel to use the operational balance, a02c36f Record manager report reconciliation regression in the catalog, bf892a8 Grant receptionist access to GET /api/bookings/price-quote, d19e069 Refresh Graphify after this session's role/payment fixes; widen portability normalizer, ef18b57 Add a manager report reconciliation test (Fase 8) (+8 more)
