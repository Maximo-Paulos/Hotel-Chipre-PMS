...
## Corpus Check
- Large corpus: 1315 files · ~1,890,997 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 11299 nodes · 29884 edges · 533 communities detected
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 6491 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: contains: 7623 · uses: 6491 · calls: 5553 · MODIFIES: 3014 · rationale_for: 1549 · imports: 1391 · ON_BRANCH: 1207 · imports_from: 1134 · inherits: 824 · method: 756 · PARENT_OF: 338 · re_exports: 4


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 1315 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 14 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `616a398`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `Base` - 378 edges
2. `Reservation` - 285 edges
3. `HotelConfiguration` - 267 edges
4. `ReservationStatusEnum` - 225 edges
5. `Room` - 212 edges
6. `HotelMembership` - 189 edges
7. `RoomCategory` - 178 edges
...
Cohesion: 0.01
Nodes (157): bootstrap_configuration_fingerprint(), BootstrapConfigurationError, Shared, secret-safe binding for the Render QA bootstrap configuration., Hash the exact provider-observed values without exposing them individually., Revalidate the actor embedded in a short-lived OAuth state token.      Provider, _cors_contains_wildcard(), get_settings(), _gmail_is_active() (+149 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (159): AllocationRunPayload, AllocationRunResponse, listRoomMovementGroups(), revertRoomMovementGroup(), RoomMoveEvent, RoomMovementGroup, triggerAllocationRecalculation(), getGuestProhibitedDetail() (+151 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (169): Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, _require_demo_mode(), reset_demo(), seed_demo(), Base (+161 more)

### Community 3 - "Community 3"
Cohesion: 0.06
Nodes (200): codex/production-render-qa, codex/public-site-inquiries, docs/auditoria-pms-20260908, feature/operational-audit-cash-daily, feature/secure-auth-marketing-release-20260926, main, 009a9ed docs(tech-0140): pin final local verification, 0254334 Merge branch 'agent-ux' into codex/feature/pms-e2e-scale-hardening (+192 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (129): AuthUser, _accept_invitation(), accept_invitation_legacy_path(), _accept_invitation_with_google(), accept_invitation_with_google_legacy_path(), AcceptPayload, _activate_invitation_for_user(), _activate_invitation_for_user_audited() (+121 more)

### Community 5 - "Community 5"
Cohesion: 0.05
Nodes (141): Apply the separate approval capability only when closing requires it., Export the existing transaction/cash ledger without creating a second balance., Read and resolve the guest room-rejection lifecycle., FastAPI routes for Guest management., Add new companions to an existing guest., Public booking-engine API authenticated only with hotel API keys., Coarse payment status derived from amounts (no sensitive detail)., Read-only reservation/payment status by confirmation code (v72 §16).      Scoped (+133 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (123): BaseModel, ActiveAllocationPolicyRead, AllocationExplanationRead, AllocationFeedbackDraftRequest, AllocationPolicySuggestionApplyRequest, AllocationPolicySuggestionApplyResponse, AllocationPolicySuggestionCreate, AllocationPolicySuggestionRead (+115 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (93): create_movement(), createStockItem(), createStockLocation(), createStockMovement(), CurrentStock, deleteStockItem(), _ensure_adjustment_permission(), get_stock_summary() (+85 more)

### Community 8 - "Community 8"
Cohesion: 0.06
Nodes (132): PaymentSurchargeCreate, PaymentSurchargeRead, PaymentSurchargeUpdate, Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, FastAPI routes for Payments., 62f0868 fix(payments): record WHO triggered a payment state change, not just when, BillingAdjustment, BillingAdjustmentTypeEnum (+124 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (61): Staff management endpoints for hotel public API keys., create_payment_surcharge(), deactivate_payment_surcharge(), _get_surcharge_or_404(), _has_per_method_nightly_price(), update_payment_surcharge(), Public WhatsApp bot hooks authenticated only by hotel API key., _backfill_not_null_nulls() (+53 more)

### Community 10 - "Community 10"
Cohesion: 0.08
Nodes (129): Authenticated field-level collaboration endpoints.  Drafts are ephemeral and saf, Issue a short-lived, one-use ticket for one tenant resource., Persist an optimistic, field-level merge under the current tenant., Process-local peers plus best-effort Redis pub/sub fan-out., Authenticate with a one-use ticket, then exchange safe draft signals., Keep collaboration permissions aligned with the normal resource API., _RoomTransport, Recover invalidation domains after a cursor without exposing payloads. (+121 more)

### Community 11 - "Community 11"
Cohesion: 0.03
Nodes (71): 15142fe feat(dashboard): hotel-vocabulary pending action titles + open-drawer button (B6.2), 230041e Fix E2E MFA journeys and clarify retention schedule, 25d09dc fix(mobile): replace two competing mobile nav mechanisms with one menu button, 2bbe038 Merge remote-tracking branch 'origin/main' into codex/clarify-mfa-code, 3e274ad Keep the landing's occupancy board in step with the real planilla, 616a398 test: scope RBAC and rate E2E selectors, 98dce04 feat(nav): collapse 29-item menu to 5 daily links + collapsible groups (B6.1), b64223d Merge branch 'agent-mobilenav' into codex/feature/pms-e2e-scale-hardening (+63 more)

### Community 12 - "Community 12"
Cohesion: 0.08
Nodes (98): apply_period_to_daily_rates(), ApplyPeriodOut, _bulk_field_value(), bulk_update_daily_rate_field(), bulk_upsert_daily_rates(), BulkFieldRateIn, BulkRateIn, BulkRateOut (+90 more)

### Community 13 - "Community 13"
Cohesion: 0.04
Nodes (76): 654d69e Add reservation arrival metadata and internal comments, 8c239da Merge pull request #100 from Maximo-Paulos/feature/reservation-arrival-comment, APIKeyPurposeEnum, LinenItem, Reservation model with strict state machine. States: pending â†’ deposit_paid â†, Derive the BRM §16.2 reportable origin for a reservation.      Precedence:, reportable_origin(), ReservationSourceEnum (+68 more)

### Community 14 - "Community 14"
Cohesion: 0.03
Nodes (53): _authorize_override(), checkin(), checkin_partial(), BridgeActivity, 81d9910 feat(notifications): notification center, push subscribe, preferences (Task 9), 958fce2 Merge pull request #31 from Maximo-Paulos/feature/mobile-first-operations, bd4af0d chore(graphify): regenerate graph after A4 code splitting, cd8d8e2 perf(frontend): sane QueryClient defaults, dev-only devtools (+45 more)

### Community 15 - "Community 15"
Cohesion: 0.02
Nodes (24): a5ee7be fix(migrations): make schema-drift-era tables idempotent against self-healed production, guest checkin profile fields  Revision ID: 17f1689785f3 Revises: 20260725_res_ho, v72 gaps phase 1: Numeric precision, room score/accessibility, guest dedup+ratin, v72 gaps phase 2: guest search indexes, OTA dedup constraint, updated guest_tag_, v72 gaps phase 3: room_movement_groups table, BillingAdjustment/ReservationAdjus, v72 gaps phase 4: PRE_CHECK_IN state, RoomMoveEvent audit fields, company_docume, v72 gaps phase 5: optimistic locking on reservations  Revision ID: 20260612_v72_, v72 gaps phase 6: payment tables (payments, payment_links, payment_webhook_event (+16 more)

### Community 16 - "Community 16"
Cohesion: 0.03
Nodes (57): _DecimalAwareEncoder, _frontend_placeholder(), _InvitationAccessLogFilter, _is_demo_mode_enabled(), lifespan(), Hotel PMS — FastAPI Main Application. Serves the API + bundled frontend files., Seed the permission matrix once at boot (per worker process).      A1 fix: seed_, Initialize database on application startup. (+49 more)

### Community 17 - "Community 17"
Cohesion: 0.06
Nodes (82): Auth endpoints: register, login, email verification and password reset. Verifica, Remove the Google login only after proving control of the password.      Google-, Complete Sign in with Apple from Apple JS SDK or another OIDC client., Start Apple's form_post redirect flow with a signed state and nonce., Complete Apple's server-side form_post redirect flow., Remove the Apple login only after proving control of the password., Validate reset code without changing password (paso previo en UI)., Prefer a hotel whose onboarding is already complete.      When a user belongs to (+74 more)

### Community 18 - "Community 18"
Cohesion: 0.07
Nodes (50): booking_webhook(), despegar_webhook(), expedia_webhook(), _guarded_json_payload(), _handle_ota_webhook(), FastAPI Webhook endpoints for OTA integrations., Receive reservation notifications from Booking.com., Receive reservation notifications from Booking.com. (+42 more)

### Community 19 - "Community 19"
Cohesion: 0.08
Nodes (77): AnalyticsAIUsageMonthly, AnalyticsAlertSetting, AnalyticsAlertSnooze, AnalyticsCurrencyDisplayEnum, AnalyticsExportFormatEnum, AnalyticsExportJob, AnalyticsExportStatusEnum, FactReservationDaily (+69 more)

### Community 20 - "Community 20"
Cohesion: 0.05
Nodes (56): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, 0503c7f docs: clarify supported login and privacy terms, 09d0b08 feat(marketing): secure inquiries and enforce 90-day retention, 609a664 fix(auth): secure invitation acceptance for existing accounts, 657df29 fix: reject contact inquiries without a recipient, 9306eb5 Add public marketing inquiry flow and SEO, b48fd42 Add public marketing inquiry flow and SEO, Seo() (+48 more)

### Community 21 - "Community 21"
Cohesion: 0.05
Nodes (63): fetchPublicPricing(), LeadPayload, PublicPricing, PublicPricingPlan, submitLead(), 16473c9 Refresh Graphify graph after the marketing and metadata pass, 9aa0bee fix: align invitation and marketing release metadata, d7e130f Merge the Hotels-PMS landing page rebuild (+55 more)

### Community 22 - "Community 22"
Cohesion: 0.04
Nodes (71): ActionStepUpChallenge, ActionStepUpHandler, actionStepUpQueue, ActionStepUpTicket, apiFetch(), AuthResponsePayload, buildAuthHeaders(), buildUrl() (+63 more)

### Community 23 - "Community 23"
Cohesion: 0.05
Nodes (59): b9763c5 Secure Google account reclamation behind MFA, CredentialResponse, Window, authResponse, _auth_headers(), _complete_onboarding(), _configure_resend(), _enroll_and_confirm_mfa() (+51 more)
...
Cohesion: 0.04
Nodes (60): email_status(), getHotelConfig(), HotelConfig, HotelConfigUpdate, FastAPI routes for Hotel Configuration (Admin Panel)., Lightweight status so the frontend can check the active system email provider., updateHotelConfig(), createPaymentSurcharge() (+52 more)

### Community 26 - "Community 26"
Cohesion: 0.03
Nodes (68): 9899fcf Add the master-admin pricing editor, PermissionGate(), MasterAdminProtectedShell(), MasterAdminRoot(), navItems, MasterAdminSessionProvider(), ContactPage(), FaqPage() (+60 more)

### Community 27 - "Community 27"
Cohesion: 0.05
Nodes (63): acceptInvitation(), acceptInvitationWithGoogle(), _allow_mfa_attempt(), apple_callback(), _apple_full_name(), apple_login(), _apple_login_config(), apple_start() (+55 more)
...
Cohesion: 0.04
Nodes (40): 5616166 Merge pull request #90 from Maximo-Paulos/feature/realtime-collaboration, 9aee997 Implement real-time synchronization and collaboration, createReservation(), localIsoDate(), owner, parseMoney(), readStat(), CollaborationPatchRequest (+32 more)

### Community 30 - "Community 30"
Cohesion: 0.06
Nodes (53): _complete_master_login(), _configure_resend(), FakeResponse, A genuine Stripe retry (network timeout, no ack received) redelivers the     SAM, If MASTER_ADMIN_EMAIL happens to collide with a real tenant's login     email (a, If MASTER_ADMIN_EMAIL happens to collide with a real tenant's login     email (a, C2 regression (SQLite, syntactic only -- RLS is a Postgres-only concern,     see, Wiring proof: every authenticated master-admin call must set the RLS     bypass (+45 more)

### Community 31 - "Community 31"
Cohesion: 0.04
Nodes (33): CategoryPayload, DepositPolicyPayload, finishOnboarding(), getOnboardingStatus(), HotelIdentityPayload, OnboardingProviderSetup, OnboardingStatus, OTAChannelsPayload (+25 more)
...
Cohesion: 0.05
Nodes (43): add_companions(), addGuestCompanions(), addGuestTag(), _build_guest_ledger_csv(), buildGuestQueryString(), checkInGuestReservation(), createGuest(), _enum_value() (+35 more)
...
Cohesion: 0.10
Nodes (54): Manual OTA reservation workflows.  Sprint 1 does not rely on live channel-manage, active_reservations(), active_reservations_select(), _active_reservations_without_hotel(), _apply_corporate_pricing(), _apply_custom_deposit_amount(), _apply_manual_total_override(), _apply_pricing_result_to_reservation() (+46 more)

### Community 38 - "Community 38"
Cohesion: 0.06
Nodes (38): list_countries(), list_timezones(), Reference data endpoints used by the frontend., Return the cached IANA timezone catalog., Return the curated country -> primary IANA timezone catalog., ReferenceCountry, listCountries(), ReferenceCountry (+30 more)
...
Cohesion: 0.07
Nodes (36): acknowledgeShiftHandoff(), createOperationalTask(), createShiftHandoff(), listOperationalTaskHistory(), listOperationalTasks(), listShiftHandoffs(), OperationalTask, OperationalTaskCreate (+28 more)
...
Cohesion: 0.12
Nodes (43): Lightweight subscription tracking for enforcement and auditing (v2 tables)., Immutable ledger entry for a subscription discount or override.      This table, SubscriptionAdjustment, SubscriptionEvent, _actor_payload(), _actor_role(), _actor_user_id(), _apply_plan() (+35 more)
...
Cohesion: 0.11
Nodes (34): SimpleRateLimiter, MasterAdminAuditEvent, MasterAdminAuthLockout, MasterAdminSession, MasterBillingPolicy, MasterStripeSettings, MasterStripeWebhookEvent, MasterSystemEmailConnection (+26 more)

### Community 54 - "Community 54"
Cohesion: 0.11
Nodes (37): CheckInRequest, FastAPI routes for Check-in / Check-out., B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan, create_restriction(), _get_tenant_guest(), list_restrictions(), FastAPI routes for GuestRestriction (formal lodging-prohibition entity)., Tenant-scoped lookup. Cross-hotel access must 404, never 403 --     existence of (+29 more)
...
Cohesion: 0.07
Nodes (33): assignWhatsAppConversation(), completeWhatsAppChannel(), createWhatsAppNote(), fetchWhatsAppChannel(), fetchWhatsAppConversations(), sendWhatsAppMessage(), WhatsAppChannelStatus, WhatsAppConversation (+25 more)
...
Cohesion: 0.07
Nodes (30): ApiKeyPurpose, HotelApiKey, HotelApiKeyIssued, issueApiKey(), IssueHotelApiKeyPayload, listApiKeys(), revokeApiKey(), Category (+22 more)

### Community 59 - "Community 59"
Cohesion: 0.06
Nodes (20): export_cash_ledger_csv(), _require_cash_difference_approval_when_requested(), recover_domain_events(), stream_domain_events(), Read-only integral operations audit endpoint., 2079890 Merge pull request #98 from Maximo-Paulos/fix/realtime-under-10s, 9a75a8e Merge pull request #99 from Maximo-Paulos/feature/operational-audit-cash-daily, ce4dbca Add operational audit and daily cash dashboard (+12 more)

### Community 60 - "Community 60"
Cohesion: 0.14
Nodes (39): Thin FastAPI transport for the tenant-scoped permission service., Keep permission exceptions aligned with the staff-management boundary., Ask for one exceptional permission without changing the requester's role., Retired: grants are consumed only inside their protected action., Tenant-scoped, single-use temporary authorization grants., One short-lived authorization for one requester and one permission., TemporaryActionGrant, TemporaryActionGrantStatusEnum (+31 more)

### Community 61 - "Community 61"
Cohesion: 0.32
Nodes (34): FastAPI routes for Reservations. Complete CRUD + cancel, modify, no-show, extend, Extend a guest's stay to a new checkout date., A manual total_amount replaces the auto-quote from Tarifas entirely --     only, Enforce payload-dependent actions before loading or mutating data., §5.2 — Run allocation engine in background after reservation changes.     Create, B2: planilla de ocupación — rooms x days grid data for one hotel., Register the deferred corporate collection (v72 §3.5): settled., Cancel a reservation. Post check-in cancellations are not allowed. (+26 more)
...
Cohesion: 0.09
Nodes (35): _authorize_oauth_state_actor(), connect_integration(), connectIntegration(), _connection_error_message(), _ensure_enabled(), fetchIntegrations(), finalizeIntegrationOAuth(), _find_integration() (+27 more)

### Community 64 - "Community 64"
Cohesion: 0.08
Nodes (25): currentUser(), linkGoogle(), setPasswordWithGoogle(), cancelPaymentLinkTest(), createMercadoPagoPaymentLinkTest(), listPaymentLinkTests(), PaymentLinkTest, PaymentLinkTestCreatePayload (+17 more)
...
Cohesion: 0.12
Nodes (36): allow_master_admin_mfa_attempt(), _as_aware(), audit_master_action(), authenticate_master_login(), authenticate_master_mfa_login(), _authorize_user_for_master_panel(), _bootstrap_master_credentials_match(), create_master_admin_mfa_challenge() (+28 more)
...
Cohesion: 0.11
Nodes (11): BookingAdapterError, Booking.com Connectivity adapter.  The adapter keeps provider traffic behind a s, Acknowledge processed reservation messages in Booking's queue., A provider operation failed before it could return normalized data., NormalizedOTAReservation, OTAAdapterContext, OTAOperationResult, OTAProviderAdapter (+3 more)
...
Cohesion: 0.08
Nodes (18): requestVerification(), verifyEmail(), BrandMark(), BrandMarkProps, 9da1f66 Rebuild the public landing page as Hotels-PMS, 9eff38b Refresh Graphify graph for the landing page rebuild, a585307 Implement PMS audit remediation workflows, fbc419c Fix invalid verification email requests (+10 more)
...
Cohesion: 0.11
Nodes (24): availability(), _booking_to_read(), cancel_booking(), checkin_booking(), checkout_booking(), create_booking(), get_booking(), list_bookings() (+16 more)

### Community 86 - "Community 86"
Cohesion: 0.09
Nodes (25): ActiveRoomBlockItem, AvailableWithReviewItem, CashSessionStatusRead, daily_report(), DailyOperationalReport, getDailyOperationalReport(), getOccupancyReport(), getOperationalAlerts() (+17 more)
...
Cohesion: 0.15
Nodes (13): ABC, GCSObjectStorage, get_object_storage(), LocalObjectStorage, ObjectStat, ObjectStorageError, Minimal object-storage abstraction: put/get/delete bytes by key.  Why this exist, Google Cloud Storage adapter with lazy optional dependency loading. (+5 more)
...
Cohesion: 0.15
Nodes (21): PrivacyRetentionHold, Current legal-retention exception for one public lead or inquiry.      Changes a, _as_utc(), create_retention_hold(), _effective(), _lock_hold_table_for_write(), _mask_email(), Use a consistent table-lock order with the scheduled purge function. (+13 more)
...
Cohesion: 0.16
Nodes (23): _app_secret(), _nested(), Meta Cloud API webhook boundary.  The endpoint verifies Meta's signature before, Walk a chain of dict keys, tolerating non-dict values at any level.      Meta's, Walk a chain of dict keys, tolerating non-dict values at any level.      Meta's, receive_webhook(), _verify_token(), verify_webhook() (+15 more)
...
Cohesion: 0.20
Nodes (22): PaymentProof, PaymentProofBlob, PaymentProofStatusEnum, Private image evidence awaiting explicit staff confirmation., Private binary payload kept separately from queryable proof metadata., approve_transfer_proof(), _decode_image(), _delete_uncommitted_object() (+14 more)
...
Cohesion: 0.15
Nodes (19): MarketingPricingPlan, Public marketing surfaces: the pricing the landing page shows, and the early-acc, A plan card as the public site renders it.      `price_amount` is nullable on pu, admin_pricing(), _decode_features(), hash_source(), _ordered_plans(), _plan_catalog_fallback() (+11 more)

### Community 137 - "Community 137"
Cohesion: 0.13
Nodes (18): insert_historical_hotel_config(), Helpers for seeding schemas before a migration under test., Insert the hotel-config shape that existed before the 2026-08 changes.      Migr, _alembic(), Migration coverage for the deduplication sweep., test_category_pricing_rows_are_folded_into_hotel_scoped_price_periods(), _assert_migrated_state(), Regression/data-migration guard for 20260727_linen_split.  The owner explicitly (+10 more)

### Community 138 - "Community 138"
Cohesion: 0.10
Nodes (6): _plan(), The two endpoints the public website calls, and the ways an anonymous caller cou, A deploy that has not run the migration yet must not 500 the page., A deploy that has not run the migration yet must not 500 the page., TestLeadCapture, TestPublicPricing
...
Cohesion: 0.16
Nodes (18): Public contact form endpoint for the marketing site., create_lead(), Unauthenticated endpoints the marketing site calls.  Nothing here touches hotel, _request_source(), 222ef84 Fix auth bootstrap and harden rate limits, PublicInquiryNotificationStatus, create_public_inquiry(), _digest_key() (+10 more)
...
Cohesion: 0.24
Nodes (14): Auditable guest communications initiated from a reservation., One attempted reservation email, scoped to the owning hotel.      ``accepted`` m, ReservationEmailDelivery, ReservationEmailKindEnum, ReservationEmailStatusEnum, build_reservation_email(), _clean_email(), _kind_value() (+6 more)
...
Cohesion: 0.18
Nodes (14): createGuestRestriction(), GuestProhibitedErrorBody, GuestRestriction, GuestRestrictionCreatePayload, GuestRestrictionResolvePayload, GuestRestrictionStatus, listGuestRestrictions(), resolveGuestRestriction() (+6 more)

### Community 169 - "Community 169"
Cohesion: 0.15
Nodes (17): cancel_reservation(), change_dates(), create_new_reservation(), create_or_update_manual_ota(), _ensure_action_permission(), _ensure_manual_rate_permission(), extend_stay(), _is_manager_context() (+9 more)
...
Cohesion: 0.33
Nodes (14): _apply_manual_amounts(), _attempt_waitlist_promotion_after_release(), _audit(), _audit_waitlist_promotion(), _clean(), create_or_update_manual_ota_reservation(), _existing_user_id(), _normalize_required() (+6 more)
...
Cohesion: 0.23
Nodes (13): _attach_actor_names(), audit_timeline(), _build_audit_timeline_csv(), _current_user(), export_audit_timeline(), Export the redacted unified timeline, capped at 5,000 rows per request., Invalidate every access token previously issued to the caller., Keep useful internal references while never exposing free-form details. (+5 more)
...
Cohesion: 0.40
Nodes (5): main(), normalize_instruction_paths(), Replace this repository's absolute root in generated instructions only., 7a14668 feat(menu): wire /operacion/planilla route and add it to the nav, d5dcfae fix(graphify): normalize /operacion/ route text in graph labels too
...
Cohesion: 0.67
Nodes (1): Bound inbound webhook bodies before parsing or signature verification.
...
## Knowledge Gaps
- **1306 isolated node(s):** `add user TOTP MFA and recovery codes  Revision ID: 0c66ee6f32fa Revises: 2026082`, `add temporary action grants  Revision ID: 0f85dca5b98b Revises: 20260820_user_se`, `Install the PostgreSQL tenant policy; no-op on other dialects.`, `Remove the PostgreSQL tenant policy before dropping the table.`, `guest checkin profile fields  Revision ID: 17f1689785f3 Revises: 20260725_res_ho` (+1301 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 81`** (1 nodes): `BookingAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 156`** (2 nodes): `OnboardingState`, `Onboarding state scoped by hotel. Tracks completion of setup steps and stores dr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 190`** (1 nodes): `DespegarAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 191`** (1 nodes): `ExpediaAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 323`** (2 nodes): `FakeRedis`, `test_availability_key_shape_and_serialization()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 368`** (2 nodes): `mercadopago_payment_link_webhook()`, `_mercadopago_webhook_impl()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 407`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 417`** (2 nodes): `_context()`, `test_generic_room_status_patch_projects_event_and_reallocates()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 429`** (2 nodes): `V72 §13 — Daily Rate Management tests.  Tests cover:   - get_price_for_date: Dai`, `TestResolveRateCalendar`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 446`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 476`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 477`** (2 nodes): `frontendRoot`, `publicRoot`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 486`** (1 nodes): `Regression: provider-supplied OAuth error text must not break out of the inline`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 490`** (1 nodes): `TestBookingWebhook`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 495`** (1 nodes): `add user TOTP MFA and recovery codes  Revision ID: 0c66ee6f32fa Revises: 2026082`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 509`** (1 nodes): `repair: ensure uq_reservation_hotel_id_id exists before payment_links FK  Revisi`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 510`** (1 nodes): `Add transactions.created_by_user_id for payment audit trail.  Transaction had cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 511`** (1 nodes): `repair: create hotel_memberships table (was never migrated)  Revision ID: 202607`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 512`** (1 nodes): `Add quoted_amount_ars/quoted_amount_usd to reservations (dual manual OTA pricing`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 513`** (1 nodes): `Add first-name indexes for the tenant-scoped guest search hot path.  Revision ID`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 514`** (1 nodes): `add permission metadata and optimistic override versions  Revision ID: 20260820_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 515`** (1 nodes): `Add tenant-scoped indexes for TECH-0063 OLTP hot paths.  The indexes mirror the`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 516`** (1 nodes): `Add normal-user server-side sessions for TECH-0021.  Revision ID: 20260820_user_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 517`** (1 nodes): `add Apple subject and first-authorization display name  Revision ID: 20260821_ap`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 518`** (1 nodes): `Add soft-delete metadata to guests and payments for TECH-0110.  The columns are`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 519`** (1 nodes): `Grant receptionist the same-category room move default.  Phase A narrowed reserv`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 520`** (1 nodes): `Persist short-lived MFA step-up ticket use to prevent cross-worker replay.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 521`** (1 nodes): `Separate payment-proof reading/review from financial reports.  Revision ID: 2026`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 522`** (1 nodes): `merge stock kind fix and laundry vendor settlements  Revision ID: 513720cf2551 R`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 523`** (1 nodes): `bind users to Google subject  Revision ID: 5e86f1b9ccbb Revises: 0c66ee6f32fa Cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 524`** (1 nodes): `merge linen split and reservation quoted dual amounts  Revision ID: 6feb3dd16d0f`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 525`** (1 nodes): `repair: ensure uq_stock_items_hotel_id_id / uq_stock_locations_hotel_id_id exist`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 526`** (1 nodes): `persist staff invitation lifecycle  Revision ID: 8b5d07cc381b Revises: 20260820_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 527`** (1 nodes): `add integration catalog  Revision ID: 9b0becb6c658 Revises: 20260407_subscriptio`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 528`** (1 nodes): `guest legal profile  Revision ID: 9c0d2f3e1a44 Revises: 3eaf48a79290 Create Date`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 529`** (1 nodes): `ota hardening: hotel-scoped mappings and webhook credentials  Revision ID: a7f3d`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 530`** (1 nodes): `add payment link tests  Revision ID: b7c1f0a8f9d2 Revises: 9b0becb6c658 Create D`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 531`** (1 nodes): `baseline  Revision ID: cb9001557529 Revises:  Create Date: 2026-03-31 19:04:53.7`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 532`** (1 nodes): `repair audit_logs stray legacy columns  Revision ID: ebd2db08c7d0 Revises: 6feb3`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 533`** (1 nodes): `Add tenant-scoped object metadata without deleting legacy file references.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 534`** (2 nodes): `graphify_state()`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 535`** (2 nodes): `main()`, `validate()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 536`** (1 nodes): `Bound inbound webhook bodies before parsing or signature verification.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 537`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 539`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 540`** (1 nodes): `Fast server-side EXPLAIN ANALYZE probe for hot queries on real PostgreSQL.  Rati`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 541`** (2 nodes): `DomainEventRecoveryResponse`, `Safe cursor response used to repair missed realtime invalidations.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 542`** (2 nodes): `RoomMoveEventRead`, `RoomMovementGroupRead`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 543`** (1 nodes): `One-shot notification cycle: generate due daily reports, then deliver pending ou`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 544`** (2 nodes): `build_gemma_hotel_context()`, `_enum_value()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 552`** (1 nodes): `Defensive datastore clients for optional infrastructure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 553`** (1 nodes): `Application decorators.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 554`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 557`** (1 nodes): `Email provider abstraction for platform transactional mail.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 558`** (1 nodes): `Master admin panel backend package.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 2` to `Community 406`, `Community 9`, `Community 69`, `Community 53`, `Community 101`, `Community 145`, `Community 179`, `Community 5`, `Community 19`, `Community 4`, `Community 59`, `Community 12`, `Community 204`, `Community 39`, `Community 38`, `Community 219`, `Community 281`, `Community 54`, `Community 13`, `Community 0`, `Community 10`, `Community 56`, `Community 105`, `Community 165`, `Community 408`, `Community 32`, `Community 136`, `Community 71`, `Community 156`, `Community 28`, `Community 8`, `Community 18`, `Community 98`, `Community 122`, `Community 57`, `Community 143`, `Community 20`, `Community 157`, `Community 7`, `Community 50`, `Community 80`, `Community 60`, `Community 123`, `Community 3`, `Community 87`, `Community 17`, `Community 309`?**
  _High betweenness centrality (0.110) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 4` to `Community 61`, `Community 5`, `Community 75`, `Community 0`, `Community 53`, `Community 38`, `Community 2`, `Community 19`, `Community 8`, `Community 13`, `Community 12`, `Community 340`, `Community 256`, `Community 32`, `Community 134`, `Community 10`, `Community 71`, `Community 126`, `Community 18`, `Community 98`, `Community 37`, `Community 50`, `Community 80`, `Community 208`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 5` to `Community 85`, `Community 61`, `Community 28`, `Community 13`, `Community 2`, `Community 19`, `Community 12`, `Community 339`, `Community 54`, `Community 126`, `Community 197`, `Community 37`, `Community 18`, `Community 8`, `Community 122`, `Community 157`, `Community 60`, `Community 10`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Are the 375 inferred relationships involving `Base` (e.g. with `Alembic creates alembic_version with version_num VARCHAR(32) by default.     Som` and `Demo-only utilities: seed sample data and reset the database. Exposed only when`) actually correct?**
  _`Base` has 375 INFERRED edges - model-reasoned connections that need verification._
- **Are the 278 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Lightweight availability placeholder. When all parameters are provided,     it r`) actually correct?**
  _`Reservation` has 278 INFERRED edges - model-reasoned connections that need verification._
- **Are the 260 inferred relationships involving `HotelConfiguration` (e.g. with `AcceptPayload` and `GoogleInvitationAcceptPayload`) actually correct?**
  _`HotelConfiguration` has 260 INFERRED edges - model-reasoned connections that need verification._
