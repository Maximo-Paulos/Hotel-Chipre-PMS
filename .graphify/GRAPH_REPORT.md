...
## Corpus Check
- Large corpus: 1287 files · ~1,870,455 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 14242 nodes · 82216 edges · 503 communities detected
- Extraction: 65% EXTRACTED · 35% INFERRED · 0% AMBIGUOUS · INFERRED: 29061 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 29061 · ON_BRANCH: 25271 · contains: 7606 · calls: 5662 · MODIFIES: 5548 · rationale_for: 3981 · imports: 1459 · imports_from: 1146 · PARENT_OF: 913 · inherits: 820 · method: 745 · re_exports: 4


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 1287 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 14 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `1b6ca04`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `Reservation` - 1473 edges
2. `HotelConfiguration` - 1391 edges
3. `ReservationStatusEnum` - 1388 edges
4. `Room` - 1241 edges
5. `RoomCategory` - 1107 edges
6. `Guest` - 1024 edges
7. `RoomStatusEnum` - 752 edges
8. `Base` - 685 edges
9. `ReservationError` - 663 edges
...
Cohesion: 0.02
Nodes (608): email_status(), Lightweight status so the frontend can check the active system email provider., Lightweight status so the frontend can check the active system email provider., daily_report(), occupancy_report(), FastAPI routes for Reports & Night Audit. Daily summaries, occupancy reports, re, Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c, Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c (+600 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (280): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, _ensure_wide_version_table(), get_url(), Alembic creates alembic_version with version_num VARCHAR(32) by default.     Som, run_migrations_offline(), run_migrations_online(), _authorize_override(), checkin() (+272 more)

### Community 2 - "Community 2"
Cohesion: 0.15
Nodes (509): agent-linensplit, agent-login, agent-mobilenav, agent-otafix, agent-settlement, agent-stockedit, agent-stocksplit, agent-ux (+501 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (487): apply_period_to_daily_rates(), ApplyPeriodOut, _bulk_field_value(), bulk_update_daily_rate_field(), bulk_upsert_daily_rates(), BulkFieldRateIn, BulkRateIn, BulkRateOut (+479 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (395): checkin_partial(), B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan, B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan, B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan, B3.1: writes PRE_CHECK_IN — the 'huésped ingresó al cuarto, faltan     acompañan, add_companions(), FastAPI routes for Guest management., Add new companions to an existing guest. (+387 more)

### Community 5 - "Community 5"
Cohesion: 0.01
Nodes (175): main(), valid_https(), Shared, secret-safe binding for the Render QA bootstrap configuration., acceptInvitation(), acceptInvitationWithGoogle(), AuthProvidersResponse, AuthResponse, AuthResult (+167 more)

### Community 6 - "Community 6"
Cohesion: 0.03
Nodes (277): AuthUser, _assert_assignable_role(), _assert_manageable_membership(), _assert_manageable_role(), EmailDeliveryStatus, invite_user(), inviteUser(), list_users() (+269 more)

### Community 7 - "Community 7"
Cohesion: 0.01
Nodes (235): ApiKeyPurpose, HotelApiKey, HotelApiKeyIssued, issueApiKey(), IssueHotelApiKeyPayload, listApiKeys(), revokeApiKey(), Category (+227 more)

### Community 8 - "Community 8"
Cohesion: 0.01
Nodes (219): AllocationRunPayload, AllocationRunResponse, listRoomMovementGroups(), revertRoomMovementGroup(), RoomMoveEvent, RoomMovementGroup, triggerAllocationRecalculation(), getHotelConfig() (+211 more)

### Community 9 - "Community 9"
Cohesion: 0.04
Nodes (276): 955325e feat(security): add critical/step-up/delegable metadata and optimistic versioning to permission catalog, LookupError, HotelRole, A stable custom role code whose policy is scoped to one hotel., HotelRoleVisibilityWindow, Reservation visibility limits configured independently per hotel role.      A mi, HotelPermissionOverride, Permission (+268 more)
...
Cohesion: 0.03
Nodes (198): booking_webhook(), despegar_webhook(), expedia_webhook(), _guarded_json_payload(), _handle_ota_webhook(), FastAPI Webhook endpoints for OTA integrations., Receive reservation notifications from Booking.com., Receive reservation notifications from Expedia. (+190 more)

### Community 13 - "Community 13"
Cohesion: 0.02
Nodes (173): claude/feature/pms-roles-housekeeping-qa, 00bf891 Explain analytics errors and add retry action, 02f4bb4 Record Apple WebKit business matrix, 03832c5 Run onboarding journey on WebKit, 03f4d70 Document cloud date and quote regressions, 0449677 Document cloud role and preview blocker, 0510fa0 Add guest companion UI journey, 05d296d Fix cash freshness after reservation payments (+165 more)

### Community 14 - "Community 14"
Cohesion: 0.04
Nodes (235): apple_callback(), _apple_full_name(), apple_login(), _apple_login_config(), apple_start(), _attach_user_session_cookies(), _audit_security_event(), auth_providers() (+227 more)

### Community 15 - "Community 15"
Cohesion: 0.02
Nodes (187): FxSnapshotRead, create_laundry_remito(), _housekeeping_remito(), LinenItemCreate, LinenItemRead, LinenLocationCreate, LinenLocationRead, LinenMovementCreate (+179 more)

### Community 16 - "Community 16"
Cohesion: 0.02
Nodes (216): Authenticated field-level collaboration endpoints.  Drafts are ephemeral and saf, Issue a short-lived, one-use ticket for one tenant resource., Persist an optimistic, field-level merge under the current tenant., Authenticate with a one-use ticket, then exchange safe draft signals., Keep collaboration permissions aligned with the normal resource API., _authorize_oauth_state_actor(), Revalidate the actor embedded in a short-lived OAuth state token.      Provider, Revalidate the actor embedded in a short-lived OAuth state token.      Provider (+208 more)

### Community 17 - "Community 17"
Cohesion: 0.01
Nodes (153): AnalyticsAIChatPage(), AnalyticsAIChatResponse, AnalyticsAIStatus, AnalyticsCategoryDetailPage(), AnalyticsChannelsPage(), AnalyticsCompanyDetailPage(), AnalyticsEnvelope, analyticsErrorMessage() (+145 more)

### Community 18 - "Community 18"
Cohesion: 0.03
Nodes (182): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Lightweight availability placeholder. When all parameters are provided,     it r, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses the ca (+174 more)

### Community 19 - "Community 19"
Cohesion: 0.02
Nodes (122): _require_cash_difference_approval_when_requested(), addCashMovement(), approveCashCloseDifference(), CashCloseReport, CashCustodyHandoff, CashDailyCollector, CashDailyEntry, CashDailyPaymentMethod (+114 more)
...
Cohesion: 0.06
Nodes (88): channel_status(), complete_channel(), create_note(), inbox(), Authenticated human WhatsApp CRM inbox.  This router never calls Meta directly., Store metadata after the backend completes Embedded Signup.      No bearer token, _require_plan(), send_message() (+80 more)
...
Cohesion: 0.07
Nodes (60): BrandMarkProps, 6e6d179 Prepare release candidate for landing and onboarding, 87b0eb6 Improve marketing SEO content, linking, and metadata, 9da1f66 Rebuild the public landing page as Hotels-PMS, b48fd42 Add public marketing inquiry flow and SEO, d7e130f Merge the Hotels-PMS landing page rebuild, Seo(), SeoProps (+52 more)

### Community 26 - "Community 26"
Cohesion: 0.03
Nodes (73): Base, FxPolicy, ProductRoomCompatibility, RatePlan, RatePlanPrice, Commercial domain models for the next reservation/OTA foundation.  These tables, SellableProduct, TaxPolicy (+65 more)
...
Cohesion: 0.05
Nodes (59): CompanyDocument, CompanyDocumentStatusEnum, CompanyDocumentTypeEnum, Document/voucher associated with a company reservation (v72 §3.6).     If requir, PaymentLinkStatusEnum, Reference values for payment_links.status (stored as String)., PendingActionPriorityEnum, PendingActionStatusEnum (+51 more)

### Community 29 - "Community 29"
Cohesion: 0.04
Nodes (54): graphify_command_error(), inline_list(), main(), Parse the simple unquoted frontmatter lists used by context packs., Parse the simple unquoted frontmatter lists used by context packs., Parse the simple unquoted frontmatter lists used by context packs., Reject context commands that the installed Graphify CLI cannot route., Reject context commands that the installed Graphify CLI cannot route. (+46 more)

### Community 30 - "Community 30"
Cohesion: 0.04
Nodes (46): _booking_to_read(), cancel_booking(), checkin_booking(), checkout_booking(), create_booking(), get_booking(), list_bookings(), _project_booking_graph() (+38 more)

### Community 31 - "Community 31"
Cohesion: 0.10
Nodes (70): _assert_manageable_membership(), create_temporary_action_grant(), Thin FastAPI transport for the tenant-scoped permission service., Keep permission exceptions aligned with the staff-management boundary., Keep permission exceptions aligned with the staff-management boundary., Keep permission exceptions aligned with the staff-management boundary., Keep permission exceptions aligned with the staff-management boundary., Keep permission exceptions aligned with the staff-management boundary. (+62 more)

### Community 32 - "Community 32"
Cohesion: 0.04
Nodes (35): CategoryPayload, DepositPolicyPayload, finishOnboarding(), getOnboardingStatus(), HotelIdentityPayload, OnboardingProviderSetup, OnboardingStatus, OTAChannelsPayload (+27 more)
...
Cohesion: 0.05
Nodes (40): 2090dc4 feat(security): make TOTP MFA mandatory for Master Admin (TECH-0023), 3690b59 fix(security): close master admin control plane gaps (TECH-0071), 9a70090 feat(security): add optional TOTP MFA for master admin (TECH-0071), b355c74 fix(security): redact sensitive metadata in master admin audit events, complete_mfa_login(), dashboard_summary(), login(), me() (+32 more)

### Community 36 - "Community 36"
Cohesion: 0.04
Nodes (33): e34785c prueba, bubbleStyles, modeLabel, roleLabel, runtimeLabel, suggestedPrompts, build_gemma_hotel_context(), _enum_value() (+25 more)
...
Cohesion: 0.04
Nodes (35): create_movement(), createStockItem(), createStockLocation(), createStockMovement(), CurrentStock, deleteStockItem(), _ensure_adjustment_permission(), get_stock_summary() (+27 more)
...
Cohesion: 0.13
Nodes (39): f3141e0 fix(migrations): chain temporary action grants migration onto permission metadata head, fc31288 feat(security): add core temporary one-time action authorization (TECH-0041), _approve(), _consume(), _create_reservation(), _request(), _reservation_id(), _set_cancel_permission() (+31 more)
...
Cohesion: 0.11
Nodes (29): SimpleRateLimiter, MasterAdminAuditEvent, MasterAdminAuthLockout, MasterAdminSession, MasterBillingPolicy, MasterStripeSettings, MasterStripeWebhookEvent, MasterSystemEmailConnection (+21 more)

### Community 64 - "Community 64"
Cohesion: 0.12
Nodes (35): da6d6d2 fix(security): shorten master admin session TTL (TECH-0071), allow_master_admin_mfa_attempt(), _as_aware(), audit_master_action(), authenticate_master_login(), authenticate_master_mfa_login(), _authorize_user_for_master_panel(), _bootstrap_master_credentials_match() (+27 more)
...
Cohesion: 0.14
Nodes (32): _configure_resend(), _fake_google_claims(), _register_owner(), test_auth_flows_fail_without_connected_mail_provider(), test_auth_providers_exposes_only_public_google_capabilities(), test_google_account_can_set_password_only_with_linked_google_proof_and_csrf(), test_google_account_link_waits_for_mfa_and_preserves_password(), test_google_link_rejects_mismatched_email_without_mutation() (+24 more)

### Community 76 - "Community 76"
Cohesion: 0.07
Nodes (33): .test" is an RFC 2606 reserved TLD email-validator blocks as     special-use by, .test" is an RFC 2606 reserved TLD email-validator blocks as     special-use by, The isolated Playwright E2E backend boots with APP_ENV=test and its     specs re, The isolated Playwright E2E backend boots with APP_ENV=test and its     specs re, JWTs are stateless with no server-side blacklist, so a password reset -     the, JWTs are stateless with no server-side blacklist, so a password reset -     the, .test" is an RFC 2606 reserved TLD email-validator blocks as     special-use by, .test" is an RFC 2606 reserved TLD email-validator blocks as     special-use by (+25 more)
...
Cohesion: 0.18
Nodes (30): _client_with_db(), _issue_permission_restore_ticket(), _override_auth(), Create an action-bound test ticket; MFA issuance is covered separately., Create an RBAC-read scope or action-bound ticket for focused API tests., Create an RBAC-read scope or action-bound ticket for focused API tests., _seed_permission_restore_state(), _step_up_headers() (+22 more)
...
Cohesion: 0.09
Nodes (20): fetchPublicPricing(), LeadPayload, PublicPricing, PublicPricingPlan, submitLead(), notForYou, EarlyAccessForm(), EarlyAccessFormProps (+12 more)
...
Cohesion: 0.15
Nodes (16): 928e7ad feat(rbac): add restore-to-default for permission overrides (TECH-0040), _auth(), _client(), Issue a synthetic, action-bound ticket for permission-admin tests., Issue a synthetic RBAC-read grant or action-bound ticket., Issue a synthetic RBAC-read grant or action-bound ticket., Issue a synthetic RBAC-read grant or action-bound ticket., Issue a synthetic RBAC-read grant or action-bound ticket. (+8 more)
...
Cohesion: 0.11
Nodes (21): ActionStepUpTicketUse, Tenant-scoped replay ledger for MFA step-up tickets., Persist only the random ticket id and action binding after first use., action_step_up_ticket_matches(), consume_action_step_up_tickets(), create_action_step_up_ticket(), create_permission_admin_read_step_up_ticket(), is_permission_admin_read_action() (+13 more)
...
Cohesion: 0.13
Nodes (20): DomainEventOutbox, One durable delivery attempt for one hotel/domain invalidation., queue_domain_change(), QueuedDomainChange, Tenant-scoped realtime domain events backed by Redis/Valkey pub/sub.  Postgres r, A safe invalidation signal waiting for the surrounding DB commit., A safe invalidation signal waiting for the surrounding DB commit., A safe invalidation signal waiting for the surrounding DB commit. (+12 more)
...
Cohesion: 0.14
Nodes (14): A guest self-registering must never end up with User.role="platform_admin"     j, A guest self-registering must never end up with User.role="platform_admin"     j, A guest self-registering must never end up with User.role="platform_admin"     j, A guest self-registering must never end up with User.role="platform_admin"     j, A guest self-registering must never end up with User.role="platform_admin"     j, The isolated Playwright E2E backend boots with APP_ENV=test and its     specs re, A guest self-registering must never end up with User.role="platform_admin"     j, A guest self-registering must never end up with User.role="platform_admin"     j (+6 more)
...
Cohesion: 0.27
Nodes (12): _create_role(), Security and API contract tests for per-hotel custom roles., _step_up_headers(), test_authenticated_context_resolves_custom_role_base_and_fails_closed_for_missing_role(), test_custom_role_downgrade_aborts_before_any_data_changes(), test_custom_role_permission_precedence_invariants_and_tenant_isolation(), test_custom_role_visibility_inherits_base_and_unknown_roles_fail_closed(), test_custom_roles_can_be_assigned_and_invited_but_owner_transfer_stays_separate() (+4 more)
...
Cohesion: 0.18
Nodes (10): PermissionCatalogItem, PermissionDecision, Set both sides of one role's reservation visibility window., RolePermissionOverrideRequest, TemporaryActionGrantApproveRequest, TemporaryActionGrantConsumeRequest, TemporaryActionGrantRequest, UserPermissionOverrideRequest (+2 more)
...
Cohesion: 0.31
Nodes (9): CategoryPricingRead, CategoryPricingSchema, Pydantic schemas for Room and RoomCategory., RoomBase, RoomCategoryBase, RoomCategoryCreate, RoomCategoryRead, RoomCreate (+1 more)
...
Cohesion: 0.38
Nodes (7): _generate_code(), _issue_email_token(), _mask_email(), register(), request_reset(), _request_source(), request_verify()
...
Cohesion: 0.40
Nodes (4): Read-only integral operations audit endpoint., OperationalAuditItemRead, OperationalAuditRead, Contracts for the owner/co-owner operational audit projection.
...
Cohesion: 0.50
Nodes (4): list_groups(), Most recent movement groups for a hotel (newest first)., Most recent movement groups for a hotel (newest first)., Most recent movement groups for a hotel (newest first).
...
## Knowledge Gaps
- **1489 isolated node(s):** `add user TOTP MFA and recovery codes  Revision ID: 0c66ee6f32fa Revises: 2026082`, `add temporary action grants  Revision ID: 0f85dca5b98b Revises: 20260820_user_se`, `Install the PostgreSQL tenant policy; no-op on other dialects.`, `Remove the PostgreSQL tenant policy before dropping the table.`, `guest checkin profile fields  Revision ID: 17f1689785f3 Revises: 20260725_res_ho` (+1484 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 74`** (1 nodes): `BookingAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 87`** (1 nodes): `test_database_foundation_complete()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 151`** (1 nodes): `OnboardingState`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 159`** (2 nodes): `ExpediaAdapter`, `OTAProviderAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 165`** (1 nodes): `DespegarAdapter`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 288`** (2 nodes): `FakeRedis`, `test_availability_key_shape_and_serialization()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 366`** (2 nodes): `_context()`, `test_generic_room_status_patch_projects_event_and_reallocates()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 392`** (1 nodes): `Staff management endpoints for hotel public API keys.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 396`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 400`** (2 nodes): `HotelConfigUpdate`, `_normalize_currency()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 408`** (1 nodes): `The marketing site's own origin must not depend on an env var being set.  hotels`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 425`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 426`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 427`** (2 nodes): `ownerCredentials`, `receptionistCredentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 434`** (2 nodes): `_override_auth()`, `test_receptionist_can_get_price_quote()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 438`** (1 nodes): `Regression: provider-supplied OAuth error text must not break out of the inline`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 452`** (1 nodes): `add user TOTP MFA and recovery codes  Revision ID: 0c66ee6f32fa Revises: 2026082`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 453`** (1 nodes): `guest checkin profile fields  Revision ID: 17f1689785f3 Revises: 20260725_res_ho`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 454`** (1 nodes): `reservation financial lifecycle  Revision ID: 20260410_reservation_financial_lif`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 455`** (1 nodes): `ai assistant sessions  Revision ID: 20260411_ai_assistant_sessions Revises: 2026`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 456`** (1 nodes): `ai assistant insights  Revision ID: 20260412_ai_assistant_insights Revises: 2026`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 457`** (1 nodes): `extend onboarding state for wizard flow  Revision ID: 20260419_onboarding_wizard`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 458`** (1 nodes): `add trial and comped fields to subscriptions  Revision ID: 20260419_subscription`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 459`** (1 nodes): `master admin panel  Revision ID: 20260421_master_admin_panel Revises: 9c0d2f3e1a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 460`** (1 nodes): `master admin system owner mail and stripe settings  Revision ID: 20260421_master`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 461`** (1 nodes): `v72 gaps phase 1: Numeric precision, room score/accessibility, guest dedup+ratin`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 462`** (1 nodes): `v72 gaps phase 2: guest search indexes, OTA dedup constraint, updated guest_tag_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 472`** (1 nodes): `v72 fx rate snapshots table.  Revision ID: 20260624_fx_rate_snapshots Revises: 2`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 473`** (1 nodes): `drop orphan payment_surcharge_configs table (R5b).  main carries TWO surcharge i`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 474`** (1 nodes): `Add configurable public API rate limit per hotel.  Revision ID: 20260625_public_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 475`** (1 nodes): `v72 §16.2: add 'company' value to reservation_channel_code_enum.  Adds the corpo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 476`** (1 nodes): `add reservations.settlement_due_date for corporate deferred billing (R5b §3.5).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 477`** (1 nodes): `reservation waitlist flags (BRM v72 §9).  Add denormalized waitlist flags to res`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 478`** (1 nodes): `soft delete audited domain records.  Revision ID: 20260625_soft_delete Revises:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 479`** (1 nodes): `Store private transfer-proof bytes separately from searchable metadata.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 480`** (1 nodes): `Add private bank-transfer proof workflow.  Revision ID: 20260724_payment_proofs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 481`** (1 nodes): `Add (hotel_id, created_at) index on reservations for A2 recent-order paging.  Re`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 482`** (1 nodes): `Add (hotel_id, room_id, check_in_date, check_out_date) index on reservations for`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 483`** (1 nodes): `Add transactions.created_by_user_id for payment audit trail.  Transaction had cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 484`** (1 nodes): `repair: create hotel_memberships table (was never migrated)  Revision ID: 202607`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 485`** (1 nodes): `Add quoted_amount_ars/quoted_amount_usd to reservations (dual manual OTA pricing`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 486`** (1 nodes): `stock_items.kind (supply vs linen) + soft-delete-aware name uniqueness  Revision`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 487`** (1 nodes): `repair: ensure (hotel_id, id) unique constraints exist on rooms/room_categories/`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 488`** (1 nodes): `add idempotency_key to stock_movements  Revision ID: 20260818_stock_movement_ide`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 489`** (1 nodes): `Add normal-user server-side sessions for TECH-0021.  Revision ID: 20260820_user_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 490`** (1 nodes): `Add soft-delete metadata to guests and payments for TECH-0110.  The columns are`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 492`** (1 nodes): `Fix ota_reservation_lifecycle_enum labels to match the ORM's values_callable.  ``
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 493`** (1 nodes): `add public marketing pricing plans and early-access leads  Revision ID: 20260910`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 494`** (1 nodes): `Persist short-lived MFA step-up ticket use to prevent cross-worker replay.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 495`** (1 nodes): `merge stock kind fix and laundry vendor settlements  Revision ID: 513720cf2551 R`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 496`** (1 nodes): `bind users to Google subject  Revision ID: 5e86f1b9ccbb Revises: 0c66ee6f32fa Cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 497`** (1 nodes): `merge linen split and reservation quoted dual amounts  Revision ID: 6feb3dd16d0f`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 498`** (1 nodes): `laundry vendors and remitos  Revision ID: 795e124adaad Revises: 62fddec52b79 Cre`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 499`** (1 nodes): `repair: ensure uq_stock_items_hotel_id_id / uq_stock_locations_hotel_id_id exist`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 500`** (1 nodes): `laundry vendor settlements (quarterly paid/not-paid mark)  Revision ID: e2c4a9f7`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 502`** (1 nodes): `Add tenant-scoped object metadata without deleting legacy file references.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 503`** (2 nodes): `main()`, `validate()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 504`** (2 nodes): `BridgeActivity`, `MainActivity`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 505`** (1 nodes): `owner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 506`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 507`** (2 nodes): `{ code }`, `source`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 510`** (1 nodes): `One-shot notification cycle: generate due daily reports, then deliver pending ou`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 511`** (1 nodes): `FakeResponse`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 514`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 516`** (2 nodes): `POST /api/checkin/checkout only enforced authentication (get_auth_context),`, `test_housekeeping_cannot_checkout_reservation()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 518`** (1 nodes): `Minimal API for recording room preferences on a guest profile.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 521`** (1 nodes): `Pydantic schemas for the small guest-room preference signal.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 525`** (1 nodes): `Tighten housekeeping's default access to occupancy planning.  Revision ID: 20260`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 526`** (1 nodes): `Set one global default without creating duplicate rows on reruns.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 527`** (1 nodes): `add tenant-scoped guest room preferences  Revision ID: 3c2dea57666d Revises: 202`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 528`** (1 nodes): `Install the PostgreSQL tenant policy; no-op on other dialects.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 529`** (1 nodes): `Remove the PostgreSQL tenant policy before dropping its table.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 11` to `Community 1`, `Community 0`, `Community 63`, `Community 112`, `Community 40`, `Community 47`, `Community 3`, `Community 24`, `Community 10`, `Community 56`, `Community 71`, `Community 26`, `Community 28`, `Community 129`, `Community 124`, `Community 4`, `Community 46`, `Community 37`, `Community 122`, `Community 16`, `Community 5`, `Community 9`, `Community 13`, `Community 43`, `Community 143`, `Community 92`, `Community 21`, `Community 151`, `Community 34`, `Community 12`, `Community 45`, `Community 94`, `Community 84`, `Community 54`, `Community 14`, `Community 397`, `Community 39`, `Community 31`, `Community 106`, `Community 69`, `Community 102`, `Community 119`, `Community 511`, `Community 196`, `Community 76`, `Community 174`, `Community 105`, `Community 437`, `Community 135`, `Community 29`, `Community 443`, `Community 444`, `Community 445`, `Community 157`, `Community 164`, `Community 183`, `Community 260`?**
  _High betweenness centrality (0.140) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 0` to `Community 16`, `Community 10`, `Community 18`, `Community 4`, `Community 122`, `Community 123`, `Community 1`, `Community 11`, `Community 26`, `Community 24`, `Community 302`, `Community 28`, `Community 12`, `Community 15`, `Community 70`, `Community 21`, `Community 95`, `Community 3`, `Community 94`, `Community 39`, `Community 120`, `Community 8`, `Community 119`, `Community 511`, `Community 196`, `Community 76`, `Community 174`, `Community 110`, `Community 105`, `Community 437`, `Community 135`, `Community 58`, `Community 63`, `Community 45`, `Community 9`, `Community 443`, `Community 444`, `Community 445`, `Community 14`, `Community 157`, `Community 164`, `Community 65`, `Community 56`, `Community 59`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 0` to `Community 18`, `Community 4`, `Community 10`, `Community 34`, `Community 1`, `Community 11`, `Community 26`, `Community 3`, `Community 47`, `Community 24`, `Community 28`, `Community 46`, `Community 95`, `Community 12`, `Community 84`, `Community 430`, `Community 31`, `Community 303`, `Community 110`, `Community 56`, `Community 59`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Are the 1466 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Lightweight availability placeholder. When all parameters are provided,     it r`) actually correct?**
  _`Reservation` has 1466 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1384 inferred relationships involving `HotelConfiguration` (e.g. with `AcceptPayload` and `GoogleInvitationAcceptPayload`) actually correct?**
  _`HotelConfiguration` has 1384 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1386 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Lightweight availability placeholder. When all parameters are provided,     it r`) actually correct?**
  _`ReservationStatusEnum` has 1386 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1235 inferred relationships involving `Room` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Lightweight availability placeholder. When all parameters are provided,     it r`) actually correct?**
  _`Room` has 1235 INFERRED edges - model-reasoned connections that need verification._