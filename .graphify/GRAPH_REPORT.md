...
## Corpus Check
- Large corpus: 743 files · ~489,409 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 6801 nodes · 22615 edges · 301 communities detected
- Extraction: 64% EXTRACTED · 36% INFERRED · 0% AMBIGUOUS · INFERRED: 8197 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 8197 · contains: 4342 · calls: 3328 · MODIFIES: 1776 · ON_BRANCH: 1265 · rationale_for: 1021 · imports: 728 · imports_from: 576 · method: 574 · inherits: 533 · PARENT_OF: 275


## Input Scope
- Requested: all
- Resolved: all (source: cli)
- Included files: 743 · Candidates: recursive
- Excluded: 0 untracked · 0 ignored · 8 sensitive · 0 missing committed

## Graph Freshness
- Built from Git commit: `381fbb1`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `ReservationStatusEnum` - 538 edges
2. `Reservation` - 537 edges
3. `HotelConfiguration` - 482 edges
4. `Room` - 401 edges
5. `Guest` - 383 edges
6. `RoomCategory` - 372 edges
7. `Base` - 342 edges
...
Cohesion: 0.04
Nodes (179): FastAPI routes for Check-in / Check-out., FastAPI routes for Reports & Night Audit. Daily summaries, occupancy reports, re, Occupancy report for a date range (default: last 30 days)., Revenue report for a date range., Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c, DocumentTypeEnum, Guest, GuestTag (+171 more)
...
Cohesion: 0.04
Nodes (153): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+145 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (119): Rate limiter with DB-backed persistence for security-sensitive endpoints.  When, acceptInvitation(), AuthResponse, AuthUser, _build_auth_response(), _generate_code(), getInvitationInfo(), _issue_email_token() (+111 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (71): _clickhouse_healthcheck(), datastores_healthcheck(), _postgres_healthcheck(), _redis_healthcheck(), Application Configuration. Uses pydantic-settings for environment variable manag, _DecimalAwareEncoder, _is_demo_mode_enabled(), Hotel PMS — FastAPI Main Application. Serves the API + bundled frontend files. (+63 more)

### Community 6 - "Community 6"
Cohesion: 0.03
Nodes (121): CheckInRequest, UpdateRolePayload, BaseModel, 805b47a Unify reservation pricing with signed quote contract, BillingPolicyPayload, BillingPolicyUpdateRequest, EmailTestRequest, MasterAdminLoginRequest (+113 more)
...
Cohesion: 0.03
Nodes (94): Demo-only utilities: seed sample data and reset the database. Exposed only when, Guard endpoints so they only run in explicit demo mode or tests., Populate the database with minimal demo data.     Idempotent: running twice simp, Drop and recreate all tables.     Keeps the app in a known-good empty state for, _require_demo_mode(), reset_demo(), seed_demo(), Base (+86 more)
...
Cohesion: 0.10
Nodes (87): cancel_reservation(), create_new_reservation(), create_or_update_manual_ota(), extend_stay(), mark_no_show(), modify_reservation(), _project_reservation_graph(), FastAPI routes for Reservations. Complete CRUD + cancel, modify, no-show, extend (+79 more)
...
Cohesion: 0.11
Nodes (58): BillingAdjustment, BillingAdjustmentTypeEnum, Operational reservation models for adjustments, room moves and audit history., ReservationAdjustment, ReservationAdjustmentKindEnum, ReservationAdjustmentStatusEnum, Transaction, TransactionTypeEnum (+50 more)

### Community 15 - "Community 15"
Cohesion: 0.09
Nodes (60): deactivate_payment_surcharge(), _get_surcharge_or_404(), PaymentSurchargeCreate, PaymentSurchargeRead, PaymentSurchargeUpdate, Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, update_payment_surcharge(), GuestRatingEnum (+52 more)

### Community 16 - "Community 16"
Cohesion: 0.06
Nodes (51): _load_manifest(), main(), ManifestContinuityError, _provider_subject(), Safe error that never prints provider values., Allow only a newer observation timestamp between provider snapshots., _timestamp(), verify_manifest_continuity() (+43 more)
...
Cohesion: 0.07
Nodes (47): _derive_payment_status(), public_reservation_status(), public_reservation_status_by_code(), Coarse payment status derived from amounts (no sensitive detail)., Coarse payment status derived from amounts (no sensitive detail)., Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by id (v72 §16).      Scoped to the API key (+39 more)
...
Cohesion: 0.09
Nodes (43): GuestCompanion, RateLimitEvent, db(), db_engine(), hotel_config(), pg_engine(), Pytest configuration and fixtures. Uses an in-memory SQLite database for isolate, Create a fresh in-memory SQLite engine for each test. (+35 more)
...
Cohesion: 0.23
Nodes (32): AnalyticsAIUsageMonthly, AnalyticsAlertSetting, AnalyticsAlertSnooze, AnalyticsCurrencyDisplayEnum, AnalyticsExportFormatEnum, AnalyticsExportJob, AnalyticsExportStatusEnum, FactReservationDaily (+24 more)
...
Cohesion: 0.08
Nodes (17): _make_reservation(), Tests for V72 §9 — Waitlist and Overbooking.  Key implementation details discove, The Reservation model must have is_wait_listed and wait_list_reason fields., A reservation can be created with is_wait_listed=True and no room., Verify is_wait_listed survives a round-trip through the database., Create one normal and one waitlisted reservation, return (normal, waitlisted)., Helper — create a reservation through the service layer., is_wait_listed=True reservations with room_id=None must not affect availability. (+9 more)
...
Cohesion: 0.36
Nodes (8): FastAPI routes for Guest management., Add new companions to an existing guest., GuestQuickProfileRead, GuestRatingUpdate, GuestStaySummary, GuestTagCreate, GuestTagRead, GuestServiceError
...
- **Thin community `Community 192`** (2 nodes): `FakeRedis`, `test_availability_key_shape_and_serialization()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 197`** (2 nodes): `TestReoptimizationIntegration`, `TestReoptimizationTrigger`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 294`** (1 nodes): `drop orphan payment_surcharge_configs table (R5b).  main carries TWO surcharge i`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 295`** (1 nodes): `Add configurable public API rate limit per hotel.  Revision ID: 20260625_public_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 296`** (1 nodes): `v72 §16.2: add 'company' value to reservation_channel_code_enum.  Adds the corpo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 297`** (1 nodes): `add reservations.settlement_due_date for corporate deferred billing (R5b §3.5).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 298`** (1 nodes): `reservation waitlist flags (BRM v72 §9).  Add denormalized waitlist flags to res`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 299`** (1 nodes): `soft delete audited domain records.  Revision ID: 20260625_soft_delete Revises:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 300`** (1 nodes): `Store private transfer-proof bytes separately from searchable metadata.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 301`** (1 nodes): `Add private bank-transfer proof workflow.  Revision ID: 20260724_payment_proofs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 302`** (1 nodes): `add integration catalog  Revision ID: 9b0becb6c658 Revises: 20260407_subscriptio`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 303`** (1 nodes): `add payment link tests  Revision ID: b7c1f0a8f9d2 Revises: 9b0becb6c658 Create D`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 304`** (1 nodes): `baseline  Revision ID: cb9001557529 Revises:  Create Date: 2026-03-31 19:04:53.7`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 311`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 312`** (2 nodes): `Derive the BRM §16.2 reportable origin for a reservation.      Precedence:`, `reportable_origin()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 9` to `Community 234`, `Community 13`, `Community 5`, `Community 87`, `Community 24`, `Community 112`, `Community 30`, `Community 2`, `Community 52`, `Community 70`, `Community 3`, `Community 12`, `Community 28`, `Community 91`, `Community 1`, `Community 133`, `Community 0`, `Community 25`, `Community 15`, `Community 6`, `Community 8`, `Community 56`, `Community 23`, `Community 113`, `Community 14`, `Community 55`, `Community 26`, `Community 20`, `Community 62`, `Community 312`, `Community 38`, `Community 57`, `Community 126`, `Community 210`, `Community 80`, `Community 188`, `Community 189`?**
  _High betweenness centrality (0.126) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 0` to `Community 8`, `Community 56`, `Community 11`, `Community 4`, `Community 27`, `Community 6`, `Community 20`, `Community 158`, `Community 9`, `Community 24`, `Community 305`, `Community 52`, `Community 70`, `Community 240`, `Community 82`, `Community 55`, `Community 58`, `Community 23`, `Community 14`, `Community 15`, `Community 2`, `Community 3`, `Community 88`, `Community 57`, `Community 126`, `Community 25`, `Community 87`, `Community 80`, `Community 26`, `Community 149`, `Community 12`, `Community 41`, `Community 197`, `Community 54`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 3` to `Community 175`, `Community 20`, `Community 0`, `Community 11`, `Community 1`, `Community 9`, `Community 24`, `Community 2`, `Community 30`, `Community 52`, `Community 70`, `Community 28`, `Community 204`, `Community 55`, `Community 58`, `Community 62`, `Community 14`, `Community 15`, `Community 88`, `Community 38`, `Community 25`, `Community 108`, `Community 149`, `Community 12`, `Community 41`, `Community 197`, `Community 54`?**
...
- **Are the 475 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 475 INFERRED edges - model-reasoned connections that need verification._
- **Are the 397 inferred relationships involving `Room` (e.g. with `RoomBlockCreate` and `RoomBlockRead`) actually correct?**
  _`Room` has 397 INFERRED edges - model-reasoned connections that need verification._