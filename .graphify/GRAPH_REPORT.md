...
## Corpus Check
- Large corpus: 788 files · ~529,264 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 7448 nodes · 25456 edges · 315 communities detected
- Extraction: 62% EXTRACTED · 38% INFERRED · 0% AMBIGUOUS · INFERRED: 9552 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 9552 · contains: 4553 · calls: 3456 · MODIFIES: 2049 · ON_BRANCH: 1689 · rationale_for: 1209 · imports: 758 · imports_from: 594 · method: 591 · inherits: 535 · PARENT_OF: 470
...
## Graph Freshness
- Built from Git commit: `a78e954`
- Compare this hash to `git rev-parse HEAD` before trusting freshness-sensitive graph output.
## God Nodes (most connected - your core abstractions)
1. `ReservationStatusEnum` - 622 edges
2. `Reservation` - 622 edges
3. `HotelConfiguration` - 556 edges
4. `Room` - 480 edges
5. `Guest` - 452 edges
6. `RoomCategory` - 437 edges
7. `Base` - 373 edges
...
Cohesion: 0.03
Nodes (273): availability(), price_quote(), FastAPI routes for Booking management (thin layer over Reservation). Provides ba, Calculate pricing for a potential booking without persisting it.     Uses Catego, Calculate pricing for a potential booking without persisting it.     Uses Catego, Quickly seed demo bookings (requires DEMO_MODE=true)., Quickly seed demo bookings (requires DEMO_MODE=true)., Ensure computed fields land in the response. (+265 more)
...
Cohesion: 0.03
Nodes (154): add_companions(), FastAPI routes for Guest management., Add new companions to an existing guest., Add new companions to an existing guest., Base, Base class for all ORM models., DeclarativeBase, audited_change() (+146 more)

### Community 5 - "Community 5"
Cohesion: 0.07
Nodes (129): FastAPI routes for Reports & Night Audit. Daily summaries, occupancy reports, re, Occupancy report for a date range (default: last 30 days)., Revenue report for a date range., Night Audit / Daily Report.     Shows arrivals, departures, occupancy, revenue c, CashCloseReport, CashCustodyHandoff, CashCustodyStatusEnum, CashMovement (+121 more)

### Community 6 - "Community 6"
Cohesion: 0.03
Nodes (128): CheckInRequest, UpdateRolePayload, BaseModel, BillingPolicyPayload, BillingPolicyUpdateRequest, EmailTestRequest, MasterAdminLoginRequest, MasterAdminLoginResponse (+120 more)

### Community 7 - "Community 7"
Cohesion: 0.07
Nodes (127): ReservationAllocationLock, AnalyticsAIUsageMonthly, AnalyticsAlertSetting, AnalyticsAlertSnooze, AnalyticsCurrencyDisplayEnum, AnalyticsExportFormatEnum, AnalyticsExportJob, AnalyticsExportStatusEnum (+119 more)

### Community 8 - "Community 8"
Cohesion: 0.05
Nodes (103): HotelConfiguration, Serialize policies dict to JSON string., Check if a specific payment method is enabled., Configuration table scoped by hotel (id == hotel_id)., Parse and return extra_policies as a dictionary., BillingAdjustment, BillingAdjustmentTypeEnum, Operational reservation models for adjustments, room moves and audit history. (+95 more)

### Community 9 - "Community 9"
Cohesion: 0.08
Nodes (117): cancel_reservation(), extend_stay(), mark_no_show(), modify_reservation(), FastAPI routes for Reservations. Complete CRUD + cancel, modify, no-show, extend, §5.2 — Run allocation engine in background after reservation changes.     Create, §5.2 — Run allocation engine in background after reservation changes.     Create, Register the deferred corporate collection (v72 §3.5): settled. (+109 more)
...
Cohesion: 0.06
Nodes (90): _derive_payment_status(), public_reservation_status(), public_reservation_status_by_code(), Public booking-engine API authenticated only with hotel API keys., Coarse payment status derived from amounts (no sensitive detail)., Coarse payment status derived from amounts (no sensitive detail)., Read-only reservation/payment status by confirmation code (v72 §16).      Scoped, Read-only reservation/payment status by confirmation code (v72 §16).      Scoped (+82 more)
...
Cohesion: 0.10
Nodes (42): Exception, AllocationAssignment, AllocationAssignmentStatusEnum, AllocationExplanation, AllocationPolicyProfile, AllocationPolicyVersion, AllocationRun, AllocationRunStatusEnum (+34 more)
...
Cohesion: 0.05
Nodes (36): 08b4e0b chore(graphify): regenerate graph after Fase 7 stock inventory QA changes, 1a0097b Refresh Graphify after transfer QA fixes, 1ab7400 fix(stock): allow "Ajuste" movements to decrease stock, not only increase it, 22be4f5 chore(graphify): regenerate graph after Fase 3 reservation QA changes, 28cdf40 test(cash-register): lock in that MercadoPago/transfer payments never touch physical cash, 3082b76 fix(stock): add a direction toggle for "Ajuste" and split history labels, 4529a78 Fix hotel payment method settings save, 4a55c1c docs(qa): record Fase 6 caja findings in the regression catalog (+28 more)
...
Cohesion: 0.12
Nodes (35): PaymentProof, PaymentProofBlob, PaymentProofStatusEnum, Manual bank-transfer evidence and approval lifecycle., Private image evidence awaiting explicit staff confirmation., Private image evidence awaiting explicit staff confirmation., Private binary payload kept separately from queryable proof metadata., Private binary payload kept separately from queryable proof metadata. (+27 more)
...
Cohesion: 0.12
Nodes (29): PricePeriod, V72 §8.5 — Per-day pricing models.  DailyRate: explicit price for a (hotel, cate, Named date-range season/period for bulk rate loading.      Higher ``priority`` w, CategoryPricing, Specific pricing configuration for a room category based on payment method and c, apply_price_period(), build_pricing_revision(), get_price_for_date() (+21 more)
...
Cohesion: 0.09
Nodes (29): deactivate_payment_surcharge(), _get_surcharge_or_404(), PaymentSurchargeCreate, PaymentSurchargeRead, PaymentSurchargeUpdate, Payment surcharges API - v72 section 12.3.  GET    /api/payment-surcharges, update_payment_surcharge(), calculate_base_amount_before_surcharge() (+21 more)
...
Cohesion: 0.32
Nodes (18): _hotel(), _reservation(), test_cash_movement_requires_open_session(), test_cash_payment_posts_income_movement_to_open_session(), test_cash_payment_without_open_session_is_rejected(), test_cash_payment_without_open_session_opens_zero_balance_session(), test_cash_payment_without_open_session_still_succeeds(), test_close_creates_zero_balance_successor_and_custody_handoff() (+10 more)
...
Cohesion: 0.28
Nodes (11): Date-ranged block on a room. NULL ends_at means indefinite.     resolved_at popu, RoomBlock, HotelOutboundEmailError, is_review_for_today(), A review is 'for today' when the stay is active on `today`.      §13.3: reviews, §13.3 review routing.      If the reservation under manual review is active *tod, A review is 'for today' when the stay is active on `today`.      §13.3: reviews, A review is 'for today' when the stay is active on `today`.      §13.3: reviews (+3 more)
...
- **Thin community `Community 58`** (2 nodes): `_hotel_default_currency()`, `OTAIntegrationService`
...
- **Thin community `Community 279`** (1 nodes): `credentials`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 281`** (1 nodes): `FakeRedis`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 326`** (1 nodes): `Dependency injection helpers (auth, etc.).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 327`** (2 nodes): `Derive the BRM §16.2 reportable origin for a reservation.      Precedence:`, `reportable_origin()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Base` connect `Community 4` to `Community 238`, `Community 208`, `Community 14`, `Community 1`, `Community 239`, `Community 151`, `Community 259`, `Community 277`, `Community 26`, `Community 114`, `Community 31`, `Community 7`, `Community 9`, `Community 5`, `Community 15`, `Community 93`, `Community 61`, `Community 51`, `Community 138`, `Community 0`, `Community 6`, `Community 8`, `Community 3`, `Community 55`, `Community 27`, `Community 115`, `Community 12`, `Community 36`, `Community 28`, `Community 22`, `Community 42`, `Community 327`, `Community 140`, `Community 48`, `Community 152`, `Community 57`, `Community 241`, `Community 63`, `Community 215`, `Community 81`, `Community 197`, `Community 198`, `Community 120`?**
  _High betweenness centrality (0.128) - this node is a cross-community bridge._
- **Why does `HotelConfiguration` connect `Community 8` to `Community 11`, `Community 55`, `Community 9`, `Community 1`, `Community 29`, `Community 6`, `Community 12`, `Community 170`, `Community 3`, `Community 4`, `Community 26`, `Community 0`, `Community 320`, `Community 7`, `Community 140`, `Community 245`, `Community 83`, `Community 36`, `Community 58`, `Community 27`, `Community 51`, `Community 57`, `Community 193`, `Community 5`, `Community 81`, `Community 28`, `Community 42`, `Community 94`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `Reservation` connect `Community 0` to `Community 4`, `Community 12`, `Community 5`, `Community 9`, `Community 3`, `Community 26`, `Community 7`, `Community 31`, `Community 15`, `Community 140`, `Community 36`, `Community 58`, `Community 22`, `Community 42`, `Community 8`, `Community 48`, `Community 124`, `Community 112`, `Community 94`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Are the 620 inferred relationships involving `ReservationStatusEnum` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`ReservationStatusEnum` has 620 INFERRED edges - model-reasoned connections that need verification._
- **Are the 615 inferred relationships involving `Reservation` (e.g. with `FastAPI routes for Booking management (thin layer over Reservation). Provides ba` and `Calculate pricing for a potential booking without persisting it.     Uses Catego`) actually correct?**
  _`Reservation` has 615 INFERRED edges - model-reasoned connections that need verification._
- **Are the 549 inferred relationships involving `HotelConfiguration` (e.g. with `FastAPI routes for Hotel Configuration (Admin Panel).` and `Lightweight status so the frontend can check the active system email provider.`) actually correct?**
  _`HotelConfiguration` has 549 INFERRED edges - model-reasoned connections that need verification._
- **Are the 475 inferred relationships involving `Room` (e.g. with `RoomBlockCreate` and `RoomBlockRead`) actually correct?**
  _`Room` has 475 INFERRED edges - model-reasoned connections that need verification._