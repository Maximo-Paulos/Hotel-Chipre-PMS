...
function renderDashboard() {
  ...
  document.getElementById('dashArrivals').innerHTML = arrivalsToday.length ? arrivalsToday.map(r=>`
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border-light)">
      <div><strong>${guestName(r.guest_id)}</strong><br><span style="font-size:0.8rem;color:var(--text-muted)">Hab. ${roomLabel(r.room_id)} · ${catName(r.category_id)}</span></div>
  ...
  document.getElementById('dashDepartures').innerHTML = depsToday.length ? depsToday.map(r=>`
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border-light)">
      <div><strong>${guestName(r.guest_id)}</strong><br><span style="font-size:0.8rem;color:var(--text-muted)">Hab. ${roomLabel(r.room_id)}</span></div>
  ...
  sortedRooms.forEach(room => {
    ...
    dates.forEach(d => {
      ...
      if (res) {
        ...
        if (isStart) {
          html += `<div class="calendar-booking status-${res.status}" onclick="showReservationDetail(${res.id})" title="${gn} (${res.confirmation_code})">${gn}</div>`;
        } else {
