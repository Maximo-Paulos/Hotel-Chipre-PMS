...
def _report_type_allowed(context: AuthContext, task_type: OperationalTaskTypeEnum) -> bool:
    ...
    if context.user_role == "receptionist":
...
def get_operational_tasks(
    ...
    tasks = list_tasks(
        ...
        task_type=task_type if context.user_role not in {"housekeeping", "receptionist"} else None,
        task_types=(
        ...
        room_id=room_id,
        assigned_to_user_ids={None, context.user_id} if context.user_role in {"housekeeping", "receptionist"} else None,
        limit=limit,
    )
    return [
        serialize_task(task, include_reservation_context=context.user_role != "housekeeping")
        for task in tasks
...
def create_operational_task(
    ...
    try:
        ...
        db.commit()
        db.refresh(task)
        return serialize_task(task, include_reservation_context=context.user_role != "housekeeping")
