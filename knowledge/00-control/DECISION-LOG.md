...
| 2026-09-25 | Mantener el motor RBAC interno; al cambiar una membership, eliminar grants individuales y conservar denegaciones explícitas. | adopted | Evita que una concesión siga a un empleado a otro puesto; `SecurityAuditLog`, transacción única, confirmación en UI y pruebas en `test_invitations.py`/`test_custom_roles.py`. El ADR conserva los criterios para reabrir OpenFGA/SpiceDB. | Reabrir ante dos casos ReBAC de producto validados y los criterios del ADR. |
| 2026-09-25 | Un reenvío de invitación rota el token y deja vigente solo el enlace más reciente. | adopted | Reduce la vida de enlaces antiguos; ahora se explica en el email/UI y hay una prueba que verifica el token vigente tras reenviar. | Revisar si la evidencia operativa muestra fallos en el último enlace, no solo en los anteriores. |
