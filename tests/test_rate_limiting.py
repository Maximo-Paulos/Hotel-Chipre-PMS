...
from app.adapters.rate_limiter import reset_request_limiter, invite_limiter
...
def authed_client(client_with_db):
    ...
    def override_auth_context():
        return AuthContext(
            hotel_id=ctx["hotel_id"], user_id=ctx["user_id"], user_email=ctx["user_email"], permissions=set()
        )
...
def test_request_reset_rate_limited(client_with_db):
    ...
    reset_request_limiter.reset(email)
    try:
...
def test_invite_rate_limited(authed_client):
    ...
    invite_limiter.reset(invite_key)
