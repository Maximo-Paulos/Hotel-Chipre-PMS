...
- La migración `20260925_public_inquiry_retention` habilita `pg_cron` en el `pg_catalog` admitido por Supabase, otorga acceso al rol `postgres` sobre el esquema del scheduler y programa el borrado diario de consultas recibidas hace más de 90 días, leads sin actualizar durante más de 90 días y claves de rate limiter de más de 15 minutos. Los leads se miden desde `updated_at`; las consultas, desde `created_at`. Los datos ya vencidos se eliminan en el primer ciclo diario posterior al despliegue. En PostgreSQL local/preview sin `pg_cron`, continúa sin programar la purga solo si `APP_ENV` está configurado explícitamente como `development`, `test`, `qa` o `preview`; si falta o es desconocido, falla cerrada. No agrega un servicio Cron de Render ni una dependencia paga.
