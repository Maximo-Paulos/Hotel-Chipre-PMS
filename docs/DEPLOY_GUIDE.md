...
- La migración `20260925_public_inquiry_retention` programa en Supabase el borrado diario de consultas recibidas hace más de 90 días y leads de acceso temprano no actualizados en más de 90 días, más claves de rate limiter de más de 15 minutos. Los datos ya vencidos se eliminan en el primer ciclo diario del job después del despliegue. En PostgreSQL local/preview sin `pg_cron`, continúa con un aviso solo si `APP_ENV` está configurado explícitamente como `development`, `test`, `qa` o `preview`; si falta o es desconocido, falla cerrada. No agrega un servicio Cron de Render ni una dependencia paga.
