...
4. Un email Google verificado no basta para reemplazar ni deshabilitar la contraseña local de una cuenta preexistente. Si el usuario ya tiene MFA activo, puede vincular Google después de validar ese factor; sin MFA debe ingresar con su método actual y vincular Google desde Seguridad, confirmando tanto su contraseña de Hotels-PMS como la identidad Google verificada. El vínculo conserva el registro y la contraseña local existentes. Un `google_sub` ya vinculado a otra cuenta se rechaza.
