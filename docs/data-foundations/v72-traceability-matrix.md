...
| §8.10 | Gateway payment tracking | `payments` — `app/models/payment.py::Payment`; service `app/services/payment_webhook_service.py` | amount > 0, FK `(hotel_id, reservation_id)`, unique `(hotel_id, provider, external_payment_id)`, performance indexes | `20260612_v72_gaps_phase6_payment_tables` | `tests/test_payment_webhook_service.py`, PG upgrade coverage | ✅ |
| §8.11 | Guest-facing payment links | `payment_links` — `app/models/payment.py::PaymentLink`; service `app/services/payment_link_service.py`; router `app/api/payment_links.py` | amount checks, FK `(hotel_id, reservation_id)`, unique `link_code`, unique external reference, HMAC-signed external_reference | `20260612_v72_gaps_phase6_payment_tables` | `tests/test_payment_link_service.py`, `tests/test_payment_links_api.py` | ✅ |
| §8.12 | Payment webhook raw event log + idempotency | `payment_webhook_events` + `transactions.idempotency_key` partial unique index | FK `(hotel_id, payment_id)`, unique `(hotel_id, provider, webhook_id)`, dual-layer idempotency (re-delivery + one-transaction-per-payment) | `20260612_v72_gaps_phase6_payment_tables` + `20260613_payment_idempotency` | `tests/test_payment_webhook_service.py::test_gateway_webhook_is_idempotent_by_provider_webhook_id`, `::test_completed_gateway_payment_creates_one_transaction` | ✅ |
| §8.13 | Transactions vs payments source-of-truth rule | `transactions` canonical ledger; `payments` gateway state; `balance_due_from_transactions()` computes from confirmed transactions only | documented idempotency/source-of-truth split | `20260612_v72_gaps_phase6_payment_tables` + `20260613_payment_idempotency` | `tests/test_payment_webhook_service.py::test_balance_due_uses_transactions_not_payments`, `docs/data-foundations/transactions-vs-payments.md` | ✅ |
