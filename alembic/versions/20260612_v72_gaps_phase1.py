...
def upgrade() -> None:
    ...
    if is_postgres:
        op.execute("CREATE TYPE guest_rating_enum AS ENUM ('normal', 'excelente', 'complicado')")
        op.execute("CREATE TYPE guest_tag_type_enum AS ENUM ('no_pago', 'robo', 'conflictivo', 'vip', 'alergias', 'otro')")
        op.execute("CREATE TYPE cash_session_status_enum AS ENUM ('open', 'closed', 'pending_approval')")
        op.execute("CREATE TYPE cash_movement_type_enum AS ENUM ('income', 'expense', 'adjustment')")
        op.execute("CREATE TYPE waitlist_status_enum AS ENUM ('waiting', 'promoted', 'cancelled', 'expired')")
        op.execute("CREATE TYPE api_key_purpose_enum AS ENUM ('whatsapp_bot', 'web_engine', 'channel_manager', 'reporting', 'other')")
