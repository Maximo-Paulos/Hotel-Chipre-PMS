...
def upgrade() -> None:
    ...
    if "master_admin_sessions" not in inspector.get_table_names():
        ...
        inspector = sa.inspect(bind)
    if "ix_master_admin_sessions_session_token_hash" not in {index["name"] for index in inspector.get_indexes("master_admin_sessions")}:
        op.create_index("ix_master_admin_sessions_session_token_hash", "master_admin_sessions", ["session_token_hash"], unique=True)

    if "master_admin_audit_events" not in inspector.get_table_names():
        op.create_table(
        ...
        inspector = sa.inspect(bind)
    if "master_admin_audit_events" in inspector.get_table_names():
        existing_indexes = {index["name"] for index in inspector.get_indexes("master_admin_audit_events")}
        if "ix_master_admin_audit_events_actor_user_id" not in existing_indexes:
            op.create_index("ix_master_admin_audit_events_actor_user_id", "master_admin_audit_events", ["actor_user_id"], unique=False)
        if "ix_master_admin_audit_events_action" not in existing_indexes:
            op.create_index("ix_master_admin_audit_events_action", "master_admin_audit_events", ["action"], unique=False)

    if "master_admin_auth_lockouts" not in inspector.get_table_names():
        op.create_table(
        ...
        inspector = sa.inspect(bind)
    if "master_admin_auth_lockouts" in inspector.get_table_names():
        existing_indexes = {index["name"] for index in inspector.get_indexes("master_admin_auth_lockouts")}
        if "ix_master_admin_lockouts_login_identifier" not in existing_indexes:
            op.create_index("ix_master_admin_lockouts_login_identifier", "master_admin_auth_lockouts", ["login_identifier"], unique=False)

    if "master_billing_policies" not in inspector.get_table_names():
        op.create_table(
        ...
        inspector = sa.inspect(bind)
    if "master_billing_policies" in inspector.get_table_names():
        existing_indexes = {index["name"] for index in inspector.get_indexes("master_billing_policies")}
        if "ix_master_billing_policies_policy_key" not in existing_indexes:
            op.create_index("ix_master_billing_policies_policy_key", "master_billing_policies", ["policy_key"], unique=False)

    if "master_stripe_webhook_events" not in inspector.get_table_names():
        op.create_table(
        ...
        inspector = sa.inspect(bind)
    if "master_stripe_webhook_events" in inspector.get_table_names():
        existing_indexes = {index["name"] for index in inspector.get_indexes("master_stripe_webhook_events")}
        if "ix_master_stripe_webhook_events_event_type" not in existing_indexes:
