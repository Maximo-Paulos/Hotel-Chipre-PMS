...
def upgrade() -> None:
    ...
    if "laundry_batches" not in inspector.get_table_names():
        op.create_table(
            "laundry_batches",
    ...
    if "ix_laundry_batches_hotel_id" not in ({ix["name"] for ix in inspector.get_indexes("laundry_batches")} if inspector.has_table("laundry_batches") else set()):
    ...
    if "ix_stock_items_hotel_id" not in ({ix["name"] for ix in inspector.get_indexes("stock_items")} if inspector.has_table("stock_items") else set()):
        op.create_index("ix_stock_items_hotel_id", "stock_items", ["hotel_id"], unique=False)

    if "stock_locations" not in inspector.get_table_names():
        op.create_table(
            "stock_locations",
            sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
            sa.Column("hotel_id", sa.Integer(), nullable=False),
    ...
    if "ix_stock_locations_hotel_id" not in ({ix["name"] for ix in inspector.get_indexes("stock_locations")} if inspector.has_table("stock_locations") else set()):
        op.create_index("ix_stock_locations_hotel_id", "stock_locations", ["hotel_id"], unique=False)

    if "laundry_items" not in inspector.get_table_names():
        op.create_table(
            "laundry_items",
            sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
            sa.Column("hotel_id", sa.Integer(), nullable=False),
    ...
    if "ix_laundry_items_hotel_id" not in ({ix["name"] for ix in inspector.get_indexes("laundry_items")} if inspector.has_table("laundry_items") else set()):
    ...
    if "ix_stock_movements_hotel_id" not in ({ix["name"] for ix in inspector.get_indexes("stock_movements")} if inspector.has_table("stock_movements") else set()):
